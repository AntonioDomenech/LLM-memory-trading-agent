"""Committed-blob-only implementation delta proof for SEC/Gemma v3.4.
...
MANIFEST_VERSION = "aapl-sec-gemma-lean-v34-implementation-delta-v1"
PREREG_COMMIT = "c82029ac1b57abb5fe875c30088e1571dd5bf571"
PREREG_TREE = "27c20058eb8e8733fded5beed230684b693de328"
PREREG_DOC_PATH = "docs/aapl_sec_gemma_lean_evidence_v3_4.md"
PREREG_DOC_BLOB = "fd043697313d4f5155dbb8ec8c57109b97abb009"
PREREG_DOC_SHA256 = (
    "b1dfa90433d137e6e5f38a20c351cb7cff403d22ad8c017e7cd9eb99feb664d9"
)
IMPLEMENTATION_BASE_COMMIT = "f411ee509870336d0d90261e8b0338c8d5220c77"
IMPLEMENTATION_BASE_TREE = "e293b7a13b4e0ee3f675d0758e8b673638866341"
_COUNTERPART_MECHANICAL_REPLACEMENTS = (
    ("sec_gemma_lean_v33", "sec_gemma_lean_v34"),
    ("SecGemmaLeanV33", "SecGemmaLeanV34"),
    ("V33", "V34"),
    ("v33", "v34"),
    ("v3_3", "v3_4"),
    ("v3-3", "v3-4"),
...
class CounterpartRule:
    """Exact committed-v3.3 comparison contract for one v3.4 delta path.

    ``expected_candidate_sha256`` binds the complete v3.4 blob for ordinary
    ...
    def validate(self, path: str) -> None:
        ...
        if modes[0]:
            ...
            if (
                path != "tests/test_sec_gemma_lean_v34_delta.py"
                or redacted_name != "V34_DELTA_MODULE_SHA256"
            ...
            path != "agent_benchmark/sec_gemma_lean_v34_delta.py"
            or anchor_path != "tests/test_sec_gemma_lean_v34_delta.py"
            or anchor_name != "V34_DELTA_MODULE_SHA256"
        ):
            raise DeltaValidationError(f"{path}: invalid external-anchor contract")
        expected_exclusions = {
            "agent_benchmark/sec_gemma_lean_v34_delta.py": (
                "assignment|$module._COUNTERPART_RULE_DATA|1",
            ),
            "tests/test_sec_gemma_lean_v34_delta.py": (
                "assignment|$module.V34_DELTA_MODULE_SHA256|1",
            ),
        }.get(path, ())
...
def _counterpart_evidence(
    ...
    *,
    ...
    """Prove one committed v3.4 blob against its pinned v3.3 counterpart."""
...
def build_delta_manifest(
    ...
    *,
    ...
    v34_paths = {entry.path for entry in implementation_diff}
    overlap = inherited_paths & v34_paths
    if overlap:
        raise DeltaValidationError(
            "inherited-at-prereg blobs changed in implementation: " + ",".join(sorted(overlap))
        )
    if v34_paths != set(allow_spec.path_rules):
        unknown = sorted(v34_paths - set(allow_spec.path_rules))
        missing = sorted(set(allow_spec.path_rules) - v34_paths)
    ...
    for entry in implementation_diff:
        ...
        entry_json["classification"] = "v34_implementation_delta"
    ...
    for entry in parent_to_implementation:
        ...
        elif entry.path in v34_paths:
            classification = "v34_implementation_delta"
        else:
    ...
    if {entry["path"] for entry in classified_parent_paths} != inherited_paths | v34_paths:
    ...
    if pins == DEFAULT_PINS:
        ...
        if len(implementation_diff) != 14 or any(
            (
                entry.status != "M"
                or entry.old_mode != _REGULAR_MODE
                or entry.new_mode != _REGULAR_MODE
                or entry.old_oid == _ZERO_OID
                or entry.new_oid == _ZERO_OID
                or entry.old_oid == entry.new_oid
            )
            if entry.path
            in {
            ...
            raise DeltaValidationError(
                ...
                "and two strict regular modifications"
            )
    ...
    body: dict[str, Any] = {
        ...
        # compatibility; the bound rules below target v3.3 and same-path data.
...
# This mechanically inherited table records the reviewed v3.3 verifier shape.
# The exact v3.4 projection below selects twelve additive paths and the two
# preregistered shared-file modifications.  Older rules remain inert history.
_V33_CLONE_PATH_RULES = {
    ...
    "tests/test_sec_filing_content.py": PathRule(
        "M",
        (
            "function|test_complete_submission_ims_envelope_matrix_rejects_mixed_duplicate_unbalanced_and_cross_family|1",
            "function|test_parse_complete_submission_accepts_exclusive_legacy_ims_without_rewriting|1",
            "function|test_submission_envelope_markers_inside_text_do_not_switch_family|1",
        ),
    ),
    ...
    "agent_benchmark/sec_gemma_lean_v34_acquisition.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="95ac48d4a26a8af0f067527747a19dbd597c76a4b3b7ef7f1d2d7ddc1f290f3c",
        expected_symbol_count=216,
        expected_import_inventory_sha256="e9fddffdfa13c550d6166b447dc7044f168eb4f4e31248d3d7cacd38d46e56b6",
        expected_import_count=29,
    ),
    "agent_benchmark/sec_gemma_lean_v34_delta.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="f079725019e0496a6b43d3e234cfd9baacbd142ba62e8a13a042fd91c3c2cc1d",
    ...
    "agent_benchmark/sec_gemma_lean_v34_journal.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="caa52003efbdcb246f8e4990dcd0322a8ec4c15e0bea05977be2aa348705284b",
    ...
    "agent_benchmark/sec_gemma_lean_v34_preflight.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="9ecb9f37c01a451b97d584b742b66c6695590b2e27bed40be3a0a7a33372ae45",
        expected_symbol_count=144,
        expected_import_inventory_sha256="35d6f7f9cce4bc575cb7b58a5c0d474b3dc507af2d3da8af1eaca040aa4a30d3",
        expected_import_count=25,
    ),
    "agent_benchmark/sec_gemma_lean_v34_source.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="1bb44c721858f7491472d7fc74921e2b71c8a56478449dbb4a8e1f1d297e2824",
    ...
    "agent_benchmark/sec_gemma_lean_v34_transport.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="431af6d56fe2a897c19ebff9e19e93d7c42333f35734217b3896c82e35b368c7",
    ...
    "tests/test_sec_gemma_lean_v34_acquisition.py": PathRule(
        "A", (), (),
        ...
        expected_import_inventory_sha256="4fdaa69aefce2a7338b4a2192f6fca591145ac4449c38d34f45601d7a9cdae0d",
        expected_import_count=21,
    ),
    "tests/test_sec_gemma_lean_v34_delta.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="e369f271354c9bc8bfb9d65a1b2f59ff45c3203a8987695dd11cd35cfdf4da5a",
        expected_symbol_count=50,
        expected_import_inventory_sha256="8f328fa7b56f89a55f912883898e0a40c7f8b5e83bf11adc9f35f14cd0f99435",
        expected_import_count=10,
    ),
    "tests/test_sec_gemma_lean_v34_journal.py": PathRule(
        "A", (), (),
        ...
        expected_import_inventory_sha256="57b0087bf0bf22864f089e192f5b1e080e9f732ec3f534a30adffc3ea838ebe2",
        expected_import_count=9,
    ),
    "tests/test_sec_gemma_lean_v34_preflight.py": PathRule(
        "A", (), (),
        ...
        expected_import_inventory_sha256="14dddd2cd944bf85436daa2c170f5f29a14b8991de9ae5c17a7eaff62933b560",
        expected_import_count=11,
    ),
    "tests/test_sec_gemma_lean_v34_source.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="0c04fdc2b984dba5c5be2ad00441b57c09bc789495eaeb7ef0baeae71e556422",
        expected_symbol_count=75,
        expected_import_inventory_sha256="05099cfbbcd23a811ee8f44964bcd370324ea197d3b1a6dff57ba26160a82db7",
        expected_import_count=12,
    ),
    "tests/test_sec_gemma_lean_v34_transport.py": PathRule(
        "A", (), (),
        ...
        expected_import_inventory_sha256="f622ab201cef12a9718f38a4db5ba17a003681e0dd67420bc7fc4836e98d5f09",
        expected_import_count=16,
    ),
}

# These are exactly the twelve additive v3.4 paths.  The two shared-file
# modifications are selected separately when DEFAULT_ALLOW_SPEC is built.
_V34_IMPLEMENTATION_PATHS = (
    "agent_benchmark/sec_gemma_lean_v34_acquisition.py",
    "agent_benchmark/sec_gemma_lean_v34_delta.py",
    "agent_benchmark/sec_gemma_lean_v34_journal.py",
    "agent_benchmark/sec_gemma_lean_v34_preflight.py",
    "agent_benchmark/sec_gemma_lean_v34_source.py",
    "agent_benchmark/sec_gemma_lean_v34_transport.py",
    "tests/test_sec_gemma_lean_v34_acquisition.py",
    "tests/test_sec_gemma_lean_v34_delta.py",
    "tests/test_sec_gemma_lean_v34_journal.py",
    "tests/test_sec_gemma_lean_v34_preflight.py",
    "tests/test_sec_gemma_lean_v34_source.py",
    "tests/test_sec_gemma_lean_v34_transport.py",
)
...
_COUNTERPART_RULE_DATA = {'agent_benchmark/sec_filing_content.py': {'counterpart_path': 'agent_benchmark/sec_filing_content.py',
                                           'expected_mechanical_counts': (0, 0, 0, 0, 0, 0, 0),
                                           'expected_candidate_sha256': 'c56f874c8aa0c0af6a5c03e6fe0e4c65d42c83f19384b189f3e06ac8c12ffd99',
                                           ...
                                           'expected_changed_symbol_ids': ('function|parse_complete_submission|1',),
                                           'expected_changed_import_specs': (),
                                           'expected_changed_symbol_evidence_sha256': '75a23ad8cf2e80cffcdf2a824ad58d658be534596efd8510d1f0a3f35af515e0'},
 'agent_benchmark/sec_gemma_lean_v34_acquisition.py': {'counterpart_path': 'agent_benchmark/sec_gemma_lean_v33_acquisition.py',
                                                       'expected_mechanical_counts': (12,
                                                                                      ...
                                                                                      7,
                                                                                      2),
                                                       'expected_candidate_sha256': '694ab193f9077c308d7fb8187792afc4dd4f60ac2eecd842fc437b81ad343a1e',
                                                       'redacted_literal_assignment': None,
                                                       'expected_redacted_candidate_sha256': None,
                                                       'external_sha256_anchor_path': None,
                                                       'external_sha256_anchor_assignment': None,
 ...
 'agent_benchmark/sec_gemma_lean_v34_delta.py': {'counterpart_path': 'agent_benchmark/sec_gemma_lean_v33_delta.py',
                                                 ...
                                                 'redacted_literal_assignment': None,
                                                 'expected_redacted_candidate_sha256': None,
                                                 'external_sha256_anchor_path': 'tests/test_sec_gemma_lean_v34_delta.py',
                                                 'external_sha256_anchor_assignment': 'V34_DELTA_MODULE_SHA256',
                                                 ...
                                                 'expected_changed_symbol_ids': ('assignment|$module.IMPLEMENTATION_BASE_COMMIT|1',
                                                                                 ...
                                                                                 'assignment|$module._V32_CLONE_PATH_RULES|1',
                                                                                 'assignment|$module._V33_CLONE_PATH_RULES|1',
                                                                                 'class|CounterpartRule|1',
                                                                                 'function|_counterpart_evidence|1',
                                                                                 'function|build_delta_manifest|1'),
                                                 'expected_changed_import_specs': (),
                                                 'expected_changed_symbol_evidence_sha256': '0c33fd6a0383f635edc56b902429ccc215be0d50f2132fc8c1512dff61a3d88a'},
 'agent_benchmark/sec_gemma_lean_v34_journal.py': {'counterpart_path': 'agent_benchmark/sec_gemma_lean_v33_journal.py',
                                                   'expected_mechanical_counts': (0,
                                                                                  ...
                                                                                  3,
                                                                                  1),
                                                   'expected_candidate_sha256': 'ea164a893bf83faee1a9740312f15dd43d828e0b70750d7ad040860f592d3024',
                                                   'redacted_literal_assignment': None,
                                                   'expected_redacted_candidate_sha256': None,
                                                   'external_sha256_anchor_path': None,
                                                   'external_sha256_anchor_assignment': None,
                                                   ...
                                                   'expected_changed_import_specs': (),
                                                   'expected_changed_symbol_evidence_sha256': '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945'},
 'agent_benchmark/sec_gemma_lean_v34_preflight.py': {'counterpart_path': 'agent_benchmark/sec_gemma_lean_v33_preflight.py',
                                                     'expected_mechanical_counts': (4,
                                                                                    ...
                                                                                    12,
                                                                                    3),
                                                     'expected_candidate_sha256': 'df7a27751ce47ca686f3d3a0c4d86de19e486513014d80a35ebf2b01842cb604',
                                                     'redacted_literal_assignment': None,
                                                     'expected_redacted_candidate_sha256': None,
                                                     'external_sha256_anchor_path': None,
                                                     'external_sha256_anchor_assignment': None,
                                                     ...
                                                     'expected_changed_import_specs': (),
                                                     'expected_changed_symbol_evidence_sha256': '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945'},
 'agent_benchmark/sec_gemma_lean_v34_source.py': {'counterpart_path': 'agent_benchmark/sec_gemma_lean_v33_source.py',
                                                  'expected_mechanical_counts': (0,
                                                                                 316,
                                                                                 0,
                                                                                 15,
                                                                                 ...
                                                                                 0,
                                                                                 9),
                                                  'expected_candidate_sha256': '42ae65ce176ff7221769106e1940374941eeb15604b5fef7e1b373ad44cb07e9',
                                                  'redacted_literal_assignment': None,
                                                  'expected_redacted_candidate_sha256': None,
                                                  'external_sha256_anchor_path': None,
                                                  'external_sha256_anchor_assignment': None,
                                                  'excluded_change_symbol_ids': (),
                                                  'expected_changed_symbol_ids': ('function|_exact_header_sgml_form|1',),
                                                  'expected_changed_import_specs': (),
                                                  'expected_changed_symbol_evidence_sha256': '781c0c8954255b1a9019324ac2fee293f9e966adf69cc16ae1710f7ceb8fe2c9'},
 'agent_benchmark/sec_gemma_lean_v34_transport.py': {'counterpart_path': 'agent_benchmark/sec_gemma_lean_v33_transport.py',
                                                     'expected_mechanical_counts': (5,
                                                                                    ...
                                                                                    2,
                                                                                    3),
                                                     'expected_candidate_sha256': '7d1432f2fe0e73663b9b69e9187b0f4c266cb4b5f24544016a152ca76ee54081',
                                                     'redacted_literal_assignment': None,
                                                     'expected_redacted_candidate_sha256': None,
                                                     'external_sha256_anchor_path': None,
                                                     'external_sha256_anchor_assignment': None,
                                                     ...
                                                     'expected_changed_import_specs': (),
                                                     'expected_changed_symbol_evidence_sha256': '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945'},
 'tests/test_sec_filing_content.py': {'counterpart_path': 'tests/test_sec_filing_content.py',
                                      'expected_mechanical_counts': (0, 0, 0, 0, 0, 0, 0),
                                      'expected_candidate_sha256': 'f051fc4a9b1a453083b2a7dbddfa5142bc1f0021079938d2131f576711e71daa',
                                      'redacted_literal_assignment': None,
                                      'expected_redacted_candidate_sha256': None,
                                      'external_sha256_anchor_path': None,
                                      'external_sha256_anchor_assignment': None,
                                      'excluded_change_symbol_ids': (),
                                      'expected_changed_symbol_ids': ('function|test_complete_submission_ims_envelope_matrix_rejects_mixed_duplicate_unbalanced_and_cross_family|1',
                                                                      'function|test_parse_complete_submission_accepts_exclusive_legacy_ims_without_rewriting|1',
                                                                      'function|test_submission_envelope_markers_inside_text_do_not_switch_family|1'),
                                      'expected_changed_import_specs': (),
                                      'expected_changed_symbol_evidence_sha256': '24d85dc1962af549985c467f40b35f1f61e3ac53193e17da22885e13f9555536'},
 'tests/test_sec_gemma_lean_v34_acquisition.py': {'counterpart_path': 'tests/test_sec_gemma_lean_v33_acquisition.py',
                                                  'expected_mechanical_counts': (11,
                                                                                 ...
                                                                                 1,
                                                                                 0),
                                                  'expected_candidate_sha256': 'aa27cb2497fbae7bb41e1f68d23387fefdd7b5921fd43fb7dc1023d02b2c9789',
                                                  'redacted_literal_assignment': None,
                                                  'expected_redacted_candidate_sha256': None,
                                                  'external_sha256_anchor_path': None,
                                                  'external_sha256_anchor_assignment': None,
                                                  ...
                                                  'expected_changed_import_specs': (),
                                                  'expected_changed_symbol_evidence_sha256': '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945'},
 'tests/test_sec_gemma_lean_v34_delta.py': {'counterpart_path': 'tests/test_sec_gemma_lean_v33_delta.py',
                                            'expected_mechanical_counts': (16, 0, 8, 1, 1, 0, 0),
                                            'expected_candidate_sha256': None,
                                            'redacted_literal_assignment': 'V34_DELTA_MODULE_SHA256',
                                            'expected_redacted_candidate_sha256': '9485a007027a4ba68ec74ec22a295690818d83ccfc98c4e56701518fe9ea40b6',
                                            'external_sha256_anchor_path': None,
                                            'external_sha256_anchor_assignment': None,
                                            'excluded_change_symbol_ids': ('assignment|$module.V34_DELTA_MODULE_SHA256|1',),
                                            'expected_changed_symbol_ids': ('assignment|$module.EXPECTED_V34_COUNTERPARTS|1',
                                                                            ...
                                                                            'function|test_checker_rejects_ims_alias_rewrite_scope_expansion_and_observed_special_case|1',
                                            ...
                                            'expected_changed_import_specs': (),
                                            'expected_changed_symbol_evidence_sha256': '79a295f0a1b5633923d196b6b2b8d9201a08b5c60f9c72dcd5cc5f94c320dafd'},
 'tests/test_sec_gemma_lean_v34_journal.py': {'counterpart_path': 'tests/test_sec_gemma_lean_v33_journal.py',
                                              'expected_mechanical_counts': (2, 43, 0, 0, 0, 0, 0),
                                              'expected_candidate_sha256': 'ced6008c5936fdefa4f461cbad6bf5986ab9791fd977a65b4cd3443b79130f29',
                                              'redacted_literal_assignment': None,
                                              'expected_redacted_candidate_sha256': None,
                                              'external_sha256_anchor_path': None,
                                              'external_sha256_anchor_assignment': None,
                                              ...
                                              'expected_changed_import_specs': (),
                                              'expected_changed_symbol_evidence_sha256': '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945'},
 'tests/test_sec_gemma_lean_v34_preflight.py': {'counterpart_path': 'tests/test_sec_gemma_lean_v33_preflight.py',
                                                'expected_mechanical_counts': (2,
                                                                               ...
                                                                               2,
                                                                               0),
                                                'expected_candidate_sha256': '77673d64e76f3a0befc49c2d4d53ef16acada49eb1a532e268671b5ed3592364',
                                                'redacted_literal_assignment': None,
                                                'expected_redacted_candidate_sha256': None,
                                                'external_sha256_anchor_path': None,
                                                'external_sha256_anchor_assignment': None,
                                                ...
                                                'expected_changed_import_specs': (),
                                                'expected_changed_symbol_evidence_sha256': '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945'},
 'tests/test_sec_gemma_lean_v34_source.py': {'counterpart_path': 'tests/test_sec_gemma_lean_v33_source.py',
                                             'expected_mechanical_counts': (2, 49, 0, 0, 0, 0, 1),
                                             'expected_candidate_sha256': 'fbfe7ad2d955c8c0f81e8336b6936d1a1ec355b7383a770afbaca49f0b7088ad',
                                             'redacted_literal_assignment': None,
                                             'expected_redacted_candidate_sha256': None,
                                             'external_sha256_anchor_path': None,
                                             'external_sha256_anchor_assignment': None,
                                             'excluded_change_symbol_ids': (),
                                             'expected_changed_symbol_ids': ('function|test_full_synthetic_source_seal_replays_legacy_ims_original_bytes|1',
                                                                             'function|test_legacy_ims_header_form_accepts_horizontal_whitespace_only|1'),
                                             'expected_changed_import_specs': (),
                                             'expected_changed_symbol_evidence_sha256': '25eff8c2b15df8647df3a7364d2dd3539fc608c878cd27c5802f0a599cc3ead7'},
 'tests/test_sec_gemma_lean_v34_transport.py': {'counterpart_path': 'tests/test_sec_gemma_lean_v33_transport.py',
                                                'expected_mechanical_counts': (1,
                                                                               ...
                                                                               0,
                                                                               0),
                                                'expected_candidate_sha256': 'd2276dd8b4e32d131ef4677863ace688fe55e5895eca834d4663062c84b94cf2',
                                                'redacted_literal_assignment': None,
                                                'expected_redacted_candidate_sha256': None,
                                                'external_sha256_anchor_path': None,
                                                'external_sha256_anchor_assignment': None,
...
_DEFAULT_EXISTING_PATH_RULES = {
    path: _V33_CLONE_PATH_RULES[path]
    for path in (
        "agent_benchmark/sec_filing_content.py",
        *_V34_IMPLEMENTATION_PATHS,
        "tests/test_sec_filing_content.py",
...
_DEFAULT_COUNTERPART_RULES = {
    ...
    for path in _DEFAULT_EXISTING_PATH_RULES
}
