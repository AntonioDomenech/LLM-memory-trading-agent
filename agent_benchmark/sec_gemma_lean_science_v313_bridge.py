"""Read-only v3.8-to-v3.13 SEC science bridge.
...
def _contract_module() -> Any:
    # The contract is a separate allowlisted v3.13 file and may be written by a
...
def _v313_sha(value: Any) -> str:
    try:
        return _contract_module().canonical_sha256(value)
    except BridgeViolation:
        raise
    except Exception:
        raise BridgeViolation("v313_canonical_value_invalid") from None


def _v313_self_hash(value: Mapping[str, Any], field: str) -> dict[str, Any]:
    if field in value:
        raise BridgeViolation("v313_self_hash_invalid")
    body = dict(value)
    return {**body, field: _v313_sha(body)}
...
def _row_identity(row: Mapping[str, Any]) -> dict[str, Any]:
    try:
        ...
        typed_hash = _v313_sha(_typed_json_identity(typed))
        ...
        compatibility_record = {
            ...
            "source_record_sha256": _v313_sha(source_body),
        ...
        return {
            ...
            "compatibility_record_sha256": _v313_sha(validated),
        }
    except BridgeViolation:
        raise
    except Exception:
    ...
    *,
    ...
    try:
        ...
        for source_sequence, row in enumerate(rows):
            ...
            source_order.append(_v313_self_hash(body, "source_order_row_sha256"))
        ...
        for item in source_order:
            ...
            prior_links.append(_v313_self_hash(body, "prior_link_sha256"))
        ...
        for event_sequence, item in enumerate(event_rows):
            ...
            event_order.append(_v313_self_hash(body, "event_order_row_sha256"))
        ...
        compatibility_body = {
            ...
            "nullable_filename_bitmap_sha256": _v313_sha(missing_bitmap),
            "source_order_sha256": _v313_sha(source_order),
            "event_order_sha256": _v313_sha(event_order),
            "prior_links_sha256": _v313_sha(prior_links),
        }
        compatibility_manifest = _v313_self_hash(
        ...
        universe_body = {
            ...
            "universe_semantic_sha256": _v313_sha(universe_semantic),
        }
        universe = _v313_self_hash(universe_body, "universe_sha256")
        try:
        ...
        content_manifest = _v313_self_hash(
        ...
        manifest_body = {
            ...
            "calendar_sessions_sha256": contract.CALENDAR_SESSIONS_SHA256,
            "universe_event_proofs_sha256": _v313_sha(universe_event_proofs),
            "documents_sha256": _canonical_sha(legacy_rows),
            "records_sha256": _v313_sha(universe_records),
            ...
            "primary_documents_sha256": _v313_sha(
                [
                    {
        ...
        manifest = _v313_self_hash(manifest_body, "bridge_sha256")
