"""Zero-effect production preflight for SEC/Gemma lean evidence v3.8.
...
SCHEMA_VERSION: Final[str] = "aapl-sec-gemma-lean-evidence-v3-8-preflight-v1"
BRANCH_NAME: Final[str] = "codex/aapl-sec-gemma-lean-evidence-v3-8"
...
CHECKPOINT_ROOT: Final[str] = "data/aapl_sec_gemma_lean_evidence_v3_8"
PREFLIGHT_ARTIFACT_PATH: Final[str] = (
    "e/aapl_sec_gemma_lean_evidence_v3_8/PREFLIGHT.json"
)
...
class SecGemmaLeanV38PreflightError(RuntimeError):
    ...
    def __repr__(self) -> str:
        return f"SecGemmaLeanV38PreflightError(code={self.code!r})"
...
def _resolved_git_executable() -> str:
    ...
    if candidate is None:
        raise SecGemmaLeanV38PreflightError("git_check_unavailable")
    try:
        path = Path(candidate).absolute()
        details = path.lstat()
        resolved = path.resolve(strict=True)
    except OSError as exc:
        raise SecGemmaLeanV38PreflightError("git_check_unavailable") from exc
    if (
...
def _canonical_json(value: Any) -> bytes:
    try:
        ...
        raise SecGemmaLeanV38PreflightError("receipt_not_canonical") from exc
...
def _strict_json_object(payload: bytes, code: str) -> dict[str, Any]:
    if type(payload) is not bytes or not payload or payload.startswith(b"\xef\xbb\xbf"):
        raise SecGemmaLeanV38PreflightError(code)
    ...
    try:
        ...
        raise SecGemmaLeanV38PreflightError(code) from exc
...
def _canonical_repo_root(value: Path) -> Path:
    if not isinstance(value, Path):
        raise SecGemmaLeanV38PreflightError("repo_root_invalid")
    try:
        absolute = value.absolute()
        resolved = absolute.resolve(strict=True)
        details = absolute.lstat()
    except OSError as exc:
        raise SecGemmaLeanV38PreflightError("repo_root_invalid") from exc
...
def _relative_path(value: str, *, suffix: str | None = None) -> str:
    if type(value) is not str or "\\" in value:
        raise SecGemmaLeanV38PreflightError("path_contract_invalid")
...
def _git_text(
    ...
    *arguments: str,
    ...
    if result.returncode != 0:
        raise SecGemmaLeanV38PreflightError("git_state_invalid")
    try:
        return result.stdout.decode("utf-8", errors="strict").strip()
    except UnicodeDecodeError as exc:
        raise SecGemmaLeanV38PreflightError("git_state_invalid") from exc
...
def _assert_git_toplevel(repo_root: Path, *, git_runner: GitRunner) -> None:
    ...
    try:
        ...
        raise SecGemmaLeanV38PreflightError("git_repository_redirected") from exc
    if observed != repo_root:
        raise SecGemmaLeanV38PreflightError("git_repository_redirected")
    ...
    *,
    ...
    for part in PurePosixPath(_relative_path(relative_path)).parts:
        ...
        try:
            ...
            raise SecGemmaLeanV38PreflightError(error_code) from exc
        if stat.S_ISLNK(details.st_mode) or _is_reparse(details):
            raise SecGemmaLeanV38PreflightError(error_code)
    if (
        details is None
        or not stat.S_ISREG(details.st_mode)
    ...
    *,
    ...
    if bind_runner:
        try:
            ...
            raise SecGemmaLeanV38PreflightError("execution_root_mismatch") from exc
        if actual != expected:
            raise SecGemmaLeanV38PreflightError("execution_root_mismatch")

    hashes: dict[str, str] = {}
    loaded: list[str] = []
    normalized_paths = tuple(sorted(set(paths)))
    if tuple(paths) != normalized_paths:
        raise SecGemmaLeanV38PreflightError("executed_source_invalid")
...
def _verify_repository(
    ...
    *,
    ...
    if upstream_ref != EXPECTED_UPSTREAM:
        raise SecGemmaLeanV38PreflightError("upstream_changed")
    upstream_head = _sha1(
        _git_text(root, "rev-parse", "@{upstream}", git_runner=git_runner)
    )
    if upstream_head != head:
        raise SecGemmaLeanV38PreflightError("head_not_at_cached_upstream")
    if not _git_success(
        root,
        "merge-base",
        "--is-ancestor",
        ...
        raise SecGemmaLeanV38PreflightError("preregistration_not_ancestor")
    doc_blob = _sha1(
        _git_text(
            root,
            "rev-parse",
    ...
    if doc_blob != delta.PREREG_DOC_BLOB:
        raise SecGemmaLeanV38PreflightError("preregistration_changed")
    ...
    try:
        ...
        raise SecGemmaLeanV38PreflightError("implementation_delta_invalid") from exc
    if validated != manifest:
        raise SecGemmaLeanV38PreflightError("implementation_delta_invalid")
...
def _verify_private_contact(
    ...
    *,
    ...
    if _git_success(
        ...
        raise SecGemmaLeanV38PreflightError("private_config_tracked")
    if not _git_success(
        root,
        "check-ignore",
        "--quiet",
        ...
        raise SecGemmaLeanV38PreflightError("private_config_not_ignored")
    ...
    try:
        ...
        if type(payload.get("secrets")) is not dict:
            raise SecGemmaLeanV38PreflightError("private_config_invalid")
    ...
    except SecGemmaLeanV38PreflightError:
        raise
    except Exception as exc:
        raise SecGemmaLeanV38PreflightError("private_contact_invalid") from exc
    finally:
        raw = b""
        payload = None
        contact = None
    if valid is not True or type(fingerprint) is not str or _TAGGED_SHA256_RE.fullmatch(fingerprint) is None:
        raise SecGemmaLeanV38PreflightError("private_contact_invalid")
    return fingerprint
...
def _safe_directory(path: Path, repo_root: Path) -> None:
    try:
        ...
        raise SecGemmaLeanV38PreflightError("checkpoint_root_invalid") from exc
    current = repo_root
    for part in relative.parts:
        ...
        try:
            ...
            raise SecGemmaLeanV38PreflightError("checkpoint_root_invalid") from exc
        if stat.S_ISLNK(details.st_mode) or _is_reparse(details):
            raise SecGemmaLeanV38PreflightError("checkpoint_root_invalid")
...
def _prepare_checkpoint_root(
    ...
    *,
    ...
    if not _git_success(
        ...
        raise SecGemmaLeanV38PreflightError("checkpoint_root_not_ignored")
    checkpoint = root / Path(relative)
    _safe_directory(checkpoint, root)
    try:
        checkpoint.mkdir(parents=True, exist_ok=True)
    except OSError as exc:
        raise SecGemmaLeanV38PreflightError("checkpoint_root_unwritable") from exc
    _safe_directory(checkpoint, root)
    return checkpoint
...
def _assert_no_active_run(
    ...
    *,
    ...
    try:
        ...
        raise SecGemmaLeanV38PreflightError("active_run_check_failed") from exc
    for candidate in candidates:
        ...
        if candidate.name == ACTIVE_LOCK_NAME or not _existing_lock_is_available(candidate):
            raise SecGemmaLeanV38PreflightError("active_run_present")
...
class _PreflightLease(AbstractContextManager["_PreflightLease"]):
    ...
    def __enter__(self) -> "_PreflightLease":
        payload = _canonical_json(
            {
                "schema_version": "aapl-sec-gemma-lean-evidence-v3-8-preflight-lease-v1",
                "pid": os.getpid(),
        ...
        try:
            ...
            raise SecGemmaLeanV38PreflightError("active_run_present") from exc
        except OSError as exc:
            self._best_effort_cleanup()
            raise SecGemmaLeanV38PreflightError("run_lock_unavailable") from exc
        return self
    ...
    def __exit__(self, exc_type: Any, exc: Any, traceback: Any) -> bool:
        ...
        try:
            ...
            raise SecGemmaLeanV38PreflightError("run_lock_cleanup_failed") from None
        if close_failed:
            raise SecGemmaLeanV38PreflightError("run_lock_cleanup_failed")
        return False


def _write_probe(checkpoint_root: Path) -> None:
    path = checkpoint_root / f".write-probe-{uuid.uuid4().hex}"
    expected = b"aapl-sec-gemma-lean-evidence-v3-8-write-probe\n"
    try:
        ...
        if path.read_bytes() != expected:
            raise SecGemmaLeanV38PreflightError("checkpoint_probe_mismatch")
    except SecGemmaLeanV38PreflightError:
        raise
    except OSError as exc:
        raise SecGemmaLeanV38PreflightError("checkpoint_root_unwritable") from exc
        ...
        try:
            ...
            raise SecGemmaLeanV38PreflightError("checkpoint_probe_cleanup_failed") from exc
...
def _assert_no_prior_identity_probe(checkpoint_root: Path) -> None:
    for name in (
        ...
        try:
            ...
            raise SecGemmaLeanV38PreflightError("identity_probe_state_invalid") from exc
        raise SecGemmaLeanV38PreflightError("identity_probe_already_attempted")
...
def _write_durable_marker(
    ...
    if name not in {
        ...
        raise SecGemmaLeanV38PreflightError("identity_probe_state_invalid")
    ...
    try:
        ...
        raise SecGemmaLeanV38PreflightError("identity_probe_already_attempted") from exc
    except OSError as exc:
...
def _durable_identity_probe_core(
    ...
    *,
    ...
    intent_body = {
        "schema_version": (
            "aapl-sec-gemma-lean-evidence-v3-8-test-identity-intent-v1"
            if injected
            else "aapl-sec-gemma-lean-evidence-v3-8-identity-intent-v1"
        ),
    ...
    try:
        ...
        raise SecGemmaLeanV38PreflightError("runtime_identity_invalid") from exc
    ...
    if (
        ...
        raise SecGemmaLeanV38PreflightError("runtime_identity_invalid")
    completion_body = {
        "schema_version": (
            "aapl-sec-gemma-lean-evidence-v3-8-test-identity-complete-v1"
            if injected
            else "aapl-sec-gemma-lean-evidence-v3-8-identity-complete-v1"
        ),
        "test_injections_used": injected,
        "attempt_nonce_sha256": attempt_nonce_sha256,
...
class _LoopbackJSONClient:
    ...
    def request(self, method: str, path: str, *, body: bytes) -> bytes:
        headers = {
            ...
            "User-Agent": "aapl-sec-gemma-lean-v3.8-preflight/1",
        }
...
class _RestrictedIdentityTransport:
    ...
    def __init__(self, delegate: Any):
        if not callable(getattr(delegate, "request", None)):
            raise SecGemmaLeanV38PreflightError("runtime_transport_invalid")
    ...
    def request(self, method: str, path: str, *, body: bytes) -> bytes:
        ...
        if position >= len(ALLOWED_IDENTITY_SEQUENCE) or (method, path) != ALLOWED_IDENTITY_SEQUENCE[position]:
            raise SecGemmaLeanV38PreflightError("runtime_probe_request_forbidden")
...
def _verify_tags(tags_bytes: bytes) -> dict[str, Any]:
    ...
    if set(payload) != {"models"} or type(payload["models"]) is not list:
        raise SecGemmaLeanV38PreflightError("runtime_tags_invalid")
...
def _build_report(
    *,
    ...
    if before_hash != after_hash:
        raise SecGemmaLeanV38PreflightError("repository_changed_during_preflight")
    try:
    ...
    if (
        ...
        raise SecGemmaLeanV38PreflightError("clock_invalid")
...
def _run_untrusted_preflight(
    ...
    *,
    ...
    try:
        ...
        raise SecGemmaLeanV38PreflightError("untrusted_preflight_failed") from exc


def _validate_sealable_report(value: Any) -> dict[str, Any]:
    if type(value) is not dict:
        raise SecGemmaLeanV38PreflightError("untrusted_report_not_sealable")
    report = value
    ...
    required_loaded = {
        "agent_benchmark",
        "agent_benchmark.sec_gemma_lean_v38_delta",
...
def _validate_durable_probe_state(
    ...
    valid = (
        ...
        == "aapl-sec-gemma-lean-evidence-v3-8-identity-intent-v1"
        ...
        == "aapl-sec-gemma-lean-evidence-v3-8-identity-complete-v1"
    ...
    *,
    ...
    def current_snapshot() -> dict[str, str]:
        ...
        if (
            ...
            raise SecGemmaLeanV38PreflightError("acquisition_repository_invalid")
    ...
    if committed.returncode != 0 or committed.stdout != artifact_bytes:
        raise SecGemmaLeanV38PreflightError("committed_preflight_invalid")
    ...
    if not _git_success(
        ...
        raise SecGemmaLeanV38PreflightError("evidence_commit_invalid")
    if not _git_success(
        root,
        "merge-base",
        "--is-ancestor",
    ...
    if evidence_diff.returncode != 0 or evidence_diff.stdout != expected_diff:
        raise SecGemmaLeanV38PreflightError("evidence_commit_not_artifact_only")
    try:
        recomputed = dict(
            delta_builder(
                root,
    ...
    if contact != report["private_sec_contact"]["sha256"]:
        raise SecGemmaLeanV38PreflightError("private_contact_changed")
    ...
    if (
        ...
        raise SecGemmaLeanV38PreflightError("repository_changed_during_validation")
    return {
        "schema_version": "aapl-sec-gemma-lean-evidence-v3-8-acquisition-gate-v1",
        "validated": True,
...
def _publish_complete_body(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        raise SecGemmaLeanV38PreflightError("publish_state_invalid") from None
    return {
        "schema_version": "aapl-sec-gemma-lean-evidence-v3-8-publish-complete-v1",
...
def _validate_publish_complete(
    ...
    if payload != _canonical_json(marker) + b"\n":
        raise SecGemmaLeanV38PreflightError("publish_state_invalid")
    expected_keys = {
        "schema_version",
        "preflight_sha256",
        "artifact_path",
...
def _atomic_publish(
    ...
    try:
        ...
        raise SecGemmaLeanV38PreflightError("preflight_artifact_unwritable") from exc
    _safe_directory(target.parent, repo_root)
    if target.exists():
        raise SecGemmaLeanV38PreflightError("preflight_already_sealed")
    ...
    try:
        if checkpoint_root.stat().st_dev != target.parent.stat().st_dev:
            raise SecGemmaLeanV38PreflightError("preflight_artifact_unwritable")
    except OSError as exc:
        raise SecGemmaLeanV38PreflightError("preflight_artifact_unwritable") from exc
    ...
    try:
        ...
        if not renamed:
            try:
                ...
                pass
        raise SecGemmaLeanV38PreflightError("preflight_already_sealed") from None
    except OSError:
        if not renamed:
            try:
                temporary.unlink(missing_ok=True)
        ...
        raise SecGemmaLeanV38PreflightError("preflight_artifact_unwritable") from None
    return relative
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="SEC/Gemma lean v3.8 zero-effect preflight")
    parser.add_argument("repo_root", nargs="?", default=".")
    arguments = parser.parse_args(argv)
    try:
        report = run_preflight(Path(arguments.repo_root))
    except SecGemmaLeanV38PreflightError as exc:
