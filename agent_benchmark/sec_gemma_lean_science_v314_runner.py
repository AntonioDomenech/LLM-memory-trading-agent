"""Development-only v3.14 SEC-to-science execution worker.
...
command after the separately pushed v3.14 preflight has authorized it.
"""
...
RUNNER_SCHEMA_VERSION: Final[str] = "aapl-sec-gemma-lean-science-v3-14-runner-v1"
MODEL_PLAN_SCHEMA_VERSION: Final[str] = "aapl-sec-gemma-lean-science-v3-14-model-plan-v1"
...
RUNTIME_AGGREGATE_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-14-runtime-aggregate-v1"
)
...
SEMANTIC_EVENT_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-14-semantic-event-v1"
)
SEMANTIC_ROW_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-14-semantic-row-v1"
)
SEMANTIC_BATCH_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-14-semantic-batch-v1"
)
DETERMINISTIC_PAYLOAD_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-14-deterministic-payload-v1"
)
TERMINAL_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-14-development-result-v1"
)
...
CONTINUATION_DOCUMENT_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-14-continuation-document-v1"
)
...
class V314RunnerError(RuntimeError):
    """A fixed public-safe v3.14 runner failure."""
    ...
    def __repr__(self) -> str:
        return f"V314RunnerError(code={self.code!r})"


def _fail(code: str) -> None:
    raise V314RunnerError(code)
...
def _validate_nullable_proof(
    ...
    *,
    ...
    """Validate the V3.14 proof without entering the forbidden legacy validator."""
...
def _build_nullable_model_bundle(
    ...
    """Own the V3.14 nullable request and model-slice construction boundary."""

    try:
        from .sec_filing_content import normalize_filing_text
        from .sec_filing_gemma_contract import CANONICAL_IDENTITY_LEXICON
        from .sec_filing_gemma_preprocessor import preprocess_filing_event
        ...
        try:
            ...
            request = {
                ...
                "supplied_sentence_ids": [
                    sentence["id"] for sentence in preprocessed["sentences"]
                ],
                "request_sha256": hashlib.sha256(request_bytes).hexdigest(),
                "request_bytes": request_bytes,
            }
            request = contract.validate_blinded_model_request(request, proof=proof)
    ...
    try:
        model_slice = contract.validate_nullable_model_slice(
            contract.build_nullable_model_slice(requests, proofs)
        )
    except Exception:
        _fail("nullable_model_slice_invalid")
    return universe, requests, proofs, model_slice
...
def build_model_plan(projection: Any) -> ModelPlan:
    ...
    try:
        ...
        universe, requests, proofs, model_slice = _build_nullable_model_bundle(
            projection
        )
    except V314RunnerError:
        raise
...
def build_deterministic_payload(
    *,
    ...
    """Mint frozen feature rows while retaining v3.14 segment provenance."""
    ...
    source_commitments = {
        **source_body,
        "v314_source_commitments_sha256": contract.canonical_sha256(source_body),
    }
...
def _v314_market_plan() -> dict[str, Any]:
    try:
...
def _checkpoint_state(*, phase: str, request_sha256: str, body_sha256: str) -> dict[str, Any]:
    return {
        "schema_version": "aapl-sec-gemma-lean-science-v3-14-checkpoint-state-v1",
        "phase": phase,
...
def _assert_complete_v314_private_namespace_private_free(
    ...
    repository and v3.14-private paths.  Those paths remain forbidden in every
...
def _fsync_public_directory(path: Path) -> None:
    try:
        from .sec_gemma_lean_science_v314_journal import fsync_directory
...
def build_comparison_update(existing: bytes, result: Mapping[str, Any]) -> bytes:
    ...
    marker = "| SEC/Gemma lean science v3.13 |"
    ...
    row = (
        "| SEC/Gemma lean science v3.14 | Authenticated v3.8 source bridge, local "
    ...
    existing_v314 = [
        ...
        if line.startswith("| SEC/Gemma lean science v3.14 |")
    ]
    if existing_v314:
        if existing_v314 != [(positions[0] + 1, row)]:
...
def _terminal_material_sha256(
    ...
    *,
    ...
    try:
        from .sec_gemma_lean_science_v314_store import (
    ...
    if (
        ...
        or _regular_file_bytes(
            comparison_path.with_name(
                comparison_path.name + ".v314-pending"
            ),
...
def _prepare_terminal_candidate(
    ...
    *,
    ...
    _assert_complete_v314_private_namespace_private_free(
        store,
        forbidden_tokens=forbidden_tokens,
    )
    return candidate, public
...
def publish_result_artifacts(repo_root: Path, value: Mapping[str, Any]) -> None:
    ...
    pending = comparison_path.with_name(comparison_path.name + ".v314-pending")
...
def build_production_development_dependencies() -> DevelopmentDependencies:
    ...
    try:
        ...
        from .sec_gemma_lean_science_v314_preflight import (
            V38_PRIVATE_ROOT,
    ...
    def authenticate(root: Path, descendant: bool) -> Mapping[str, Any]:
        if not descendant:
            ...
            # and gated F314 authority.  A failed development gate must never
...
def status(repo_root: Path) -> dict[str, Any]:
    """Read only the safe v3.14 attempt summary; never open private payload bodies."""

    try:
        from .sec_gemma_lean_science_v314_store import audit_attempt
        from .sec_gemma_lean_science_v314_preflight import (
            load_execution_authority,
            load_publication_recovery_authority,
        )
...
def run_development(
    ...
    *,
    ...
    descendant = (
        ...
        or (
            root / Path(contract.PAUSE_ARTIFACT_PATH + ".v314-pending")
        ).exists()
        or (root / Path(contract.RESULT_ARTIFACT_PATH)).exists()
        or (
            root / Path(contract.RESULT_ARTIFACT_PATH + ".v314-pending")
        ).exists()
    ...
    v314_private = str((root / Path(contract.PRIVATE_NAMESPACE)).resolve(strict=False))
    forbidden_tokens: tuple[bytes | str, ...] = (
        contact,
        str(root),
        str(root).replace("\\", "/"),
    ...
    try:
        ...
        if invocation_parent["invocation_parent_kind"] == "continuation":
            # C314 and the fresh permission are authority for the durable
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="AAPL SEC/Gemma v3.14 science runner")
    parser.add_argument(
        "command",
