"""Strict one-shot HTTPS transport for lean SEC evidence v3.5.
...
TRANSPORT_RECEIPT_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-evidence-v3-5-transport-receipt-v1"
)
...
_WORKER_MODULE: Final[str] = "agent_benchmark.sec_gemma_lean_v35_transport"
...
source = root / "agent_benchmark" / "sec_gemma_lean_v35_transport.py"
...
if (
    ...
    or source.name != "sec_gemma_lean_v35_transport.py"
    or source.suffix != ".py"
    or executable.resolve(strict=True) != executable
    or module_name != "agent_benchmark.sec_gemma_lean_v35_transport"
...
class SecGemmaLeanV35TransportError(RuntimeError):
...
class _PrivateSecContact:
    """In-memory contact closure plus the exact v3.5 privacy needles."""
    ...
    def __init__(self, contact: str) -> None:
        if type(contact) is not str or not 6 <= len(contact) <= 512:
            raise SecGemmaLeanV35TransportError("transport_error")
        try:
            utf8 = contact.encode("utf-8", errors="strict")
        except UnicodeEncodeError:
            raise SecGemmaLeanV35TransportError("transport_error") from None
...
class ResponseHead:
    ...
    def values(self, name: bytes | str) -> tuple[bytes, ...]:
        ...
        if type(target) is not bytes:
            raise SecGemmaLeanV35TransportError("http_framing_rejected")
...
class _DeadlineSocketReader:
    ...
    def read_some(self, maximum_bytes: int) -> bytes:
        ...
        if remaining <= 0.0:
            raise SecGemmaLeanV35TransportError("hard_timeout")
        try:
            self._connection.settimeout(remaining)
            value = self._connection.recv(maximum_bytes)
        except (TimeoutError, socket.timeout):
            raise SecGemmaLeanV35TransportError("hard_timeout") from None
...
def _sha256_file(path: Path) -> tuple[int, str]:
    try:
        ...
        raise SecGemmaLeanV35TransportError("tls_configuration_rejected") from None
...
def execution_identity() -> dict[str, Any]:
    ...
    try:
        ...
        raise SecGemmaLeanV35TransportError("tls_configuration_rejected") from None
    if source.name != "sec_gemma_lean_v35_transport.py" or source.suffix != ".py":
        raise SecGemmaLeanV35TransportError("tls_configuration_rejected")
...
def derive_sec_role(url: str) -> SecRoleBinding:
    ...
    if type(url) is not str or not url or len(url) > 4096 or "\x00" in url:
        raise SecGemmaLeanV35TransportError("url_mismatch")
...
def canonical_sec_url(url: str) -> str:
    """Accept only one exact frozen v3.5 role URL, with no normalization."""
...
def response_body_framing(head: ResponseHead) -> BodyFraming:
    ...
    if len(content_encodings) > 1:
        raise SecGemmaLeanV35TransportError("content_encoding_rejected")
    ...
    if len(content_lengths) > 1:
        raise SecGemmaLeanV35TransportError("content_length_rejected")
    if len(transfer_encodings) > 1:
        raise SecGemmaLeanV35TransportError("transfer_encoding_rejected")
...
def _read_line(
    ...
    *,
    ...
    while len(value) <= maximum_bytes:
        ...
        if not byte:
            raise SecGemmaLeanV35TransportError(eof_code)
        if len(byte) != 1:
            raise SecGemmaLeanV35TransportError("http_framing_rejected")
        value.extend(byte)
        if value.endswith(b"\r\n"):
...
def _read_head(reader: _ReadSome) -> bytes:
    ...
    while len(value) < MAX_RESPONSE_HEAD_BYTES:
        byte = reader.read_some(1)
        if not byte:
            raise SecGemmaLeanV35TransportError("premature_eof")
        if len(byte) != 1:
            raise SecGemmaLeanV35TransportError("http_framing_rejected")
        value.extend(byte)
        if value.endswith(b"\r\n\r\n"):
...
def _expect_connection_eof(reader: _ReadSome, observed: int) -> None:
    ...
    if extra:
        raise SecGemmaLeanV35TransportError(
...
def _read_content_length_body(
    ...
    *,
    ...
    if declared > body_limit:
        raise SecGemmaLeanV35TransportError("body_limit_exceeded")
...
def _read_chunked_body(reader: _ReadSome, *, body_limit: int) -> bytes:
    ...
    while True:
        ...
        if size == 0:
            ...
            if trailer_or_end:
                raise SecGemmaLeanV35TransportError("trailers_rejected")
            _expect_connection_eof(reader, len(body))
            return bytes(body)
        remaining_with_sentinel = body_limit + 1 - len(body)
...
def _parse_from_reader(
    ...
    *,
    ...
    if contact is not None and contact.contains_echo(raw_head):
        raise SecGemmaLeanV35TransportError("privacy_echo")
    head = parse_http_response_head(raw_head)
    if head.status_code != 200:
        raise SecGemmaLeanV35TransportError("http_status_rejected")
    ...
    *,
    ...
    try:
        ...
        if wrapped.context.check_hostname is not True:
            ...
            raise SecGemmaLeanV35TransportError("tls_configuration_rejected")
        return wrapped
    except SecGemmaLeanV35TransportError:
        raise
...
def _safe_output_path(value: str) -> Path:
    if type(value) is not str or not value:
        raise SecGemmaLeanV35TransportError("blob_persistence_failed")
    path = Path(value)
    if (
    ...
    try:
        ...
        raise SecGemmaLeanV35TransportError("blob_persistence_failed") from None
...
def _worker_main() -> int:
    ...
    try:
        ...
        return 0
    except SecGemmaLeanV35TransportError as error:
