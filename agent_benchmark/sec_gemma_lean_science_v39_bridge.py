"""Read-only v3.8-to-v3.9 SEC science bridge.
...
BRIDGE_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-9-streaming-bridge-v1"
)
...
def _contract_module() -> Any:
    # The contract is a separate allowlisted v3.9 file and may be written by a
...
def _hash_path(
    ...
    if not stable or counted != before.st_size:
...
def _authenticate_public_source(repo_root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    contract = _contract_module()
    try:
        if _git(repo_root, "rev-parse", f"{contract.BASE_COMMIT}^{{tree}}") != contract.BASE_TREE:
            raise BridgeViolation("repository_authority_invalid")
        checks = (
        ...
        for path, expected in checks:
            if _git_blob(repo_root, contract.BASE_COMMIT, path) != expected:
                raise BridgeViolation("repository_authority_invalid")
        literal_checks = (
