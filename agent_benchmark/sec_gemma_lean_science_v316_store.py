"""One-shot private attempt store for the v3.16 development experiment.
...
LOCK_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-16-attempt-lock-v1"
)
RESPONSE_PAYLOAD_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-16-response-payload-v1"
)
CHECKPOINT_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-16-checkpoint-v1"
)
RESPONSE_CHECKPOINT_STATE_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-16-checkpoint-state-v1"
)
PAUSE_CHECKPOINT_STATE_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-16-pause-checkpoint-state-v2"
)
...
TERMINAL_CANDIDATE_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-16-terminal-evidence-candidate-v1"
)
PAUSE_CANDIDATE_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-16-pause-publication-candidate-v1"
)
RECOVERY_INTENT_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-16-recovery-intent-v1"
)
RECOVERY_TERMINAL_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-16-recovery-terminal-v1"
)
# Kept as a compatibility export for callers that previously named the private
# terminal envelope.  V3.16 stores a terminal *candidate*, not V3.10 evidence.
...
class V316StoreError(RuntimeError):
...
class V316StoreConflict(V316StoreError):
    """The attempt is already locked or an immutable destination exists."""


class V316StorePoisoned(V316StoreError):
    """The persisted attempt cannot be accepted or resumed."""


def _translate_journal_error(exc: V316JournalError) -> V316StorePoisoned:
    return V316StorePoisoned(exc.code)


def _validate_authority(authority: Mapping[str, Any]) -> dict[str, Any]:
    if not isinstance(authority, Mapping):
        raise V316StoreError("authority_invalid")
    ...
    if not (
        ...
        raise V316StoreError("attempt_identity_invalid")
    try:
    ...
    except (V316JournalError, json.JSONDecodeError):
        raise V316StoreError("authority_invalid") from None
...
def _mkdir_new(path: Path) -> None:
    ...
    except V316JournalError as exc:
        raise _translate_journal_error(exc) from None
    except OSError:
        raise V316StorePoisoned("attempt_directory_creation_failed") from None
...
def _write_exclusive(path: Path, encoded: bytes, *, code: str) -> None:
    ...
    try:
        ...
        raise V316StorePoisoned(exc.code) from None
    except OSError:
        raise V316StorePoisoned(code) from None
    finally:
...
def _entry_exists(path: Path) -> bool:
    try:
        ...
        raise V316StorePoisoned("attempt_root_invalid") from None
    return True
...
def _publish_attempt_root_noreplace(source: Path, destination: Path) -> None:
    ...
    if (
        ...
        raise V316StorePoisoned("attempt_initialization_failed")
    ...
    if _entry_exists(destination):
        raise V316StoreConflict("attempt_already_exists")
    ...
    except V316StoreError:
        raise
    except (OSError, V316JournalError):
        if _entry_exists(destination):
            raise V316StoreConflict("attempt_already_exists") from None
        raise V316StorePoisoned("attempt_initialization_failed") from None
    finally:
...
def _initialize_attempt_root(
    ...
    *,
    ...
    try:
        ...
        staging = Path(
            tempfile.mkdtemp(
                prefix=".v316-attempt-initialization-",
        ...
        if len(machine.events) != 1 or machine.events[0].event_type != "attempt_intent":
            raise V316StorePoisoned("attempt_initialization_invalid")
        ...
        fsync_directory(staging_parent)
    except (V316StoreError, V316JournalError):
        raise
    ...
    def acquire(self) -> None:
        with _ACTIVE_LOCKS_GUARD:
            if self.path in _ACTIVE_LOCKS:
                raise V316StoreConflict("attempt_locked")
        ...
        try:
            ...
            if (
                ...
                raise V316StorePoisoned("attempt_lock_identity_changed")
            handle.seek(0)
            if handle.read() != self.expected_bytes:
                raise V316StorePoisoned("attempt_lock_invalid")
            self._handle = handle
            ...
            if isinstance(exc, V316StoreError):
                raise
            if isinstance(exc, V316JournalError):
                raise _translate_journal_error(exc) from None
            raise V316StoreConflict("attempt_locked") from None
...
class TerminalEvidenceReceipt:
    """Replay-audited terminal event plus its immutable V3.16 candidate."""
...
def _payload_self_hash(payload: Mapping[str, Any], field: str) -> str:
    if type(payload) is not dict or field not in payload:
        raise V316StorePoisoned("self_hash_invalid")
...
def _decode_runtime_binding_receipt(
    ...
    if not _exact_keys(payload, set(_RUNTIME_RECEIPT_KEYS)):
        raise V316StorePoisoned("runtime_binding_receipt_invalid")
    if (
    ...
    except V316StorePoisoned:
        raise V316StorePoisoned("runtime_binding_receipt_invalid") from None
...
def _derive_response_checkpoint_state(
    ...
    if response is None:
        raise V316StorePoisoned("checkpoint_subject_invalid")
    intent = machine.intents.get(
        response.payload["request_intent_event_sha256"]
    )
    if intent is None:
...
def _derive_pause_checkpoint_state(
    ...
    if (
        ...
        raise V316StorePoisoned("pause_checkpoint_state_invalid")
...
def _validate_request_transition(
    ...
    *,
    ...
    if machine.terminal_event is not None:
        raise V316StorePoisoned("event_after_terminal")
    if machine.active_intent is not None or machine.pending_subject_event_sha256:
        raise V316StorePoisoned("request_boundary_incomplete")
    if machine.pause_committed and machine.continuation_event is None:
        raise V316StorePoisoned("request_while_paused")
    if intent.ordinal != len(machine.intents) + 1:
        raise V316StorePoisoned("request_order_invalid")
    if intent.request_id in machine.intent_ids:
        raise V316StorePoisoned("duplicate_request")
    ...
    if intent.effect_kind == "yahoo":
        if (
            ...
            raise V316StorePoisoned("yahoo_order_invalid")
        return
    ...
    if phase == "pre_probe_start":
        ...
        if (
            ...
            raise V316StorePoisoned("model_segment_start_invalid")
        return
    if intent.segment_id != machine.open_segment_id:
        raise V316StorePoisoned("model_segment_mismatch")
    if phase == "pre_probe":
        if machine.segment_phase != "pre" or machine.segment_pre_count != 1:
            raise V316StorePoisoned("pre_probe_order_invalid")
        return
    if phase == "post_probe":
        if machine.segment_phase != "generation" or machine.segment_post_count:
            raise V316StorePoisoned("post_probe_order_invalid")
        ...
        if count != expected:
            raise V316StorePoisoned("post_probe_generation_count_invalid")
        return
    if phase == "post_probe_close":
        if machine.segment_phase != "post" or machine.segment_post_count != 1:
            raise V316StorePoisoned("post_probe_order_invalid")
        return
    raise V316StorePoisoned("model_phase_invalid")
...
def _validate_private_open_transition(machine: _Machine, event_type: str) -> None:
    if not _private_values_ready(machine):
        raise V316StorePoisoned("private_values_open_order_invalid")
...
def _apply_response(machine: _Machine, event: JournalEvent) -> None:
    ...
    if not _exact_keys(
        ...
        raise V316StorePoisoned("response_event_invalid")
    ...
    if active.effect_kind == "yahoo" and not (
        ...
        raise V316StorePoisoned("yahoo_response_bytes_invalid")
    if active.effect_kind == "identity" and not (
        1 <= payload["body_bytes"] <= MODEL_RESPONSE_CAP_BYTES
    ):
        raise V316StorePoisoned("runtime_probe_response_bytes_invalid")
    if active.effect_kind == "gemma" and not (
        1 <= payload["body_bytes"] <= MODEL_RESPONSE_CAP_BYTES
    ):
        raise V316StorePoisoned("model_response_bytes_invalid")
    ...
    if active.effect_kind == "gemma":
        ...
        raise V316StorePoisoned("response_duration_invalid")
    ...
    if active.effect_kind == "yahoo":
        ...
        if machine.yahoo_body_bytes > YAHOO_BATCH_BODY_CAP_BYTES:
            raise V316StorePoisoned("yahoo_batch_body_cap_exceeded")
...
def _replay_state(
    ...
    if not replay.events:
        raise V316StorePoisoned("attempt_intent_missing")
    for index, event in enumerate(replay.events):
        ...
        if event.event_type not in _EVENT_TYPES:
            raise V316StorePoisoned("foreign_event")
        payload = event.payload
        if (
            machine.runtime_binding_failed is not None
            and event.event_type not in {"candidate_abandoned", "attempt_terminal"}
        ):
            raise V316StorePoisoned("event_after_runtime_binding_failure")
        if event.event_type == "attempt_intent":
            if (
                ...
                raise V316StorePoisoned("attempt_intent_invalid")
            continue
        if index == 0:
            raise V316StorePoisoned("attempt_intent_missing")
        ...
        if event.event_type == "runtime_binding_failed":
            if not _exact_keys(
                ...
                raise V316StorePoisoned("runtime_binding_failure_invalid")
            expected_previous = (
                machine.runtime_receipts[-1].runtime_binding_receipt_sha256
                if machine.runtime_receipts
                else None
        ...
        if event.event_type == "candidate_abandoned":
            if not _exact_keys(
                ...
                raise V316StorePoisoned("candidate_abandonment_invalid")
            if (
        ...
        if event.event_type == "request_intent":
            if not _exact_keys(
                ...
                raise V316StorePoisoned("request_intent_invalid")
            intent = RequestIntent(
                event.event_sha256,
                payload["request_id"],
                payload["effect_kind"],
        ...
        if event.event_type == "response_committed":
            if machine.pending_subject_event_sha256 is not None:
                raise V316StorePoisoned("response_order_invalid")
        ...
        if event.event_type == "checkpoint_committed":
            if not _exact_keys(
                ...
                raise V316StorePoisoned("checkpoint_event_invalid")
            if (
        ...
        if event.event_type == "paused_for_justification":
            if not _exact_keys(
                ...
                raise V316StorePoisoned("pause_event_invalid")
            expected_durations = (
        ...
        if event.event_type == "continuation_authorized":
            if not _exact_keys(
                ...
                raise V316StorePoisoned("continuation_event_invalid")
        ...
        if event.event_type in {
            ...
            if not _exact_keys(payload, set()):
                raise V316StorePoisoned("private_values_open_event_invalid")
        ...
        if event.event_type == "attempt_terminal":
            if not _exact_keys(
                ...
                raise V316StorePoisoned("terminal_event_invalid")
            if (
            ...
            if payload["status"] == "completed":
                ...
                if (
                    ...
                    raise V316StorePoisoned("completed_effect_budget_invalid")
...
def _decode_canonical_file(path: Path, *, code: str) -> tuple[dict[str, Any], bytes]:
    try:
        ...
        raise
    except (V316JournalError, OSError, UnicodeDecodeError, json.JSONDecodeError):
        raise V316StorePoisoned(code) from None
...
def _load_response(
    ...
    if stored is None:
        raise V316StorePoisoned("response_payload_missing")
    ...
    if (
        ...
        raise V316StorePoisoned("response_payload_invalid")
    try:
        body = base64.b64decode(value["body_base64"], validate=True)
    except (ValueError, TypeError):
        raise V316StorePoisoned("response_payload_invalid") from None
...
def _audit_contents(
    ...
    if set(payloads) != referenced_payloads:
        raise V316StorePoisoned("orphan_response_payload")
    ...
    for subject, checkpoint_event in machine.checkpoints.items():
        ...
        if stored is None:
            raise V316StorePoisoned("checkpoint_missing")
        value, encoded = stored
        if (
            ...
            raise V316StorePoisoned("checkpoint_invalid")
    ...
    if candidates:
        ...
        if machine.pending_subject_event_sha256 is None or len(candidates) != 1:
            raise V316StorePoisoned("orphan_checkpoint")
...
def _audit_terminal_candidate(
    ...
    try:
        ...
        raise V316StorePoisoned("terminal_candidate_invalid") from None
    if (
        set(value) != keys
        ...
        raise V316StorePoisoned("terminal_candidate_invalid")
    ...
    if (
        ...
        raise V316StorePoisoned("terminal_candidate_binding_invalid")
...
def _audit_pause_candidate(
    ...
    if set(value) != {
        ...
        raise V316StorePoisoned("pause_candidate_invalid")
    try:
        digest = _payload_self_hash(value, "pause_candidate_sha256")
    except V316StorePoisoned:
        raise V316StorePoisoned("pause_candidate_invalid") from None
    if (
    ...
    if machine.pause_event is not None:
        ...
        if (
            ...
            raise V316StorePoisoned("pause_candidate_binding_invalid")
    return receipt
...
def _audit_publication_candidates(
    ...
    except (V316JournalError, OSError):
        raise V316StorePoisoned("publication_candidate_directory_poisoned") from None
    if not names.issubset({TERMINAL_BUNDLE_DIRECTORY, PAUSE_BUNDLE_DIRECTORY}):
        raise V316StorePoisoned("publication_candidate_directory_poisoned")
    ...
    if TERMINAL_BUNDLE_DIRECTORY in names:
        ...
        raise V316StorePoisoned("terminal_candidate_missing")
    if PAUSE_BUNDLE_DIRECTORY in names:
        ...
        raise V316StorePoisoned("pause_candidate_missing")
...
def _replay_recovery_state(
    ...
    for event in replay.events:
        ...
        if event.event_type not in _RECOVERY_EVENT_TYPES:
            raise V316StorePoisoned("recovery_foreign_event")
        if event.event_type == "recovery_intent":
            if (
                ...
                raise V316StorePoisoned("recovery_after_publication")
            if not _exact_keys(
                payload,
                {
                    "ordinal",
                ...
                raise V316StorePoisoned("recovery_intent_invalid")
            ...
            try:
                ...
                raise V316StorePoisoned("recovery_intent_invalid") from None
        ...
        if event.event_type == "runtime_binding_receipt":
            if current_intent is None or machine.phase != "intent":
                raise V316StorePoisoned("recovery_receipt_order_invalid")
            receipt = _decode_runtime_binding_receipt(event)
            if machine.runtime_receipts:
                predecessor_valid = (
            ...
            if (
                ...
                raise V316StorePoisoned("recovery_receipt_invalid")
            machine.runtime_receipts.append(receipt)
            current_receipt = receipt
            machine.phase = "receipt"
            continue
        if current_intent is None or current_receipt is None or machine.phase != "receipt":
            raise V316StorePoisoned("recovery_terminal_order_invalid")
        if not _exact_keys(
            payload,
            {
                "ordinal",
            ...
            raise V316StorePoisoned("recovery_terminal_invalid")
        if (
        ...
        try:
            ...
            raise V316StorePoisoned("recovery_terminal_invalid") from None
...
def _audit_publication_links(
    ...
    if terminal_candidate is not None:
        ...
        if (
            ...
            raise V316StorePoisoned("terminal_candidate_predecessor_invalid")
    if pause_candidate is not None:
        ...
        if (
            ...
            raise V316StorePoisoned("pause_candidate_predecessor_invalid")
    ...
    if attempt.pause_event is not None:
        ...
        if normal is not None:
            if (
                ...
                raise V316StorePoisoned("pause_publication_binding_invalid")
        elif (
    ...
    if attempt.candidate_abandoned is not None:
        ...
        if (
            ...
            raise V316StorePoisoned("candidate_abandonment_binding_invalid")
    ...
    if attempt.terminal_event is not None:
        ...
        if normal is not None:
            if (
                ...
                raise V316StorePoisoned("terminal_publication_binding_invalid")
    ...
    for intent in recovery.intents:
        if intent.target_kind == "pause":
            if (
                ...
                raise V316StorePoisoned("recovery_candidate_binding_invalid")
    ...
    for terminal in recovery.terminals:
        ...
        if not complete:
            raise V316StorePoisoned("recovery_publication_incomplete")
...
def _root_layout(root: Path) -> None:
    ...
    if names != _ROOT_ENTRIES:
        raise V316StorePoisoned("attempt_root_poisoned")


def _content_bytes(subject_event_sha256: str, state: Mapping[str, Any]) -> bytes:
    if not isinstance(state, Mapping):
        raise V316StoreError("checkpoint_state_invalid")
    value = {
...
class AttemptStore:
    """Exclusively own and advance one immutable v3.16 attempt namespace."""
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(root, Path) or not root.is_absolute():
            raise V316StoreError("attempt_root_invalid")
    ...
    def _require_open(self) -> None:
        if self._closed:
            raise V316StoreError("store_closed")
        if self._poisoned:
            raise V316StorePoisoned("store_poisoned")
    ...
    def privacy_scan_locked_payloads(self) -> Mapping[str, bytes]:
        ...
        if self._lock is None:
            raise V316StoreError("attempt_lock_invalid")
    ...
    def _append(self, event_type: str, payload: Mapping[str, Any]) -> JournalEvent:
        ...
        if self._journal is None:
            raise V316StoreError("store_not_initialized")
        try:
    ...
    def _runtime_binding_payload(
        ...
        *,
        ...
        if (
            ...
            raise V316StoreError("runtime_binding_receipt_invalid")
        return {
            **body,
    ...
    def append_runtime_binding_receipt(
        ...
        *,
        ...
        try:
            ...
            raise V316StoreError("runtime_binding_receipt_invalid") from None
    ...
    def record_runtime_binding_failure(
        ...
        if (
            ...
            raise V316StoreError("runtime_binding_failure_invalid")
        previous = (
            machine.runtime_receipts[-1].runtime_binding_receipt_sha256
            if machine.runtime_receipts
            else None
    ...
    def build_terminal_candidate(
        ...
        *,
        ...
        if (
            ...
            raise V316StoreError("terminal_candidate_invalid")
        try:
            normalized_evidence = json.loads(
                canonical_json_bytes(_thaw_json(evidence))
            )
        except (V316JournalError, json.JSONDecodeError, TypeError, ValueError):
            raise V316StoreError("terminal_candidate_invalid") from None
    ...
    def write_pause_candidate(
        ...
        if (
            ...
            raise V316StoreError("pause_candidate_invalid")
        body = {
    ...
    def begin_recovery(
        ...
        *,
        ...
        if (
            ...
            raise V316StoreError("recovery_already_published")
        ...
        if (
            ...
            raise V316StoreError("recovery_intent_invalid")
        values = (
            public_artifact_sha256,
            public_artifact_bytes,
            comparison_before_sha256,
        ...
        if refreshed.event_sha256 != event.event_sha256:
            raise V316StorePoisoned("recovery_intent_missing")
        return refreshed
    ...
    def append_recovery_runtime_binding_receipt(
        ...
        *,
        ...
        if not recovery.intents or recovery.phase != "intent":
            raise V316StoreError("recovery_receipt_order_invalid")
        previous = (
    ...
    def finish_recovery(
        ...
        if (
            ...
            raise V316StoreError("recovery_terminal_invalid")
        intent = recovery.intents[-1]
        ...
        if terminal.event_sha256 != event.event_sha256:
            raise V316StorePoisoned("recovery_terminal_missing")
        return terminal
    ...
    def abandon_terminal_candidate(
        ...
        if (
            ...
            raise V316StoreError("candidate_abandonment_invalid")
        return self._append(
            "candidate_abandoned",
            {
    ...
    def append_attempt_terminal(
        ...
        *,
        ...
        if (
            ...
            raise V316StoreError("terminal_invalid")
        if machine.candidate_abandoned is not None:
        ...
        if self._terminal_evidence is None:
            raise V316StorePoisoned("terminal_evidence_missing")
    ...
    def begin_request(
        ...
        *,
        ...
        if self.snapshot.status in {
            ...
            raise V316StoreError("request_not_authorized")
        payload = {
        ...
        if (
            ...
            raise V316StoreError("request_intent_invalid")
        _validate_request_transition(
            machine,
            candidate,
    ...
    def commit_response(
        ...
        *,
        ...
        if (
            ...
            raise V316StoreError("response_commit_invalid")
        if active.effect_kind == "yahoo" and not (
            1 <= len(body) <= YAHOO_RESPONSE_CAP_BYTES
        ):
            raise V316StoreError("yahoo_response_bytes_invalid")
        if active.effect_kind == "identity" and not (
            1 <= len(body) <= MODEL_RESPONSE_CAP_BYTES
        ):
            raise V316StoreError("runtime_probe_response_bytes_invalid")
        if active.effect_kind == "gemma" and not (
            1 <= len(body) <= MODEL_RESPONSE_CAP_BYTES
        ):
            raise V316StoreError("model_response_bytes_invalid")
        payload_value = {
    ...
    def _write_content(
        ...
        *,
        ...
        if type(encoded) is not bytes or not 1 <= len(encoded) <= _MAX_CONTENT_BYTES:
            raise V316StoreError("content_size_invalid")
        if sha256_bytes(encoded) != digest:
            raise V316StoreError("content_hash_invalid")
        ...
        if final_exists:
            ...
            if allow_existing_exact:
                try:
                    ...
                    self._poisoned = True
                    raise V316StorePoisoned(exc.code) from None
            raise V316StoreConflict(conflict_code)
        if pending_exists:
            raise V316StorePoisoned("content_write_incomplete")
        try:
            ...
            try:
                ...
                except V316JournalError as presence_exc:
                    raise V316StorePoisoned(presence_exc.code) from None
                if destination_exists:
                    raise V316StoreConflict(conflict_code) from None
                raise V316StorePoisoned(exc.code) from None
            fsync_directory(directory)
            ...
            if reread != encoded:
                raise V316StorePoisoned("content_write_failed")
            return final
        except Exception:
            self._poisoned = True
            raise
    ...
    def commit_checkpoint(
        ...
        *,
        ...
        if (
            ...
            raise V316StoreError("checkpoint_subject_invalid")
    ...
    def derive_pending_checkpoint_state(self) -> Mapping[str, Any]:
        ...
        if subject is None or subject_type not in {"response", "pause"}:
            raise V316StoreError("checkpoint_not_pending")
    ...
    def committed_responses(
        ...
        *,
        ...
        if effect_kind is not None and effect_kind not in _EFFECT_KINDS:
            raise V316StoreError("response_filter_invalid")
        ...
        if any(
            ...
            raise V316StoreError("response_batch_unsealed")
        ...
        if effect_kind is None:
            ...
            if not (
                ...
                raise V316StoreError("response_values_not_opened")
            return selected
        ...
        for response in identities:
            ...
            if identity_segment is None:
                raise V316StorePoisoned("identity_segment_missing")
    ...
    def recover_pending_checkpoint(
        ...
        if not callable(builder):
            raise V316StoreError("checkpoint_builder_invalid")
        snapshot = self.snapshot
        if snapshot.status != "checkpoint_recovery":
            raise V316StoreError("checkpoint_recovery_forbidden")
        ...
        try:
            ...
            raise V316StoreError("checkpoint_rebuild_failed") from None
        if not isinstance(rebuilt, Mapping):
            raise V316StoreError("checkpoint_rebuild_failed")
        ...
        if self._checkpoint_candidates:
            if self._checkpoint_candidates != (digest,):
                self._poisoned = True
                raise V316StorePoisoned("checkpoint_mismatch")
    ...
    def commit_planned_pause(
        ...
        *,
        ...
        if (
            ...
            raise V316StoreError("pause_not_authorized")
        durations = machine.segments[0].generation_durations_ns
        projected = sum(durations) + REMAINING_COUNT * max(durations)
        if projected <= PAUSE_THRESHOLD_NS:
            raise V316StoreError("pause_threshold_not_crossed")
    ...
    def authorize_continuation(
        ...
        *,
        ...
        if (
            ...
            raise V316StoreError("continuation_not_authorized")
        return self._append(
            "continuation_authorized",
            {
    ...
    def mark_market_values_opened(self) -> JournalEvent:
        ...
        try:
            ...
            raise V316StoreError("private_values_open_not_authorized") from None
    ...
    def committed_terminal_evidence(self) -> TerminalEvidenceReceipt:
        ...
        self._require_open()
        if self._terminal_evidence is None:
            raise V316StoreError("terminal_evidence_not_committed")
        return self._terminal_evidence
