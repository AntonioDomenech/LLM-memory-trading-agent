"""Disk-backed zero-retry SEC acquisition for lean Gemma evidence v3.7.
...
import agent_benchmark.sec_gemma_lean_v37_journal as journal_module
from agent_benchmark.sec_gemma_lean_v37_journal import (
    AcquisitionJournal,
...
ACQUISITION_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-evidence-v3-7-acquisition-v1"
)
PARSE_RECEIPT_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-evidence-v3-7-parse-receipt-v1"
)
PHASE_RECEIPT_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-evidence-v3-7-phase-receipt-v1"
)
...
def _repository_root() -> Path:
    try:
        ...
        raise SecGemmaLeanV37AcquisitionError("root_invalid") from None


class SecGemmaLeanV37AcquisitionError(RuntimeError):
...
def _source_diagnostic_code(error: BaseException) -> str:
    ...
    if type(error) is SecGemmaLeanV37SourceError:
        message = str(error)
...
def _bare_manifest_body_sha256(
    ...
    *,
    ...
    if match is None:
        raise SecGemmaLeanV37AcquisitionError(error_code)
...
class SharedSecDispatchLedger:
    ...
    def __init__(
        ...
        *,
        ...
        if not callable(monotonic) or not callable(sleep):
            raise SecGemmaLeanV37AcquisitionError("dispatch_clock_invalid")
        if (
        ...
        if (
            ...
            raise SecGemmaLeanV37AcquisitionError("dispatch_ledger_invalid")
    ...
    def dispatch(
        ...
        *,
        ...
        if (
            ...
            raise SecGemmaLeanV37AcquisitionError("dispatch_request_invalid")
        entered = self._read_clock()
        if not _PROCESS_DISPATCH_LOCK.acquire(
            timeout=self._lock_timeout_seconds
        ):
            raise SecGemmaLeanV37AcquisitionError("dispatch_lock_conflict")
        try:
            with _locked_file(
            ...
            try:
                ...
                raise SecGemmaLeanV37AcquisitionError(
    ...
    def _dispatch_locked(
        ...
        *,
        ...
        if in_flight:
            raise SecGemmaLeanV37AcquisitionError("dispatch_indeterminate")
        ...
        if ready + 1e-9 < target:
            raise SecGemmaLeanV37AcquisitionError("dispatch_gap_too_short")
        ...
        if type(result) is not str or _SHA256_RE.fullmatch(result) is None:
            raise SecGemmaLeanV37AcquisitionError("journal_binding_invalid")
    ...
    def _read_clock(self) -> float:
        try:
            ...
            raise SecGemmaLeanV37AcquisitionError("dispatch_clock_invalid") from None
    ...
    def _read_persisted_dispatch(self) -> tuple[float | None, bool]:
        try:
        ...
        except SecGemmaLeanV37AcquisitionError as error:
            ...
            raise SecGemmaLeanV37AcquisitionError("dispatch_ledger_invalid") from None
        if (
    ...
    try:
        ...
        if handle is not None:
            try:
                ...
                pass
        raise SecGemmaLeanV37AcquisitionError("run_lock_invalid") from None
    except (SecGemmaLeanV37AcquisitionError, SecGemmaLeanV37JournalError):
        if handle is not None:
            try:
                handle.close()
            except OSError:
    ...
    while not acquired:
        try:
            ...
            if time.monotonic() >= deadline:
                handle.close()
                raise SecGemmaLeanV37AcquisitionError(busy_code) from None
            time.sleep(0.01)
    try:
        yield
    finally:
...
def _secure_directory(path: Path) -> Path:
    if not isinstance(path, Path) or not path.is_absolute():
        raise SecGemmaLeanV37AcquisitionError("root_invalid")
    try:
...
def _secure_regular_file(path: Path) -> os.stat_result:
    try:
        ...
        raise SecGemmaLeanV37AcquisitionError("artifact_invalid") from None
    if (
        ...
        raise SecGemmaLeanV37AcquisitionError("artifact_invalid")
    return details
...
def _canonical_bytes(value: Any) -> bytes:
    ...
    while pending:
        ...
        if type(current) is dict:
            if any(type(key) is not str for key in current):
                raise SecGemmaLeanV37AcquisitionError("non_json_source_output")
    ...
    try:
        ...
        raise SecGemmaLeanV37AcquisitionError("non_json_source_output") from None
        ...
        with path.open("rb") as handle:
            while True:
                ...
                if maximum_bytes is not None and counted > maximum_bytes:
                    raise SecGemmaLeanV37AcquisitionError("artifact_invalid")
                digest.update(chunk)
    except SecGemmaLeanV37AcquisitionError:
        raise
    except OSError:
        raise SecGemmaLeanV37AcquisitionError("artifact_invalid") from None
...
def _write_immutable(path: Path, payload: bytes) -> None:
    if type(payload) is not bytes or not payload:
        raise SecGemmaLeanV37AcquisitionError("artifact_persistence_failed")
    try:
        ...
        raise
    except (OSError, SecGemmaLeanV37JournalError):
        raise SecGemmaLeanV37AcquisitionError(
...
def _prepare_layout(private_root: Path, public_root: Path, stage: str) -> _Layout:
    ...
    try:
        if private_root == public_root or private_root in public_root.parents or public_root in private_root.parents:
            raise SecGemmaLeanV37AcquisitionError("root_overlap")
        repo_root = _repository_root()
        private_relative = private_root.relative_to(repo_root)
        public_relative = public_root.relative_to(repo_root)
    except ValueError:
        raise SecGemmaLeanV37AcquisitionError("root_namespace_invalid") from None
    if private_relative.parts != ("data", ROOT_NAMESPACE) or public_relative.parts != (
        "e",
        ROOT_NAMESPACE,
    ):
        raise SecGemmaLeanV37AcquisitionError("root_namespace_invalid")
    try:
        ignore_lines = {
            line.strip()
        ...
        raise SecGemmaLeanV37AcquisitionError("private_root_not_ignored") from None
    if "data/" not in ignore_lines and "/data/" not in ignore_lines:
        raise SecGemmaLeanV37AcquisitionError("private_root_not_ignored")
...
def strict_transport_factory(private_contact: str) -> TransportFactory:
    ...
    if type(private_contact) is not str:
        raise SecGemmaLeanV37AcquisitionError("private_contact_invalid")
...
class DiskBackedSecAcquisition:
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            raise SecGemmaLeanV37AcquisitionError("configuration_invalid")
        if (
        ...
        try:
            ...
            raise SecGemmaLeanV37AcquisitionError("private_contact_invalid") from None
        self._stage = stage
        ...
        try:
            ...
            raise SecGemmaLeanV37AcquisitionError("transport_invalid") from None
        if (
            ...
            raise SecGemmaLeanV37AcquisitionError("transport_invalid")
        ...
        except SecGemmaLeanV37JournalError as error:
            raise SecGemmaLeanV37AcquisitionError(error.code) from None
    ...
    def _validate_prior(
        ...
        if index == 0:
            if prior is not None:
                raise SecGemmaLeanV37AcquisitionError("prior_stage_invalid")
            return None
        if (
    ...
    def _validate_source_surface(self) -> None:
        ...
        if self._source is None or any(
            ...
            raise SecGemmaLeanV37AcquisitionError("source_adapter_invalid")

    def _now(self) -> float:
        try:
            value = float(self._monotonic())
        except Exception:
            raise SecGemmaLeanV37AcquisitionError("clock_invalid") from None
        if not math.isfinite(value) or value < 0.0:
            raise SecGemmaLeanV37AcquisitionError("clock_invalid")
        return value
    ...
    def _audit_layout_names(self) -> None:
        ...
        try:
            ...
            raise SecGemmaLeanV37AcquisitionError("artifact_inventory_invalid") from None
        private_directories = {
            item.name for item in private_entries if item.name in private_expected
        }
        if private_directories != private_expected:
            raise SecGemmaLeanV37AcquisitionError("artifact_inventory_invalid")
    ...
    def _validate_source_evidence(self, raw: Any) -> dict[str, Any]:
        ...
        if (
            ...
            raise SecGemmaLeanV37AcquisitionError("source_evidence_invalid")
    ...
    def _parse_output_parts(
        ...
        try:
            ...
            raise SecGemmaLeanV37AcquisitionError("source_output_invalid") from None
        if (
            ...
            raise SecGemmaLeanV37AcquisitionError("source_output_invalid")
        try:
            visible = vars(output)
        except TypeError:
            raise SecGemmaLeanV37AcquisitionError("source_output_invalid") from None
    ...
    def _validate_transport_result(
        ...
        *,
        ...
        if type(result) is not TransportResult:
            raise SecGemmaLeanV37AcquisitionError("transport_result_invalid")
        try:
            ...
            raise SecGemmaLeanV37AcquisitionError("transport_result_invalid") from None
        if (
    ...
    def _persist_manifest(
        ...
        if (
            ...
            raise SecGemmaLeanV37AcquisitionError("source_manifest_invalid")
    ...
    def _parse_role(
        ...
        if role.phase == "historical_submissions":
            ...
            if not role.role_id.startswith(prefix):
                raise SecGemmaLeanV37AcquisitionError("role_identity_invalid")
        ...
        if role.phase == "complete_submission":
            ...
            if type(targets) is not tuple:
                raise SecGemmaLeanV37AcquisitionError("phase_state_invalid")
            target = next(
                (
                    item
                    for item in targets
    ...
    def _process_role(
        ...
        *,
        ...
        if isinstance(dispatch.callback_error, SecGemmaLeanV37TransportError):
    ...
    def _authenticate_prior(self) -> Any | None:
        ...
        try:
            ...
            raise SecGemmaLeanV37AcquisitionError("prior_stage_invalid") from None
        if (
    ...
    def _rehydrate_current(
        ...
        *,
        ...
        try:
            ...
            raise SecGemmaLeanV37AcquisitionError("source_replay_rejected") from None
        if (
            ...
            raise SecGemmaLeanV37AcquisitionError("source_replay_rejected")
        self._contact.assert_redacted(receipt)
    ...
    def _persist_checkpoint(
        ...
        if (
            ...
            raise SecGemmaLeanV37AcquisitionError("checkpoint_invalid")
        self._contact.assert_redacted(value)
    ...
    def _terminal_result(
        ...
        *,
        ...
        if not state.accounting_complete or not state.passed:
            raise SecGemmaLeanV37AcquisitionError("accounting_incomplete")
        events = self._journal_events()
            ...
            if state.next_role_id is not None:
                ...
                try:
                    ...
                    try:
                        ...
                        raise SecGemmaLeanV37AcquisitionError(error.code) from None
                    raise SecGemmaLeanV37AcquisitionError(failure.code) from None
                invocation_ms = max(
                    invocation_role_ms,
                    self._role_duration_ms(invocation_started),
                )
...
def _read_production_json(
    ...
    try:
        ...
        raise SecGemmaLeanV37AcquisitionError(code) from None
    if type(value) is not dict:
        raise SecGemmaLeanV37AcquisitionError(code)
    return value, raw
...
def _verify_production_development_gate(
    ...
    try:
        from agent_benchmark import sec_gemma_lean_v37_preflight as preflight
    ...
    if (
        ...
        != "aapl-sec-gemma-lean-evidence-v3-7-acquisition-gate-v1"
    ...
    except (OSError, SecGemmaLeanV37AcquisitionError):
        raise SecGemmaLeanV37AcquisitionError(
            "production_repository_invalid"
        ) from None
    if shown_root != root:
...
def run_production_acquisition(
    ...
    source_adapter = loaded["agent_benchmark.sec_gemma_lean_v37_source"]
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="AAPL SEC/Gemma lean v3.7 source acquisition"
    )
    parser.add_argument(
...
__all__ = [
    ...
    "SecGemmaLeanV37AcquisitionError",
