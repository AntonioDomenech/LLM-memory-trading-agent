"""Disk-backed zero-retry SEC acquisition for lean Gemma evidence v3.8.
...
from agent_benchmark.sec_gemma_lean_v38_journal import (
    AcquisitionJournal,
...
ACQUISITION_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-evidence-v3-8-acquisition-v1"
)
PARSE_RECEIPT_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-evidence-v3-8-parse-receipt-v1"
)
PHASE_RECEIPT_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-evidence-v3-8-phase-receipt-v1"
)
...
def _repository_root() -> Path:
    try:
        ...
        raise SecGemmaLeanV38AcquisitionError("root_invalid") from None


class SecGemmaLeanV38AcquisitionError(RuntimeError):
...
def _source_diagnostic_code(error: BaseException) -> str:
    ...
    if type(error) is SecGemmaLeanV38SourceError:
        message = str(error)
...
def _bare_manifest_body_sha256(
    ...
    *,
    ...
    if match is None:
        raise SecGemmaLeanV38AcquisitionError(error_code)
...
class SharedSecDispatchLedger:
    ...
    def __init__(
        ...
        *,
        ...
        if not callable(monotonic) or not callable(sleep):
            raise SecGemmaLeanV38AcquisitionError("dispatch_clock_invalid")
        if (
        ...
        if (
            ...
            raise SecGemmaLeanV38AcquisitionError("dispatch_ledger_invalid")
    ...
    def dispatch(
        ...
        *,
        ...
        if (
            ...
            raise SecGemmaLeanV38AcquisitionError("dispatch_request_invalid")
        entered = self._read_clock()
        if not _PROCESS_DISPATCH_LOCK.acquire(
            timeout=self._lock_timeout_seconds
        ):
            raise SecGemmaLeanV38AcquisitionError("dispatch_lock_conflict")
        try:
            with _locked_file(
            ...
            try:
                ...
                raise SecGemmaLeanV38AcquisitionError(
    ...
    def _dispatch_locked(
        ...
        *,
        ...
        if in_flight:
            raise SecGemmaLeanV38AcquisitionError("dispatch_indeterminate")
        ...
        if ready + 1e-9 < target:
            raise SecGemmaLeanV38AcquisitionError("dispatch_gap_too_short")
        ...
        if type(result) is not str or _SHA256_RE.fullmatch(result) is None:
            raise SecGemmaLeanV38AcquisitionError("journal_binding_invalid")
    ...
    def _read_clock(self) -> float:
        try:
            ...
            raise SecGemmaLeanV38AcquisitionError("dispatch_clock_invalid") from None
    ...
    def _read_persisted_dispatch(self) -> tuple[float | None, bool]:
        try:
        ...
        except SecGemmaLeanV38AcquisitionError as error:
            ...
            raise SecGemmaLeanV38AcquisitionError("dispatch_ledger_invalid") from None
        if (
    ...
    try:
        ...
        if handle is not None:
            try:
                ...
                pass
        raise SecGemmaLeanV38AcquisitionError("run_lock_invalid") from None
    except (SecGemmaLeanV38AcquisitionError, SecGemmaLeanV38JournalError):
        if handle is not None:
            try:
                handle.close()
            except OSError:
    ...
    while not acquired:
        try:
            ...
            if time.monotonic() >= deadline:
                handle.close()
                raise SecGemmaLeanV38AcquisitionError(busy_code) from None
            time.sleep(0.01)
    try:
        yield
    finally:
...
def _secure_directory(path: Path) -> Path:
    if not isinstance(path, Path) or not path.is_absolute():
        raise SecGemmaLeanV38AcquisitionError("root_invalid")
    try:
...
def _secure_regular_file(path: Path) -> os.stat_result:
    try:
        ...
        raise SecGemmaLeanV38AcquisitionError("artifact_invalid") from None
    if (
        ...
        raise SecGemmaLeanV38AcquisitionError("artifact_invalid")
    return details
...
def _canonical_bytes(value: Any) -> bytes:
    ...
    while pending:
        ...
        if type(current) is dict:
            if any(type(key) is not str for key in current):
                raise SecGemmaLeanV38AcquisitionError("non_json_source_output")
    ...
    try:
        ...
        raise SecGemmaLeanV38AcquisitionError("non_json_source_output") from None
        ...
        with path.open("rb") as handle:
            while True:
                ...
                if maximum_bytes is not None and counted > maximum_bytes:
                    raise SecGemmaLeanV38AcquisitionError("artifact_invalid")
                digest.update(chunk)
    except SecGemmaLeanV38AcquisitionError:
        raise
    except OSError:
        raise SecGemmaLeanV38AcquisitionError("artifact_invalid") from None
...
def _write_immutable(path: Path, payload: bytes) -> None:
    if type(payload) is not bytes or not payload:
        raise SecGemmaLeanV38AcquisitionError("artifact_persistence_failed")
    try:
        ...
        raise
    except (OSError, SecGemmaLeanV38JournalError):
        raise SecGemmaLeanV38AcquisitionError(
...
def _prepare_layout(private_root: Path, public_root: Path, stage: str) -> _Layout:
    ...
    try:
        if private_root == public_root or private_root in public_root.parents or public_root in private_root.parents:
            raise SecGemmaLeanV38AcquisitionError("root_overlap")
        repo_root = _repository_root()
        private_relative = private_root.relative_to(repo_root)
        public_relative = public_root.relative_to(repo_root)
    except ValueError:
        raise SecGemmaLeanV38AcquisitionError("root_namespace_invalid") from None
    if private_relative.parts != ("data", ROOT_NAMESPACE) or public_relative.parts != (
        "e",
        ROOT_NAMESPACE,
    ):
        raise SecGemmaLeanV38AcquisitionError("root_namespace_invalid")
    try:
        ignore_lines = {
            line.strip()
        ...
        raise SecGemmaLeanV38AcquisitionError("private_root_not_ignored") from None
    if "data/" not in ignore_lines and "/data/" not in ignore_lines:
        raise SecGemmaLeanV38AcquisitionError("private_root_not_ignored")
...
def strict_transport_factory(private_contact: str) -> TransportFactory:
    ...
    if type(private_contact) is not str:
        raise SecGemmaLeanV38AcquisitionError("private_contact_invalid")
...
class DiskBackedSecAcquisition:
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            raise SecGemmaLeanV38AcquisitionError("configuration_invalid")
        if (
        ...
        try:
            ...
            raise SecGemmaLeanV38AcquisitionError("private_contact_invalid") from None
        self._stage = stage
        ...
        try:
            ...
            raise SecGemmaLeanV38AcquisitionError("transport_invalid") from None
        if (
            ...
            raise SecGemmaLeanV38AcquisitionError("transport_invalid")
        ...
        except SecGemmaLeanV38JournalError as error:
            raise SecGemmaLeanV38AcquisitionError(error.code) from None
    ...
    def _validate_prior(
        ...
        if index == 0:
            if prior is not None:
                raise SecGemmaLeanV38AcquisitionError("prior_stage_invalid")
            return None
        if (
    ...
    def _validate_source_surface(self) -> None:
        ...
        if self._source is None or any(
            ...
            raise SecGemmaLeanV38AcquisitionError("source_adapter_invalid")

    def _now(self) -> float:
        try:
            value = float(self._monotonic())
        except Exception:
            raise SecGemmaLeanV38AcquisitionError("clock_invalid") from None
        if not math.isfinite(value) or value < 0.0:
            raise SecGemmaLeanV38AcquisitionError("clock_invalid")
        return value
    ...
    def _audit_layout_names(self) -> None:
        ...
        try:
            ...
            raise SecGemmaLeanV38AcquisitionError("artifact_inventory_invalid") from None
        private_directories = {
            item.name for item in private_entries if item.name in private_expected
        }
        if private_directories != private_expected:
            raise SecGemmaLeanV38AcquisitionError("artifact_inventory_invalid")
    ...
    def _validate_source_evidence(self, raw: Any) -> dict[str, Any]:
        ...
        if (
            ...
            raise SecGemmaLeanV38AcquisitionError("source_evidence_invalid")
    ...
    def _parse_output_parts(
        ...
        try:
            ...
            raise SecGemmaLeanV38AcquisitionError("source_output_invalid") from None
        if (
            ...
            raise SecGemmaLeanV38AcquisitionError("source_output_invalid")
        try:
            visible = vars(output)
        except TypeError:
            raise SecGemmaLeanV38AcquisitionError("source_output_invalid") from None
    ...
    def _validate_transport_result(
        ...
        *,
        ...
        if type(result) is not TransportResult:
            raise SecGemmaLeanV38AcquisitionError("transport_result_invalid")
        try:
            ...
            raise SecGemmaLeanV38AcquisitionError("transport_result_invalid") from None
        if (
            temporary.parent != self._layout.temporary
            or temporary.resolve(strict=True) != temporary
            or result.schema_version
            != "aapl-sec-gemma-lean-evidence-v3-8-transport-v1"
    ...
    def _persist_manifest(
        ...
        if (
            ...
            raise SecGemmaLeanV38AcquisitionError("source_manifest_invalid")
    ...
    def _parse_role(
        ...
        if role.phase == "historical_submissions":
            ...
            if not role.role_id.startswith(prefix):
                raise SecGemmaLeanV38AcquisitionError("role_identity_invalid")
        ...
        if role.phase == "complete_submission":
            ...
            if type(targets) is not tuple:
                raise SecGemmaLeanV38AcquisitionError("phase_state_invalid")
            target = next(
                (
                    item
                    for item in targets
    ...
    def _process_role(
        ...
        *,
        ...
        if isinstance(dispatch.callback_error, SecGemmaLeanV38TransportError):
    ...
    def _authenticate_prior(self) -> Any | None:
        ...
        try:
            ...
            raise SecGemmaLeanV38AcquisitionError("prior_stage_invalid") from None
        if (
    ...
    def _rehydrate_current(
        ...
        *,
        ...
        try:
            ...
            raise SecGemmaLeanV38AcquisitionError("source_replay_rejected") from None
        if (
            ...
            raise SecGemmaLeanV38AcquisitionError("source_replay_rejected")
        self._contact.assert_redacted(receipt)
    ...
    def _persist_checkpoint(
        ...
        if (
            ...
            raise SecGemmaLeanV38AcquisitionError("checkpoint_invalid")
        self._contact.assert_redacted(value)
    ...
    def _terminal_result(
        ...
        *,
        ...
        if not state.accounting_complete or not state.passed:
            raise SecGemmaLeanV38AcquisitionError("accounting_incomplete")
        events = self._journal_events()
            ...
            if state.next_role_id is not None:
                ...
                try:
                    ...
                    try:
                        ...
                        raise SecGemmaLeanV38AcquisitionError(error.code) from None
                    raise SecGemmaLeanV38AcquisitionError(failure.code) from None
                invocation_ms = max(
                    invocation_role_ms,
                    self._role_duration_ms(invocation_started),
                )
...
def _read_production_json(
    ...
    try:
        ...
        raise SecGemmaLeanV38AcquisitionError(code) from None
    if type(value) is not dict:
        raise SecGemmaLeanV38AcquisitionError(code)
    return value, raw
...
def _verify_production_development_gate(
    ...
    except (OSError, SecGemmaLeanV38AcquisitionError):
        raise SecGemmaLeanV38AcquisitionError(
            "production_repository_invalid"
        ) from None
    if shown_root != root:
...
def run_production_acquisition(
    ...
    source_adapter = loaded["agent_benchmark.sec_gemma_lean_v38_source"]
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="AAPL SEC/Gemma lean v3.8 source acquisition"
    )
    parser.add_argument(
