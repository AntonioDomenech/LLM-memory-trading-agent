"""One-shot private attempt store for the v3.10 development experiment.
...
LOCK_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-10-attempt-lock-v1"
)
RESPONSE_PAYLOAD_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-10-response-payload-v1"
)
CHECKPOINT_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-10-checkpoint-v1"
)
RESPONSE_CHECKPOINT_STATE_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-10-checkpoint-state-v1"
)
PAUSE_CHECKPOINT_STATE_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-10-pause-checkpoint-state-v2"
)
...
class V310StoreError(RuntimeError):
...
class V310StoreConflict(V310StoreError):
    """The attempt is already locked or an immutable destination exists."""


class V310StorePoisoned(V310StoreError):
    """The persisted attempt cannot be accepted or resumed."""


def _translate_journal_error(exc: V310JournalError) -> V310StorePoisoned:
    return V310StorePoisoned(exc.code)


def _validate_authority(authority: Mapping[str, Any]) -> dict[str, Any]:
    if not isinstance(authority, Mapping):
        raise V310StoreError("authority_invalid")
    ...
    if not (
        ...
        raise V310StoreError("attempt_identity_invalid")
    try:
    ...
    except (V310JournalError, json.JSONDecodeError):
        raise V310StoreError("authority_invalid") from None
...
def _mkdir_new(path: Path) -> None:
    ...
    except V310JournalError as exc:
        raise _translate_journal_error(exc) from None
    except OSError:
        raise V310StorePoisoned("attempt_directory_creation_failed") from None
...
def _write_exclusive(path: Path, encoded: bytes, *, code: str) -> None:
    ...
    try:
        ...
        raise V310StorePoisoned(exc.code) from None
    except OSError:
        raise V310StorePoisoned(code) from None
    finally:
...
def _entry_exists(path: Path) -> bool:
    try:
        ...
        raise V310StorePoisoned("attempt_root_invalid") from None
    return True
...
def _publish_attempt_root_noreplace(source: Path, destination: Path) -> None:
    ...
    if (
        ...
        raise V310StorePoisoned("attempt_initialization_failed")
    ...
    if _entry_exists(destination):
        raise V310StoreConflict("attempt_already_exists")
    ...
    except V310StoreError:
        raise
    except (OSError, V310JournalError):
        if _entry_exists(destination):
            raise V310StoreConflict("attempt_already_exists") from None
        raise V310StorePoisoned("attempt_initialization_failed") from None
    finally:
...
def _initialize_attempt_root(
    ...
    *,
    ...
    try:
        ...
        staging = Path(
            tempfile.mkdtemp(
                prefix=".v310-attempt-initialization-",
        ...
        if len(machine.events) != 1 or machine.events[0].event_type != "attempt_intent":
            raise V310StorePoisoned("attempt_initialization_invalid")
        ...
        fsync_directory(staging_parent)
    except (V310StoreError, V310JournalError):
        raise
    ...
    def acquire(self) -> None:
        with _ACTIVE_LOCKS_GUARD:
            if self.path in _ACTIVE_LOCKS:
                raise V310StoreConflict("attempt_locked")
        ...
        try:
            ...
            if (
                ...
                raise V310StorePoisoned("attempt_lock_identity_changed")
            handle.seek(0)
            if handle.read() != self.expected_bytes:
                raise V310StorePoisoned("attempt_lock_invalid")
            self._handle = handle
            ...
            if isinstance(exc, V310StoreError):
                raise
            if isinstance(exc, V310JournalError):
                raise _translate_journal_error(exc) from None
            raise V310StoreConflict("attempt_locked") from None
...
def _derive_response_checkpoint_state(
    ...
    if response is None:
        raise V310StorePoisoned("checkpoint_subject_invalid")
...
def _derive_pause_checkpoint_state(
    ...
    if (
        ...
        raise V310StorePoisoned("pause_checkpoint_state_invalid")
...
def _validate_request_transition(machine: _Machine, intent: RequestIntent) -> None:
    if machine.terminal_event is not None:
        raise V310StorePoisoned("event_after_terminal")
    if machine.active_intent is not None or machine.pending_subject_event_sha256:
        raise V310StorePoisoned("request_boundary_incomplete")
    if machine.pause_committed and machine.continuation_event is None:
        raise V310StorePoisoned("request_while_paused")
    if intent.ordinal != len(machine.intents) + 1:
        raise V310StorePoisoned("request_order_invalid")
    if intent.request_id in machine.intent_ids:
        raise V310StorePoisoned("duplicate_request")
    ...
    if intent.effect_kind == "yahoo":
        if (
            ...
            raise V310StorePoisoned("yahoo_order_invalid")
        return
    ...
    if phase == "pre_probe_start":
        if (
            ...
            raise V310StorePoisoned("model_segment_start_invalid")
        return
    if intent.segment_id != machine.open_segment_id:
        raise V310StorePoisoned("model_segment_mismatch")
    if phase == "pre_probe":
        if machine.segment_phase != "pre" or machine.segment_pre_count != 1:
            raise V310StorePoisoned("pre_probe_order_invalid")
        return
    if phase == "post_probe":
        if machine.segment_phase != "generation" or machine.segment_post_count:
            raise V310StorePoisoned("post_probe_order_invalid")
        ...
        if count != expected:
            raise V310StorePoisoned("post_probe_generation_count_invalid")
        return
    if phase == "post_probe_close":
        if machine.segment_phase != "post" or machine.segment_post_count != 1:
            raise V310StorePoisoned("post_probe_order_invalid")
        return
    raise V310StorePoisoned("model_phase_invalid")
...
def _validate_private_open_transition(machine: _Machine, event_type: str) -> None:
    if not _private_values_ready(machine):
        raise V310StorePoisoned("private_values_open_order_invalid")
...
def _apply_response(machine: _Machine, event: JournalEvent) -> None:
    ...
    if not _exact_keys(
        ...
        raise V310StorePoisoned("response_event_invalid")
    ...
    if active.effect_kind == "yahoo" and not (
        ...
        raise V310StorePoisoned("yahoo_response_bytes_invalid")
    if active.effect_kind == "identity" and not (
        1 <= payload["body_bytes"] <= MODEL_RESPONSE_CAP_BYTES
    ):
        raise V310StorePoisoned("runtime_probe_response_bytes_invalid")
    if active.effect_kind == "gemma" and not (
        1 <= payload["body_bytes"] <= MODEL_RESPONSE_CAP_BYTES
    ):
        raise V310StorePoisoned("model_response_bytes_invalid")
    ...
    if active.effect_kind == "gemma":
        ...
        raise V310StorePoisoned("response_duration_invalid")
    ...
    if active.effect_kind == "yahoo":
        ...
        if machine.yahoo_body_bytes > YAHOO_BATCH_BODY_CAP_BYTES:
            raise V310StorePoisoned("yahoo_batch_body_cap_exceeded")
...
def _replay_state(
    ...
    if not replay.events:
        raise V310StorePoisoned("attempt_intent_missing")
    for index, event in enumerate(replay.events):
        ...
        if event.event_type not in _EVENT_TYPES:
            raise V310StorePoisoned("foreign_event")
        payload = event.payload
        if event.event_type == "attempt_intent":
            if (
                ...
                raise V310StorePoisoned("attempt_intent_invalid")
            continue
        if index == 0:
            raise V310StorePoisoned("attempt_intent_missing")
        ...
        if event.event_type == "request_intent":
            if not _exact_keys(
                ...
                raise V310StorePoisoned("request_intent_invalid")
            intent = RequestIntent(
                event.event_sha256,
                payload["request_id"],
                payload["effect_kind"],
        ...
        if event.event_type == "response_committed":
            if machine.pending_subject_event_sha256 is not None:
                raise V310StorePoisoned("response_order_invalid")
        ...
        if event.event_type == "checkpoint_committed":
            if not _exact_keys(
                ...
                raise V310StorePoisoned("checkpoint_event_invalid")
            if (
        ...
        if event.event_type == "paused_for_justification":
            if not _exact_keys(
                ...
                raise V310StorePoisoned("pause_event_invalid")
            expected_durations = (
        ...
        if event.event_type == "continuation_authorized":
            if not _exact_keys(
                ...
                raise V310StorePoisoned("continuation_event_invalid")
            if (
        ...
        if event.event_type in {
            ...
            if not _exact_keys(payload, set()):
                raise V310StorePoisoned("private_values_open_event_invalid")
        ...
        if event.event_type == "attempt_terminal":
            if not _exact_keys(
                ...
                raise V310StorePoisoned("terminal_event_invalid")
            if (
            ...
            if payload["status"] == "completed":
                ...
                if (
                    ...
                    raise V310StorePoisoned("completed_effect_budget_invalid")
...
def _decode_canonical_file(path: Path, *, code: str) -> tuple[dict[str, Any], bytes]:
    try:
        ...
        raise
    except (V310JournalError, OSError, UnicodeDecodeError, json.JSONDecodeError):
        raise V310StorePoisoned(code) from None
...
def _load_response(
    ...
    if stored is None:
        raise V310StorePoisoned("response_payload_missing")
    ...
    if (
        ...
        raise V310StorePoisoned("response_payload_invalid")
    try:
        body = base64.b64decode(value["body_base64"], validate=True)
    except (ValueError, TypeError):
        raise V310StorePoisoned("response_payload_invalid") from None
...
def _audit_contents(
    ...
    if set(payloads) != referenced_payloads:
        raise V310StorePoisoned("orphan_response_payload")
    ...
    for subject, checkpoint_event in machine.checkpoints.items():
        ...
        if stored is None:
            raise V310StorePoisoned("checkpoint_missing")
        value, encoded = stored
        if (
            ...
            raise V310StorePoisoned("checkpoint_invalid")
    ...
    if candidates:
        ...
        if machine.pending_subject_event_sha256 is None or len(candidates) != 1:
            raise V310StorePoisoned("orphan_checkpoint")
...
def _audit_terminal_evidence(
    ...
    if event is None:
        if stored:
            raise V310StorePoisoned("orphan_terminal_evidence")
        return None
    digest = event.payload["evidence_sha256"]
    if set(stored) != {digest}:
        if digest not in stored:
            raise V310StorePoisoned("terminal_evidence_missing")
        raise V310StorePoisoned("orphan_terminal_evidence")
    ...
    if (
        ...
        raise V310StorePoisoned("terminal_evidence_invalid")
...
def _root_layout(root: Path) -> None:
    ...
    if names != _ROOT_ENTRIES:
        raise V310StorePoisoned("attempt_root_poisoned")


def _content_bytes(subject_event_sha256: str, state: Mapping[str, Any]) -> bytes:
    if not isinstance(state, Mapping):
        raise V310StoreError("checkpoint_state_invalid")
    value = {
...
class AttemptStore:
    """Exclusively own and advance one immutable v3.10 attempt namespace."""
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(root, Path) or not root.is_absolute():
            raise V310StoreError("attempt_root_invalid")
    ...
    def _require_open(self) -> None:
        if self._closed:
            raise V310StoreError("store_closed")
        if self._poisoned:
            raise V310StorePoisoned("store_poisoned")
    ...
    def privacy_scan_locked_payloads(self) -> Mapping[str, bytes]:
        ...
        if self._lock is None:
            raise V310StoreError("attempt_lock_invalid")
    ...
    def _append(self, event_type: str, payload: Mapping[str, Any]) -> JournalEvent:
        ...
        if self._journal is None:
            raise V310StoreError("store_not_initialized")
        try:
    ...
    def begin_request(
        ...
        *,
        ...
        if self.snapshot.status in {
            ...
            raise V310StoreError("request_not_authorized")
        payload = {
        ...
        if (
            ...
            raise V310StoreError("request_intent_invalid")
    ...
    def commit_response(
        ...
        *,
        ...
        if (
            ...
            raise V310StoreError("response_commit_invalid")
        if active.effect_kind == "yahoo" and not (
            1 <= len(body) <= YAHOO_RESPONSE_CAP_BYTES
        ):
            raise V310StoreError("yahoo_response_bytes_invalid")
        if active.effect_kind == "identity" and not (
            1 <= len(body) <= MODEL_RESPONSE_CAP_BYTES
        ):
            raise V310StoreError("runtime_probe_response_bytes_invalid")
        if active.effect_kind == "gemma" and not (
            1 <= len(body) <= MODEL_RESPONSE_CAP_BYTES
        ):
            raise V310StoreError("model_response_bytes_invalid")
        payload_value = {
    ...
    def _write_content(
        ...
        *,
        ...
        if type(encoded) is not bytes or not 1 <= len(encoded) <= _MAX_CONTENT_BYTES:
            raise V310StoreError("content_size_invalid")
        if sha256_bytes(encoded) != digest:
            raise V310StoreError("content_hash_invalid")
        ...
        if final_exists:
            ...
            if allow_existing_exact:
                try:
                    ...
                    self._poisoned = True
                    raise V310StorePoisoned(exc.code) from None
            raise V310StoreConflict(conflict_code)
        if pending_exists:
            raise V310StorePoisoned("content_write_incomplete")
        try:
            ...
            try:
                ...
                except V310JournalError as presence_exc:
                    raise V310StorePoisoned(presence_exc.code) from None
                if destination_exists:
                    raise V310StoreConflict(conflict_code) from None
                raise V310StorePoisoned(exc.code) from None
            ...
            if reread != encoded:
                raise V310StorePoisoned("content_write_failed")
            return final
        except Exception:
            self._poisoned = True
            raise
    ...
    def commit_checkpoint(
        ...
        *,
        ...
        if (
            ...
            raise V310StoreError("checkpoint_subject_invalid")
    ...
    def derive_pending_checkpoint_state(self) -> Mapping[str, Any]:
        ...
        if subject is None or subject_type not in {"response", "pause"}:
            raise V310StoreError("checkpoint_not_pending")
    ...
    def committed_responses(
        ...
        *,
        ...
        if effect_kind is not None and effect_kind not in _EFFECT_KINDS:
            raise V310StoreError("response_filter_invalid")
        ...
        if any(
            ...
            raise V310StoreError("response_batch_unsealed")
        ...
        if effect_kind is None:
            ...
            if not (
                ...
                raise V310StoreError("response_values_not_opened")
            return selected
        ...
        for response in identities:
            ...
            if identity_segment is None:
                raise V310StorePoisoned("identity_segment_missing")
    ...
    def recover_pending_checkpoint(
        ...
        if not callable(builder):
            raise V310StoreError("checkpoint_builder_invalid")
        snapshot = self.snapshot
        if snapshot.status != "checkpoint_recovery":
            raise V310StoreError("checkpoint_recovery_forbidden")
        ...
        try:
            ...
            raise V310StoreError("checkpoint_rebuild_failed") from None
        if not isinstance(rebuilt, Mapping):
            raise V310StoreError("checkpoint_rebuild_failed")
        ...
        if self._checkpoint_candidates:
            if self._checkpoint_candidates != (digest,):
                self._poisoned = True
                raise V310StorePoisoned("checkpoint_mismatch")
    ...
    def commit_planned_pause(
        ...
        if (
            ...
            raise V310StoreError("pause_not_authorized")
        durations = machine.segments[0].generation_durations_ns
        projected = sum(durations) + REMAINING_COUNT * max(durations)
        if projected <= PAUSE_THRESHOLD_NS:
            raise V310StoreError("pause_threshold_not_crossed")
    ...
    def authorize_continuation(
        ...
        *,
        ...
        if (
            ...
            raise V310StoreError("continuation_not_authorized")
        return self._append(
            "continuation_authorized",
            {
    ...
    def mark_market_values_opened(self) -> JournalEvent:
        ...
        try:
            ...
            raise V310StoreError("private_values_open_not_authorized") from None
    ...
    def seal_terminal(
        ...
        *,
        ...
        if (
            ...
            raise V310StoreError("terminal_invalid")
        if status == "completed":
            expected_identity = 4 if len(machine.segments) == 1 else 8
            if (
                machine.active_intent is not None
                ...
                raise V310StoreError("completed_effect_budget_invalid")
    ...
    def committed_terminal_evidence(self) -> TerminalEvidenceReceipt:
        ...
        self._require_open()
        if self._terminal_evidence is None:
            raise V310StoreError("terminal_evidence_not_committed")
        return self._terminal_evidence
