"""Committed-blob-only implementation delta proof for SEC/Gemma v3.6.
...
MANIFEST_VERSION = "aapl-sec-gemma-lean-v36-implementation-delta-v1"
PREREG_COMMIT = "18e3c47b4e3b29449d95a241b0989d60487c623f"
PREREG_TREE = "b75ded8fcddf81f80a0a0f4d17a150d47d2c260d"
PREREG_DOC_PATH = "docs/aapl_sec_gemma_lean_evidence_v3_6.md"
PREREG_DOC_BLOB = "81dea7bddb023716e2615302a31e9be88743b3d0"
PREREG_DOC_SHA256 = (
    "8ef18188b1813f0ab759fc705205b528bc39fddf56472f15b44da9d1bfa99d73"
)
IMPLEMENTATION_BASE_COMMIT = "d1fec255a5ae6a77a047906683fc3e1f50f49298"
IMPLEMENTATION_BASE_TREE = "d85a6a79ac2f198ce50daf3aaeb25fbe8ff8b646"
_COUNTERPART_MECHANICAL_REPLACEMENTS = (
    ("sec_gemma_lean_v35", "sec_gemma_lean_v36"),
    ("SecGemmaLeanV35", "SecGemmaLeanV36"),
    ("V35", "V36"),
    ("v35", "v36"),
    ("v3_5", "v3_6"),
    ("v3-5", "v3-6"),
...
class CounterpartRule:
    """Exact committed-v3.3 comparison contract for one v3.6 delta path.

    ``expected_candidate_sha256`` binds the complete v3.6 blob for ordinary
    ...
    def validate(self, path: str) -> None:
        ...
        if modes[0]:
            ...
            if (
                path != "tests/test_sec_gemma_lean_v36_delta.py"
                or redacted_name != "V36_DELTA_MODULE_SHA256"
            ...
            path != "agent_benchmark/sec_gemma_lean_v36_delta.py"
            or anchor_path != "tests/test_sec_gemma_lean_v36_delta.py"
            or anchor_name != "V36_DELTA_MODULE_SHA256"
        ):
        ...
        expected_exclusions = {
            ...
            "tests/test_sec_gemma_lean_v36_delta.py": (
                "assignment|$module.V36_DELTA_MODULE_SHA256|1",
            ),
        }.get(path, ())
...
def _counterpart_evidence(
    ...
    *,
    ...
    """Prove one committed v3.6 blob against its pinned v3.3 counterpart."""
...
def build_delta_manifest(
    ...
    *,
    ...
    v36_paths = {entry.path for entry in implementation_diff}
    overlap = inherited_paths & v36_paths
    if overlap:
        raise DeltaValidationError(
            "inherited-at-prereg blobs changed in implementation: " + ",".join(sorted(overlap))
        )
    if v36_paths != set(allow_spec.path_rules):
        unknown = sorted(v36_paths - set(allow_spec.path_rules))
        missing = sorted(set(allow_spec.path_rules) - v36_paths)
    ...
    for entry in implementation_diff:
        ...
        entry_json["classification"] = "v36_implementation_delta"
    ...
    for entry in parent_to_implementation:
        ...
        elif entry.path in v36_paths:
            classification = "v36_implementation_delta"
        else:
    ...
    if {entry["path"] for entry in classified_parent_paths} != inherited_paths | v36_paths:
...
# This table binds the exact twelve-file v3.6 projection derived from v3.5.
_V35_CLONE_PATH_RULES = {
    "agent_benchmark/sec_gemma_lean_v36_acquisition.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="f3bfc0b6e3567b5e2016d70325aef27c21e456ce0b52919cb3c46239b10548f7",
        expected_symbol_count=216,
        expected_import_inventory_sha256="b7596de7d27b85b1b93ed3efa27f45363801cc6fd0c394afe3820fa348d9f3ae",
        expected_import_count=29,
    ),
    "agent_benchmark/sec_gemma_lean_v36_delta.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="ccf695c5daebb064a7d6b87b5ff25df8335cce34bd757cd697d0798a884ec2a9",
    ...
    "agent_benchmark/sec_gemma_lean_v36_journal.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="a8c83c5c5c4a512607424579d552ebdf2698eae2d13650fb73ddd2e5beb1e667",
    ...
    "agent_benchmark/sec_gemma_lean_v36_preflight.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="d8800b6bcaf4976827edd6a10caac434e59deb93617fdbc928525409dba922c0",
        expected_symbol_count=144,
        expected_import_inventory_sha256="811b8d2c9d831ee2c8378bd750059fa287ed074851ba68b85093137c96e668f8",
        expected_import_count=25,
    ),
    "agent_benchmark/sec_gemma_lean_v36_source.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="38a53f603cddf28b1f1ca033a1c31ec977060ab2e2e39593763f98b6c20ef1c5",
        expected_symbol_count=332,
        expected_import_inventory_sha256="19509969d65006670d70395eaa9ff4c83c7d9e0d47fb59925d5ceff9d134ec06",
        expected_import_count=14,
    ),
    "agent_benchmark/sec_gemma_lean_v36_transport.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="ff91eec2b11906129c42a4ab9871b850299d10601e952d71c58571ca0797546d",
    ...
    "tests/test_sec_gemma_lean_v36_acquisition.py": PathRule(
        "A", (), (),
        ...
        expected_import_inventory_sha256="da44326ae42c1ca2be644483d9241a80a87b10408d1c00dd562ab91177fe7794",
        expected_import_count=21,
    ),
    "tests/test_sec_gemma_lean_v36_delta.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="261b7ce6f1df94b06fc51a8d2b8f6aef4a5fb6c801a84ac16196dbeace703cd3",
        expected_symbol_count=52,
        expected_import_inventory_sha256="0b916b420e379bd7d15394939c5678a79f434a1a9b46086e4db794f7be76e21c",
        expected_import_count=10,
    ),
    "tests/test_sec_gemma_lean_v36_journal.py": PathRule(
        "A", (), (),
        ...
        expected_import_inventory_sha256="58b8add037745a9684bd810eeae465a3d673ca296975c26a9f520ba2acadcc9b",
        expected_import_count=9,
    ),
    "tests/test_sec_gemma_lean_v36_preflight.py": PathRule(
        "A", (), (),
        ...
        expected_import_inventory_sha256="50b4e508de95f06a0523f6aa30bcb9e4b90ce7a823df8dcd9f49ee4cd303191c",
        expected_import_count=11,
    ),
    "tests/test_sec_gemma_lean_v36_source.py": PathRule(
        "A", (), (),
        ...
        expected_import_inventory_sha256="d95973461f09f5572cf09f3f55730b5c4bfa6b76dfae40dbcf02253562ac21c3",
        expected_import_count=12,
    ),
    "tests/test_sec_gemma_lean_v36_transport.py": PathRule(
        "A", (), (),
        ...
        expected_import_inventory_sha256="cd8ada4939de83e65cb2c2f883a55b2800dbe6d578883c24621047ef61e53106",
        expected_import_count=16,
    ),
}
# These are exactly the twelve additive v3.6 paths.  No shared-file
# modification is admitted when DEFAULT_ALLOW_SPEC is built.
_V36_IMPLEMENTATION_PATHS = (
    ...
    "tests/test_sec_gemma_lean_v36_acquisition.py",
    "tests/test_sec_gemma_lean_v36_delta.py",
    "tests/test_sec_gemma_lean_v36_journal.py",
    "tests/test_sec_gemma_lean_v36_preflight.py",
    "tests/test_sec_gemma_lean_v36_source.py",
    "tests/test_sec_gemma_lean_v36_transport.py",
)
...
_COUNTERPART_RULE_DATA = {'agent_benchmark/sec_gemma_lean_v36_acquisition.py': {'counterpart_path': 'agent_benchmark/sec_gemma_lean_v35_acquisition.py',
                                                       'excluded_change_symbol_ids': (),
                                                       'expected_candidate_sha256': 'ab84d66545622e6dbf320ad9b09222ea52125b8f2250f3daa84e88579deab5d0',
 ...
 'agent_benchmark/sec_gemma_lean_v36_delta.py': {'counterpart_path': 'agent_benchmark/sec_gemma_lean_v35_delta.py',
                                                 'excluded_change_symbol_ids': ('assignment|$module._COUNTERPART_RULE_DATA|1',),
                                                 'expected_candidate_sha256': None,
                                                 'expected_changed_import_specs': (),
                                                 'expected_changed_symbol_evidence_sha256': 'eaf879c608208f3e8a1c6a16e433ad232045534f5dfa7786642a85a1406e4209',
                                                 'expected_changed_symbol_ids': ('assignment|$module.IMPLEMENTATION_BASE_COMMIT|1',
                                                                                 ...
                                                                                 'assignment|$module._V34_CLONE_PATH_RULES|1',
                                                                                 'assignment|$module._V35_CLONE_PATH_RULES|1'),
                                                 'expected_mechanical_counts': (43,
                                                                                ...
                                                                                7),
                                                 'expected_redacted_candidate_sha256': None,
                                                 'external_sha256_anchor_assignment': 'V36_DELTA_MODULE_SHA256',
                                                 'external_sha256_anchor_path': 'tests/test_sec_gemma_lean_v36_delta.py',
                                                 'redacted_literal_assignment': None},
 'agent_benchmark/sec_gemma_lean_v36_journal.py': {'counterpart_path': 'agent_benchmark/sec_gemma_lean_v35_journal.py',
                                                   'excluded_change_symbol_ids': (),
                                                   'expected_candidate_sha256': '1bef2c7ab87986b1b11cc59203cf9bf8f5f8ec49bb5ff6eb94edee7d461d36a3',
                                                   'expected_changed_import_specs': (),
                                                   'expected_changed_symbol_evidence_sha256': '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945',
                                                   'expected_changed_symbol_ids': (),
                                                   ...
                                                   'external_sha256_anchor_path': None,
                                                   'redacted_literal_assignment': None},
 'agent_benchmark/sec_gemma_lean_v36_preflight.py': {'counterpart_path': 'agent_benchmark/sec_gemma_lean_v35_preflight.py',
                                                     'excluded_change_symbol_ids': (),
                                                     'expected_candidate_sha256': '579f47c1c3e673caf5d41e5e97f5e85f97870dafad46f2136c51a9dfbdb096f1',
                                                     'expected_changed_import_specs': (),
                                                     'expected_changed_symbol_evidence_sha256': '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945',
                                                     'expected_changed_symbol_ids': (),
                                                     ...
                                                     'external_sha256_anchor_path': None,
                                                     'redacted_literal_assignment': None},
 'agent_benchmark/sec_gemma_lean_v36_source.py': {'counterpart_path': 'agent_benchmark/sec_gemma_lean_v35_source.py',
                                                  'excluded_change_symbol_ids': (),
                                                  'expected_candidate_sha256': '2d65b74a73660e901688a69d8aa754e918d3cdc0acd2de017917a8afa7c1a583',
                                                  'expected_changed_import_specs': ('from:0:datetime:date,datetime,timedelta,timezone',
                                                                                    'from:0:zoneinfo:ZoneInfo',
                                                                                    'from:1:sec_point_in_time:AAPL_CIK,FilingRecord,SecPointInTimeError,content_sha256,parse_submissions_acceptance_datetime as _parse_submissions_acceptance_wall_clock'),
                                                  'expected_changed_symbol_evidence_sha256': '434349b5a35021b41b8717216a19935d8922a511b35c37095aac9cf90eefbc95',
                                                  'expected_changed_symbol_ids': ('function|parse_submissions_acceptance_datetime|1',
                                                                                  'import|$module.__import__|12',
                                                                                  'import|$module.__import__|14',
                                                  ...
                                                  'external_sha256_anchor_path': None,
                                                  'redacted_literal_assignment': None},
 'agent_benchmark/sec_gemma_lean_v36_transport.py': {'counterpart_path': 'agent_benchmark/sec_gemma_lean_v35_transport.py',
                                                     'excluded_change_symbol_ids': (),
                                                     'expected_candidate_sha256': '2abce230e86044b240836123a212f513990e5730443317136fe3bea40219cc64',
                                                     'expected_changed_import_specs': (),
                                                     'expected_changed_symbol_evidence_sha256': '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945',
                                                     'expected_changed_symbol_ids': (),
                                                     ...
                                                     'external_sha256_anchor_path': None,
                                                     'redacted_literal_assignment': None},
 'tests/test_sec_gemma_lean_v36_acquisition.py': {'counterpart_path': 'tests/test_sec_gemma_lean_v35_acquisition.py',
                                                  'excluded_change_symbol_ids': (),
                                                  'expected_candidate_sha256': 'd8c342e0aa28ec841b0da46177f3e76db4d1cbe9ff30d2cc606f0f3ed97864e3',
                                                  'expected_changed_import_specs': (),
                                                  'expected_changed_symbol_evidence_sha256': '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945',
                                                  'expected_changed_symbol_ids': (),
                                                  ...
                                                  'external_sha256_anchor_path': None,
                                                  'redacted_literal_assignment': None},
 'tests/test_sec_gemma_lean_v36_delta.py': {'counterpart_path': 'tests/test_sec_gemma_lean_v35_delta.py',
                                            'excluded_change_symbol_ids': ('assignment|$module.V36_DELTA_MODULE_SHA256|1',),
                                            'expected_candidate_sha256': None,
                                            'expected_changed_import_specs': (),
                                            'expected_changed_symbol_evidence_sha256': '5a283a7c1d54d6302ee00c3a7ae206a05b7872dfbdc90101cdf64ff807385c16',
                                            'expected_changed_symbol_ids': ('assignment|$module.EXPECTED_V36_COUNTERPARTS|1',
                                                                             ...
                                                                             'function|test_checker_rejects_fixed_offset_broad_timestamp_scope_and_observed_special_case|1',
                                            ...
                                            'expected_mechanical_counts': (17, 0, 8, 1, 1, 0, 0),
                                            'expected_redacted_candidate_sha256': '32b0fcadb598bcc60b3cb6664ab06714f5c659336f532db824aa9fc9673ab9fe',
                                            'external_sha256_anchor_assignment': None,
                                            'external_sha256_anchor_path': None,
                                            'redacted_literal_assignment': 'V36_DELTA_MODULE_SHA256'},
 'tests/test_sec_gemma_lean_v36_journal.py': {'counterpart_path': 'tests/test_sec_gemma_lean_v35_journal.py',
                                              'excluded_change_symbol_ids': (),
                                              'expected_candidate_sha256': '2b2d12ca98ab2a1637a62a413d22e17b30afc3f2bbfa3f4b7f9ce21fc3281c50',
                                              'expected_changed_import_specs': (),
                                              'expected_changed_symbol_evidence_sha256': '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945',
                                              'expected_changed_symbol_ids': (),
                                              ...
                                              'external_sha256_anchor_path': None,
                                              'redacted_literal_assignment': None},
 'tests/test_sec_gemma_lean_v36_preflight.py': {'counterpart_path': 'tests/test_sec_gemma_lean_v35_preflight.py',
                                                'excluded_change_symbol_ids': (),
                                                'expected_candidate_sha256': '7a877928d3111cbcc74e0147e75ee11497ba2af38b0f4a6f77e9759f2160d3d1',
                                                'expected_changed_import_specs': (),
                                                'expected_changed_symbol_evidence_sha256': '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945',
                                                'expected_changed_symbol_ids': (),
                                                ...
                                                'external_sha256_anchor_path': None,
                                                'redacted_literal_assignment': None},
 'tests/test_sec_gemma_lean_v36_source.py': {'counterpart_path': 'tests/test_sec_gemma_lean_v35_source.py',
                                             'excluded_change_symbol_ids': (),
                                             'expected_candidate_sha256': 'a6166de374e46c6a511f84155b2bc2ea156f164df6e6d0a2627865131ee1f69e',
                                             'expected_changed_import_specs': (),
                                             'expected_changed_symbol_evidence_sha256': '07213534d111ddf865b17f34a2ffdfe7d0f471b369b8c1ffa7a36083c4ac3674',
                                             'expected_changed_symbol_ids': ('function|test_acceptance_mismatch_and_missingness_contract_is_unchanged|1',
                                                                             'function|test_acceptance_representation_semantics_are_exact_and_dst_hostile|1',
                                                                             'function|test_utc_acceptance_reconciles_to_eastern_header_across_date_rollover|1'),
                                             'expected_mechanical_counts': (2, 51, 0, 0, 0, 0, 1),
                                             'expected_redacted_candidate_sha256': None,
                                             'external_sha256_anchor_assignment': None,
                                             'external_sha256_anchor_path': None,
                                             'redacted_literal_assignment': None},
 'tests/test_sec_gemma_lean_v36_transport.py': {'counterpart_path': 'tests/test_sec_gemma_lean_v35_transport.py',
                                                'excluded_change_symbol_ids': (),
                                                'expected_candidate_sha256': 'fe4269188d830ae740b45ede9d89472efebaf8d01a40e55fe6048cbc4c293c90',
                                                'expected_changed_import_specs': (),
                                                'expected_changed_symbol_evidence_sha256': '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945',
                                                'expected_changed_symbol_ids': (),
...
_DEFAULT_EXISTING_PATH_RULES = {
    path: _V35_CLONE_PATH_RULES[path] for path in _V36_IMPLEMENTATION_PATHS
}
_DEFAULT_COUNTERPART_RULES = {
    path: _counterpart_rule_from_data(_COUNTERPART_RULE_DATA[path])
    for path in _V36_IMPLEMENTATION_PATHS
}
