"""Owned, one-shot SEC reader for an exact consumed stage grant.

This is the only production entry point that may turn a persisted stage grant
into SEC network effects.  The caller supplies no stage, candidate, URL,
digest, path, byte payload, transport, or budget.  Those capabilities are
derived after an atomic claim from the reveal store's independently validated
current tip.  A claim is never retried after an indeterminate process exit.
...
from .sec_filing_gemma_contract import canonical_sha256
