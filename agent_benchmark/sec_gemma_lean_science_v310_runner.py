"""Development-only v3.10 SEC-to-science execution worker.
...
command after the separately pushed v3.10 preflight has authorized it.
"""
...
RUNNER_SCHEMA_VERSION: Final[str] = "aapl-sec-gemma-lean-science-v3-10-runner-v1"
MODEL_PLAN_SCHEMA_VERSION: Final[str] = "aapl-sec-gemma-lean-science-v3-10-model-plan-v1"
...
RUNTIME_AGGREGATE_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-10-runtime-aggregate-v1"
)
...
SEMANTIC_EVENT_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-10-semantic-event-v1"
)
SEMANTIC_ROW_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-10-semantic-row-v1"
)
SEMANTIC_BATCH_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-10-semantic-batch-v1"
)
DETERMINISTIC_PAYLOAD_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-10-deterministic-payload-v1"
)
TERMINAL_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-10-development-result-v1"
)
...
CONTINUATION_DOCUMENT_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-10-continuation-document-v1"
)
...
class V310RunnerError(RuntimeError):
    """A fixed public-safe v3.10 runner failure."""
    ...
    def __repr__(self) -> str:
        return f"V310RunnerError(code={self.code!r})"


def _fail(code: str) -> None:
    raise V310RunnerError(code)
...
def build_attempt_authority(
    *,
    ...
    if (
        ...
        or source.get("base_commit") != contract.V38_SOURCE_COMMIT
        or source.get("base_tree") != contract.V38_SOURCE_TREE
...
def build_deterministic_payload(
    *, stage_slice: Mapping[str, Any], semantic_payload: Mapping[str, Any]
    ...
    """Mint frozen feature rows while retaining v3.10 segment provenance."""
    ...
    source_commitments = {
        **source_body,
        "v310_source_commitments_sha256": contract.canonical_sha256(source_body),
    }
...
def _v310_market_plan() -> dict[str, Any]:
    try:
...
def _checkpoint_state(*, phase: str, request_sha256: str, body_sha256: str) -> dict[str, Any]:
    return {
        "schema_version": "aapl-sec-gemma-lean-science-v3-10-checkpoint-state-v1",
        "phase": phase,
...
def _assert_complete_v310_private_namespace_private_free(
    ...
    repository and v3.10-private paths.  Those paths remain forbidden in every
...
def _fsync_public_directory(path: Path) -> None:
    try:
        from .sec_gemma_lean_science_v310_journal import fsync_directory
...
def build_comparison_update(existing: bytes, result: Mapping[str, Any]) -> bytes:
    ...
    marker = "| SEC/Gemma lean science v3.9 |"
    ...
    row = (
        "| SEC/Gemma lean science v3.10 | Authenticated v3.8 source bridge, local "
    ...
    existing_v310 = [
        ...
        if line.startswith("| SEC/Gemma lean science v3.10 |")
    ]
    if existing_v310:
        if existing_v310 != [(positions[0] + 1, row)]:
...
def publish_result_artifacts(repo_root: Path, value: Mapping[str, Any]) -> None:
    ...
    pending = comparison_path.with_name(comparison_path.name + ".v310-pending")
...
def build_production_development_dependencies() -> DevelopmentDependencies:
    ...
    try:
        ...
        from .sec_gemma_lean_science_v310_preflight import (
            V38_PRIVATE_ROOT,
...
def _load_and_validate_pause_artifact(
    ...
    *,
    ...
    _assert_complete_v310_private_namespace_private_free(
        store,
        forbidden_tokens=forbidden_tokens,
    )
...
def status(repo_root: Path) -> dict[str, Any]:
    """Read only the safe v3.10 attempt summary; never open private payload bodies."""

    try:
        from .sec_gemma_lean_science_v310_store import audit_attempt
        from .sec_gemma_lean_science_v310_preflight import (
            load_execution_authority,
            load_publication_recovery_authority,
        )
...
def run_development(
    ...
    *,
    ...
    descendant = (
        ...
        or (
            root / Path(contract.PAUSE_ARTIFACT_PATH + ".v310-pending")
        ).exists()
        or (root / Path(contract.RESULT_ARTIFACT_PATH)).exists()
        or (
            root / Path(contract.RESULT_ARTIFACT_PATH + ".v310-pending")
        ).exists()
    ...
    v310_private = str((root / Path(contract.PRIVATE_NAMESPACE)).resolve(strict=False))
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="AAPL SEC/Gemma v3.10 science runner")
