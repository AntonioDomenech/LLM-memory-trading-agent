"""Pure offline source sealing for AAPL SEC/Gemma lean evidence v3.8.
...
SOURCE_SCHEMA_VERSION = "aapl-sec-gemma-lean-v38-source-seal-v1"
DETACHED_REPLAY_SCHEMA_VERSION = (
    "aapl-sec-gemma-lean-v38-detached-source-replay-v1"
)
LEGACY_SCIENCE_PROJECTION_SCHEMA_VERSION = (
    "aapl-sec-gemma-lean-v38-legacy-science-projection-v1"
)
PRE_MASTER_UPPER_BOUND_SCHEMA_VERSION = (
    "aapl-sec-gemma-lean-v38-pre-master-upper-bound-v1"
)
COMPLETED_YEAR_COVERAGE_SCHEMA_VERSION = (
    "aapl-sec-gemma-lean-v38-completed-year-source-coverage-v1"
)
COMPACT_ROLE_EVIDENCE_SCHEMA_VERSION = (
    "aapl-sec-gemma-lean-v38-compact-role-evidence-v1"
)
COMPACT_SUBMISSIONS_SCHEMA_VERSION = (
    "aapl-sec-gemma-lean-v38-compact-submissions-v1"
)
COMPACT_RECONCILIATION_SCHEMA_VERSION = (
    "aapl-sec-gemma-lean-v38-compact-reconciliation-v1"
)
COMPACT_COMPLETE_SCHEMA_VERSION = (
    "aapl-sec-gemma-lean-v38-compact-complete-v1"
)
COMPACT_ROLE_MANIFEST_SCHEMA_VERSION = (
    "aapl-sec-gemma-lean-v38-compact-role-manifest-v1"
)
COMPACT_CHECKPOINT_SCHEMA_VERSION = (
    "aapl-sec-gemma-lean-v38-compact-checkpoint-v1"
)
COMPACT_REPLAY_SCHEMA_VERSION = (
    "aapl-sec-gemma-lean-v38-compact-detached-replay-v1"
)
...
class SecGemmaLeanV38SourceError(SecPointInTimeError):
    """Caller-supplied bytes violate the frozen v3.8 source contract."""
...
def _canonical_json_bytes(value: Any) -> bytes:
    try:
        ...
        raise SecGemmaLeanV38SourceError(
            "v3.8 source evidence is not finite canonical JSON"
        ) from None
...
def _json_object(payload: bytes, *, location: str) -> dict[str, Any]:
    if type(payload) is not bytes:
        raise SecGemmaLeanV38SourceError(f"{location} must be exact response bytes")
    ...
    except SecGemmaLeanV38SourceError:
        raise SecGemmaLeanV38SourceError(
            f"{location} contains a duplicate JSON object key"
        ) from None
    except (UnicodeDecodeError, json.JSONDecodeError):
        raise SecGemmaLeanV38SourceError(f"{location} is not strict UTF-8 JSON") from None
    if type(value) is not dict:
        raise SecGemmaLeanV38SourceError(f"{location} must be a JSON object")
...
def _canonical_date(value: Any, *, location: str, allow_empty: bool = False) -> str:
    if type(value) is not str:
        raise SecGemmaLeanV38SourceError(f"{location} must remain text")
    if allow_empty and value == "":
        return value
    try:
        parsed = date.fromisoformat(value)
    except ValueError:
        raise SecGemmaLeanV38SourceError(f"{location} is not a canonical ISO date") from None
    if parsed.isoformat() != value:
        raise SecGemmaLeanV38SourceError(f"{location} is not a canonical ISO date")
    return value
...
def _canonical_cik(value: Any, *, location: str) -> str:
    ...
    if not raw.isdigit() or len(raw) > 10:
        raise SecGemmaLeanV38SourceError(f"{location} is not a canonical SEC CIK")
...
def _flag(value: Any, *, location: str) -> bool:
    ...
    if type(value) is str and value in {"", "0", "1"}:
        return value == "1"
    raise SecGemmaLeanV38SourceError(f"{location} must be an exact zero/one flag")
...
def _safe_primary_name(value: str, *, location: str, allow_empty: bool) -> str:
    ...
    if (
        ...
        raise SecGemmaLeanV38SourceError(f"{location} is not a safe basename")
    return value
...
def _stage_config(value: str | StageSourceConfig) -> StageSourceConfig:
    if isinstance(value, StageSourceConfig):
        ...
        if expected != value:
            raise SecGemmaLeanV38SourceError("Stage configuration is not frozen v3.8")
        return value
    if type(value) is not str or value not in STAGE_SOURCE_CONFIGS:
        raise SecGemmaLeanV38SourceError("Unknown v3.8 source stage")
...
def quarter_roles(
    ...
    if len(result) != config.quarter_count:
        raise SecGemmaLeanV38SourceError("Frozen quarter count is internally inconsistent")
    return tuple(result)
...
def _historical_references(main: Mapping[str, Any]) -> tuple[HistoricalReference, ...]:
    if _canonical_cik(main.get("cik", ""), location="main Submissions CIK") != AAPL_CIK:
        raise SecGemmaLeanV38SourceError("Main Submissions response is not Apple")
    filings = main.get("filings")
    if type(filings) is not dict or type(filings.get("recent")) is not dict:
        raise SecGemmaLeanV38SourceError("Main Submissions lacks filings.recent")
    raw_references = filings.get("files")
    if type(raw_references) is not list:
        raise SecGemmaLeanV38SourceError("Main Submissions filings.files is not an array")
    if len(raw_references) > MAX_HISTORICAL_REFERENCES:
        raise SecGemmaLeanV38SourceError("Historical Submissions reference cap exceeded")
    ...
    for position, raw in enumerate(raw_references):
        ...
        if (
            ...
            raise SecGemmaLeanV38SourceError("Historical reference name is unsafe")
        if name in seen:
            raise SecGemmaLeanV38SourceError("Historical reference name is duplicated")
        seen.add(name)
        ...
        if filing_from > filing_to:
            raise SecGemmaLeanV38SourceError("Historical reference range is reversed")
        result.append(
            HistoricalReference(
                name=name,
...
def _parse_columns(
    ...
    *,
    ...
    if type(columns) is not dict:
        raise SecGemmaLeanV38SourceError(f"{source_name} is not a column object")
    mandatory_text = (
        "accessionNumber",
        "acceptanceDateTime",
        "form",
    ...
    for position in range(size):
        ...
        if not form or form != form.strip():
            raise SecGemmaLeanV38SourceError(f"{source_name} contains an invalid form")
...
def _quarter_bounds(year: int, quarter: int) -> tuple[date, date]:
    if type(year) is not int or type(quarter) is not int or not 1 <= quarter <= 4:
        raise SecGemmaLeanV38SourceError("Master role year/quarter is invalid")
...
def _strict_gzip_decompress(payload: bytes) -> bytes:
    if type(payload) is not bytes:
        raise SecGemmaLeanV38SourceError("master.gz must be exact response bytes")
    if len(payload) > MAX_MASTER_COMPRESSED_BYTES:
        raise SecGemmaLeanV38SourceError("master.gz compressed byte cap exceeded")
    if len(payload) < 18 or payload[:2] != b"\x1f\x8b":
        raise SecGemmaLeanV38SourceError("master.gz has invalid gzip magic")
    ...
    try:
        ...
        raise SecGemmaLeanV38SourceError("master.gz failed CRC/ISIZE/gzip validation") from None
    if total > MAX_MASTER_DECOMPRESSED_BYTES:
        raise SecGemmaLeanV38SourceError("master.gz decompressed byte cap exceeded")
    if not decoder.eof:
        raise SecGemmaLeanV38SourceError("master.gz is truncated before member EOF")
...
def parse_strict_master_gzip(
    ...
    *,
    ...
    for line in lines[start:]:
        ...
        if line in seen_raw:
            raise SecGemmaLeanV38SourceError("master.gz contains a repeated raw row")
        seen_raw.add(line)
        ...
        if not form or not filename or (is_apple_target and not company) or any(
            ...
            raise SecGemmaLeanV38SourceError("master.gz row contains an empty field")
...
def reconcile_master_exact_set(
    ...
    if isinstance(master_quarters, (str, bytes)) or not isinstance(
        ...
        raise SecGemmaLeanV38SourceError("master quarters must be a sequence")
    ...
    for evidence in master_quarters:
        if not isinstance(evidence, MasterQuarterEvidence):
            raise SecGemmaLeanV38SourceError("master evidence type is invalid")
        ...
        if evidence.role.key in evidence_by_key:
            raise SecGemmaLeanV38SourceError("master quarter evidence is duplicated")
        evidence_by_key[evidence.role.key] = evidence
    if set(evidence_by_key) != {role.key for role in expected_roles}:
        raise SecGemmaLeanV38SourceError("master quarter evidence set is not exact")
    ...
    if len(targets) > config.i_cap:
        raise SecGemmaLeanV38SourceError("Frozen I cap is exceeded")
    body = {
        "stage": config.stage,
...
def _validate_seal_hash(seal: Mapping[str, Any]) -> dict[str, Any]:
    if not isinstance(seal, Mapping):
        raise SecGemmaLeanV38SourceError("Prior stage source seal is not an object")
    value = dict(seal)
    ...
    if type(supplied) is not str or _BARE_SHA256_RE.fullmatch(supplied) is None:
        raise SecGemmaLeanV38SourceError("Stage source seal hash is malformed")
    if _canonical_sha256(value) != supplied:
        raise SecGemmaLeanV38SourceError("Stage source seal hash does not validate")
    return dict(seal)
...
def _authenticated_prior_authority(
    ...
    if not isinstance(prior_bundle, StageSourceBundle):
        raise SecGemmaLeanV38SourceError(
            "A later v3.8 stage requires its exact adjacent prior raw bundle"
        )
    expected_prior_stage = _STAGE_ORDER[_STAGE_ORDER.index(config.stage) - 1]
    if prior_bundle.config.stage != expected_prior_stage:
        raise SecGemmaLeanV38SourceError("Prior source bundle stage is not adjacent")
...
def _submissions_candidates(
    ...
    for row in snapshot.rows:
        ...
        if filed < coverage_start:
            raise SecGemmaLeanV38SourceError("master_boundary_incomplete")
...
def derive_complete_submission_plan(
    ...
    *,
    ...
    if config.stage == "development" and len(plan) != len(reconciliation.targets):
        raise SecGemmaLeanV38SourceError("Development requires P exactly equal I")
    if len(plan) > config.p_cap:
        raise SecGemmaLeanV38SourceError("Frozen P cap is exceeded")
    return plan
...
def _selector_record(row: SubmissionRow, normalized_acceptance: str | None) -> FilingRecord:
    # FilingRecord's constructor predates v3.8 and requires an acceptance value.
...
def _exact_header_sgml_form(
    ...
    *,
    ...
    """Require one exact raw SEC/IMS-header form value for v3.8."""
...
def _exact_document_form_sequence_topology(
    ...
    *,
    ...
    """Require v3.8's exact all-document TYPE/sequence topology."""
    ...
    if not 1 <= selected_ordinal <= len(documents):
        raise SecGemmaLeanV38SourceError("Selected SGML document ordinal is invalid")
    ...
    for ordinal, document in enumerate(documents, start=1):
        ...
        if text_start is None:
            raise SecGemmaLeanV38SourceError("SGML document lacks TEXT")
...
def reconcile_complete_submission(
    ...
    *,
    ...
    if acquisition_stage not in _STAGE_ORDER:
        raise SecGemmaLeanV38SourceError("Complete source acquisition stage is invalid")
    ...
    if len(supplied_acceptances) > 1:
        raise SecGemmaLeanV38SourceError("Acceptance values do not reconcile lexically")
    ...
    if not (
        ...
        raise SecGemmaLeanV38SourceError("Embedded TEXT byte offsets are invalid")
    ...
    if not normalized.usable:
        raise SecGemmaLeanV38SourceError("Selected embedded TEXT is unusable")
...
def validate_cross_stage_frozen_prefix(
    ...
    if (
        ...
        raise SecGemmaLeanV38SourceError("Cross-stage source seals are not adjacent")
    expected_chain = [
    ...
    if type(prior_config) is not dict:
        raise SecGemmaLeanV38SourceError("Prior source seal config is invalid")
    ...
    def indexed(rows: Any, *, cutoff: str | None) -> dict[str, dict[str, Any]]:
        if type(rows) is not list:
            raise SecGemmaLeanV38SourceError("Stage source target rows are invalid")
        result: dict[str, dict[str, Any]] = {}
        for raw in rows:
            if type(raw) is not dict:
                raise SecGemmaLeanV38SourceError("Stage source target row is invalid")
    ...
    *,
    ...
    for source in complete_sources:
        if not isinstance(source, CompleteSourceEvidence):
            raise SecGemmaLeanV38SourceError("Complete source evidence type is invalid")
        if source.accession_number in source_by_accession:
            raise SecGemmaLeanV38SourceError("Complete source evidence is duplicated")
...
def _assemble_stage_seal_metadata(
    *,
    ...
    if p_accessions != expected_order or len(p_accessions) != len(set(p_accessions)):
        raise SecGemmaLeanV38SourceError("P is not in frozen filing-date/accession order")
    if len(p_accessions) > config.p_cap:
        raise SecGemmaLeanV38SourceError("Frozen P cap is exceeded")
    u_rows = sorted(
        (
    ...
    if not config.d_min <= len(d_rows) <= config.d_max:
        raise SecGemmaLeanV38SourceError("Frozen admitted D range/cap is violated")
    ...
    body = {
        ...
        "source_plan": {
            ...
            "subtraction": "I_minus_authenticated_adjacent_prior_v38_seals",
        },
        "counts": {
...
def _validate_target_row_binding(row: Mapping[str, Any]) -> tuple[str, str]:
    ...
    if (
        ...
        raise SecGemmaLeanV38SourceError("Stage source target row is not bound")
    ...
    if set(prefix) != expected_prefix_keys:
        raise SecGemmaLeanV38SourceError("Frozen-prefix schema is not exact")
    ...
    if (
        ...
        raise SecGemmaLeanV38SourceError("Master identity schema is not exact")
    form = row.get("form")
    ...
    if type(header) is not dict or set(header) != {
        ...
        raise SecGemmaLeanV38SourceError("SGML header identity schema is not exact")
    if (
    ...
    if type(acceptance) is not dict or set(acceptance) != {
        ...
        raise SecGemmaLeanV38SourceError("Acceptance evidence schema is not exact")
    for name in ("submissions_missing", "header_missing", "exact"):
        if type(acceptance[name]) is not bool:
            raise SecGemmaLeanV38SourceError("Acceptance missingness is not exact")
    ...
    if normalized_acceptance is not None and (
        ...
        raise SecGemmaLeanV38SourceError("Normalized acceptance is malformed")
    if (
    ...
    if type(response) is not dict or set(response) != {"url", "sha256", "length"}:
        raise SecGemmaLeanV38SourceError("Complete-response provenance is not exact")
    if response.get("url") != (
        "https://www.sec.gov/Archives/"
        f"edgar/data/320193/{accession}.txt"
    ):
        raise SecGemmaLeanV38SourceError("Complete-response URL is not exact")
    if type(extracted) is not dict or set(extracted) != {
        ...
        raise SecGemmaLeanV38SourceError("Extracted-TEXT provenance is not exact")
    if type(normalized) is not dict or set(normalized) != {
        ...
        raise SecGemmaLeanV38SourceError("Normalized-text provenance is not exact")
    ...
    if type(selection) is not dict or set(selection) != expected_selection_keys:
        raise SecGemmaLeanV38SourceError("Primary-selection schema is not exact")
    if (
...
def validate_stage_source_seal(seal: Mapping[str, Any]) -> None:
    ...
    if set(value) != expected_top_level_keys:
        raise SecGemmaLeanV38SourceError("Stage source seal schema is not exact")
    if value.get("schema_version") != SOURCE_SCHEMA_VERSION:
        raise SecGemmaLeanV38SourceError("Stage source seal schema is invalid")
    stage = value.get("stage")
    config = _stage_config(stage)
    if value.get("config") != config.to_dict():
        raise SecGemmaLeanV38SourceError("Stage source seal config is not frozen")
    ...
    if len(accessions) != len(set(accessions)):
        raise SecGemmaLeanV38SourceError("Stage source target rows are duplicated")
    ...
    if type(counts) is not dict or type(source_plan) is not dict:
        raise SecGemmaLeanV38SourceError("Stage source seal counts/plan are invalid")
    if set(source_plan) != {
        ...
        raise SecGemmaLeanV38SourceError("Stage source plan schema is not exact")
    expected_count_keys = {
        "H",
        "Q",
        "I",
    ...
    if set(counts) != expected_count_keys or any(
        ...
        raise SecGemmaLeanV38SourceError("Stage source count schema is invalid")
    ...
    if (
        ...
        raise SecGemmaLeanV38SourceError("Stage source plan/counts do not reconcile")
    p_set = set(p)
    ...
    if (
        ...
        != "I_minus_authenticated_adjacent_prior_v38_seals"
    ):
        raise SecGemmaLeanV38SourceError("Stage source plan order is invalid")
    ...
    if (
        ...
        raise SecGemmaLeanV38SourceError("Stage source U/D counts do not reconcile")
    exact_count = sum(
...
def _parse_master_set(
    ...
    if type(master_gzip_payloads) is not dict:
        raise SecGemmaLeanV38SourceError("master.gz payload set must be an exact dictionary")
    expected = {role.key for role in quarter_roles(config)}
    if set(master_gzip_payloads) != expected:
        raise SecGemmaLeanV38SourceError("master.gz payload set is not exact")
    return tuple(
    ...
    for target in reconciliation.targets:
        ...
        if accession in current_complete_payloads:
            ...
            if (
                ...
                raise SecGemmaLeanV38SourceError("Carried source tuple is invalid")
...
def build_stage_source_seal(
    *,
    ...
    if config.stage == "development":
        if prior_bundle is not None:
            raise SecGemmaLeanV38SourceError("Development cannot carry a prior bundle")
...
def _validate_compact_evidence(
    ...
    *,
    ...
    if type(evidence) is not dict:
        raise SecGemmaLeanV38SourceError("Compact source evidence is not an object")
...
def _output_evidence(value: RoleParseOutput | PhaseOutput | Mapping[str, Any]) -> dict[str, Any]:
    ...
    raise SecGemmaLeanV38SourceError("Compact evidence input type is invalid")
...
def _submission_row_from_compact(value: Mapping[str, Any]) -> SubmissionRow:
    ...
    if type(value) is not dict or set(value) != expected:
        raise SecGemmaLeanV38SourceError("Compact Submissions row schema is not exact")
    if type(value["typed_values"]) is not dict:
        raise SecGemmaLeanV38SourceError("Compact Submissions typed values are invalid")
    try:
        row = SubmissionRow(
        ...
        raise SecGemmaLeanV38SourceError("Compact Submissions row types are invalid") from None
    if (
        ...
        raise SecGemmaLeanV38SourceError("Compact Submissions row is not authenticated")
...
def _historical_reference_from_compact(value: Mapping[str, Any]) -> HistoricalReference:
    if type(value) is not dict or set(value) != {
        ...
        raise SecGemmaLeanV38SourceError("Compact historical reference is invalid")
    ...
    if (
        ...
        raise SecGemmaLeanV38SourceError("Compact historical reference is malformed")
    ...
    if reference.filing_from > reference.filing_to:
        raise SecGemmaLeanV38SourceError("Compact historical reference is reversed")
    return reference
...
def _source_from_summary(summary: Mapping[str, Any]) -> SubmissionsSource:
    if type(summary) is not dict or set(summary) != {
        ...
        raise SecGemmaLeanV38SourceError("Compact Submissions source summary is invalid")
    ...
    if source.summary() != summary:
        raise SecGemmaLeanV38SourceError("Compact Submissions source summary drifted")
    return source
...
def _snapshot_from_compact_phase(evidence: Mapping[str, Any]) -> SubmissionsSnapshot:
    ...
    if snapshot.summary() != value["submissions_snapshot"]:
        raise SecGemmaLeanV38SourceError("Compact Submissions snapshot does not replay")
    return snapshot
...
def _validate_main_compact(evidence: Mapping[str, Any], config: StageSourceConfig) -> dict[str, Any]:
    ...
    if (
        ...
        raise SecGemmaLeanV38SourceError("Compact main evidence is invalid")
    references = [
        _historical_reference_from_compact(item) for item in value["references"]
    ]
    if (
        ...
        raise SecGemmaLeanV38SourceError("Compact main reference order drifted")
    ...
    if source.name != MAIN_SUBMISSIONS_NAME or source.raw_row_count != len(rows):
        raise SecGemmaLeanV38SourceError("Compact main source counts drifted")
    return value
    ...
    *,
    ...
    if config.stage == "development":
        if prior is not None:
            raise SecGemmaLeanV38SourceError("Development compact stage cannot carry prior")
        return None
    ...
    if prior.seal.get("stage") != expected:
        raise SecGemmaLeanV38SourceError("Compact prior stage is not adjacent")
    if validate:
...
def finalize_submissions(
    ...
    if isinstance(historical_evidence, (str, bytes)) or not isinstance(
        ...
        raise SecGemmaLeanV38SourceError("Historical compact evidence must be ordered")
    ...
    for raw in historical_evidence:
        ...
        if (
            ...
            raise SecGemmaLeanV38SourceError("Historical compact evidence is invalid")
...
def _master_target_from_compact(value: Mapping[str, Any]) -> MasterTargetRow:
    if type(value) is not dict or set(value) != {
        ...
        raise SecGemmaLeanV38SourceError("Compact master target schema is not exact")
    ...
    if (
        ...
        raise SecGemmaLeanV38SourceError("Compact master quarter identity is invalid")
    ...
    if target.to_dict() != value:
        raise SecGemmaLeanV38SourceError("Compact master target identity drifted")
    return target
...
def _master_evidence_from_compact(value: Mapping[str, Any]) -> MasterQuarterEvidence:
    ...
    if (
        ...
        raise SecGemmaLeanV38SourceError("Compact master role evidence is invalid")
    role = QuarterRole(
        year=compact["year"],
    ...
    if compact.get("role_id") != f"master/{role.year}/QTR{role.quarter}" or role.url != (
        ...
        raise SecGemmaLeanV38SourceError("Compact master role identity is not exact")
    ...
    if evidence.summary() != summary:
        raise SecGemmaLeanV38SourceError("Compact master summary does not authenticate")
    return evidence
...
def parse_master(
    ...
    if (year, quarter) not in allowed:
        raise SecGemmaLeanV38SourceError("Master compact role is outside frozen Q")
...
def _reconciled_target_from_compact(value: Mapping[str, Any]) -> ReconciledTarget:
    if type(value) is not dict or set(value) != {
        ...
        raise SecGemmaLeanV38SourceError("Compact reconciled target schema is invalid")
    body = {"submissions": value["submissions"], "master": value["master"]}
    if value["target_sha256"] != _canonical_sha256(body):
        raise SecGemmaLeanV38SourceError("Compact reconciled target hash is invalid")
    ...
    if (
        ...
        raise SecGemmaLeanV38SourceError("Compact target identities do not reconcile")
...
def _validate_compact_reconciliation(
    ...
    if (
        ...
        raise SecGemmaLeanV38SourceError("Compact reconciliation evidence is invalid")
    ...
    if order != sorted(order) or len(order) != len(set(order)):
        raise SecGemmaLeanV38SourceError("Compact reconciliation target order is invalid")
    ...
    if (
        ...
        raise SecGemmaLeanV38SourceError("Compact reconciliation P/carry is invalid")
    return value
...
def reconcile_masters(
    ...
    if phase.get("stage") != config.stage:
        raise SecGemmaLeanV38SourceError("Compact Submissions stage differs")
    ...
    if isinstance(ordered_master_evidence, (str, bytes)) or not isinstance(
        ...
        raise SecGemmaLeanV38SourceError("Compact master evidence must be ordered")
    ...
    for raw in ordered_master_evidence:
        ...
        if value.get("stage") != config.stage:
            raise SecGemmaLeanV38SourceError("Compact master stage differs")
        values.append(value)
    ...
    if [quarter.role.key for quarter in quarters] != [role.key for role in expected_roles]:
        raise SecGemmaLeanV38SourceError("Compact Q role set/order is not exact")
    master_by_accession: dict[str, MasterTargetRow] = {}
    ...
    if len(targets) > config.i_cap:
        raise SecGemmaLeanV38SourceError("Compact I cap is exceeded")
    prior_accessions = (
    ...
    if not set(prior_accessions) <= set(current_accessions):
        raise SecGemmaLeanV38SourceError("Compact current I omits prior target")
    if prior_seal is not None:
        prior_rows = {
    ...
    if len(p_accessions) > config.p_cap:
        raise SecGemmaLeanV38SourceError("Compact P cap is exceeded")
    ...
    if phase["pre_master_upper_bound"] != _pre_master_upper_bound_from_prior_seal(
        ...
        raise SecGemmaLeanV38SourceError("Compact pre-master bound did not replay")
    body = {
        "schema_version": COMPACT_RECONCILIATION_SCHEMA_VERSION,
        "stage": config.stage,
...
def parse_complete(
    ...
    if compact_target not in reconciliation["complete_targets"]:
        raise SecGemmaLeanV38SourceError("Complete compact target is not an exact P member")
...
def _validate_compact_complete(
    ...
    if (
        ...
        raise SecGemmaLeanV38SourceError("Compact complete evidence is invalid")
    target = next(
        (
            item
...
def _compact_reconciliation_objects(
    ...
    if (
        ...
        raise SecGemmaLeanV38SourceError("Compact reconciliation Q summary count differs")
...
def _finalize_stage_core(
    *,
    ...
    if phase.get("stage") != config.stage:
        raise SecGemmaLeanV38SourceError("Compact finalize Submissions stage differs")
    snapshot = _snapshot_from_compact_phase(phase)
    ...
    if phase["prior_stage_source_seal_sha256"] != (
        ...
        raise SecGemmaLeanV38SourceError("Compact phase prior authority differs")
    p_accessions = tuple(
    ...
    if isinstance(complete_evidence, (str, bytes)) or not isinstance(
        ...
        raise SecGemmaLeanV38SourceError("Compact complete evidence must be ordered")
...
def _validate_compact_stage_output(output: StageOutput) -> None:
    if not isinstance(output, StageOutput):
        raise SecGemmaLeanV38SourceError("Compact stage authority type is invalid")
...
def _opaque_receipt_hash(value: Any, *, location: str) -> str:
    if type(value) is not str or _BARE_SHA256_RE.fullmatch(value) is None:
        raise SecGemmaLeanV38SourceError(f"{location} opaque receipt hash is malformed")
    return value
...
def _compact_role_body_identity(
    ...
    if schema == COMPACT_ROLE_EVIDENCE_SCHEMA_VERSION:
        ...
        if role_type in {"submissions_main", "submissions_historical"}:
            ...
            raise SecGemmaLeanV38SourceError("Compact role type has no body identity")
...
def build_compact_role_manifest(
    *,
    ...
    if type(sequence) is not int or sequence < 0:
        raise SecGemmaLeanV38SourceError("Compact role sequence is invalid")
    if type(role_id) is not str or not role_id or type(url) is not str or not url:
        raise SecGemmaLeanV38SourceError("Compact role identity is invalid")
    if type(blob_name) is not str or _SAFE_BLOB_NAME_RE.fullmatch(blob_name) is None:
        raise SecGemmaLeanV38SourceError("Compact blob name is not a safe logical name")
    if type(payload) is not bytes:
        raise SecGemmaLeanV38SourceError("Compact role manifest requires exact bytes")
    if not isinstance(parse_output, RoleParseOutput):
        raise SecGemmaLeanV38SourceError("Compact role parse output type is invalid")
...
def _validate_compact_role_manifest(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if type(value) is not dict or set(value) != expected:
        raise SecGemmaLeanV38SourceError("Compact role manifest schema is not exact")
    ...
    if (
        ...
        raise SecGemmaLeanV38SourceError("Compact role manifest is malformed")
    for name in (
        "transport_receipt_sha256",
        "parse_receipt_sha256",
        "source_evidence_sha256",
...
def build_compact_checkpoint(
    *,
    ...
    if stage_output.seal["stage"] != config.stage:
        raise SecGemmaLeanV38SourceError("Compact checkpoint stage differs")
    if isinstance(role_manifests, (str, bytes)) or not isinstance(
        role_manifests, Sequence
    ):
        raise SecGemmaLeanV38SourceError("Compact role manifests must be ordered")
    ...
    if [item["sequence"] for item in manifests] != list(range(len(manifests))):
        raise SecGemmaLeanV38SourceError("Compact role manifest sequence is not exact")
    if not manifests or manifests[0]["role_id"] != "submissions/main":
        raise SecGemmaLeanV38SourceError("Compact checkpoint lacks main role first")
...
def _validate_compact_checkpoint(
    ...
    if type(checkpoint) is not dict or set(checkpoint) != expected:
        raise SecGemmaLeanV38SourceError("Compact checkpoint schema is not exact")
    ...
    if (
        ...
        raise SecGemmaLeanV38SourceError("Compact checkpoint hash/bindings are invalid")
    for name in (
        "expected_stage_source_seal_sha256",
        "submissions_source_evidence_sha256",
        "reconciliation_source_evidence_sha256",
...
def _load_compact_role(
    *,
    ...
    try:
        ...
        raise SecGemmaLeanV38SourceError("Compact blob loader failed safely") from None
    if type(payload) is not bytes:
        raise SecGemmaLeanV38SourceError("Compact blob loader did not return exact bytes")
    if (
...
def _detached_replay_compact_core(
    *,
    ...
    if not callable(load_blob):
        raise SecGemmaLeanV38SourceError("Compact blob loader is not callable")
    manifests = [_validate_compact_role_manifest(item) for item in role_manifests]
    if [item["sequence"] for item in manifests] != list(range(len(manifests))):
        raise SecGemmaLeanV38SourceError("Compact manifest sequence is not exact")
    role_ids = [item["role_id"] for item in manifests]
    if len(role_ids) != len(set(role_ids)):
        raise SecGemmaLeanV38SourceError("Compact role manifest is duplicated")
    checkpoint_value = _validate_compact_checkpoint(checkpoint, manifests, config)
    if config.stage == "development":
        if prior is not None:
            raise SecGemmaLeanV38SourceError("Development compact replay cannot carry prior")
        prior_output = None
    else:
        ...
        if prior.stage != expected_prior:
            raise SecGemmaLeanV38SourceError("Compact replay prior stage is not adjacent")
