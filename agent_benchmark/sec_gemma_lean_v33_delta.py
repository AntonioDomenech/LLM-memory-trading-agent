"""Committed-blob-only implementation delta proof for SEC/Gemma v3.3.
...
MANIFEST_VERSION = "aapl-sec-gemma-lean-v33-implementation-delta-v1"
PREREG_COMMIT = "d2c3f9e7eac704471c1ddab61943da364a188f77"
PREREG_TREE = "e6f5250bee6bb1837948ac7cc7b77112a841a94f"
PREREG_DOC_PATH = "docs/aapl_sec_gemma_lean_evidence_v3_3.md"
PREREG_DOC_BLOB = "a9e0ed7fa937e50abe18ce71b67e02be61d4db78"
PREREG_DOC_SHA256 = (
    "b883deefe42a1b31da9afe0529a418689d72bbcaff27d4340788b8f95bb52d2e"
)
IMPLEMENTATION_BASE_COMMIT = "c329a5de1099c3d7f21072eae7a12f3537f07121"
IMPLEMENTATION_BASE_TREE = "b084fade98637cfc0bfa1b0032ee771fc3fde6b4"
_COUNTERPART_MECHANICAL_REPLACEMENTS = (
    ("sec_gemma_lean_v32", "sec_gemma_lean_v33"),
    ("SecGemmaLeanV32", "SecGemmaLeanV33"),
    ("V32", "V33"),
    ("v32", "v33"),
    ("v3_2", "v3_3"),
    ("v3-2", "v3-3"),
...
class CounterpartRule:
    """Exact committed-v3.2 comparison contract for one added v3.3 file.

    ``expected_candidate_sha256`` binds the complete v3.3 blob for ordinary
    ...
    def validate(self, path: str) -> None:
        ...
        if modes[0]:
            ...
            if (
                path != "tests/test_sec_gemma_lean_v33_delta.py"
                or redacted_name != "V33_DELTA_MODULE_SHA256"
            ...
            path != "agent_benchmark/sec_gemma_lean_v33_delta.py"
            or anchor_path != "tests/test_sec_gemma_lean_v33_delta.py"
            or anchor_name != "V33_DELTA_MODULE_SHA256"
        ):
            raise DeltaValidationError(f"{path}: invalid external-anchor contract")
        expected_exclusions = {
            "agent_benchmark/sec_gemma_lean_v33_delta.py": (
                "assignment|$module._COUNTERPART_RULE_DATA|1",
            ),
            "tests/test_sec_gemma_lean_v33_delta.py": (
                "assignment|$module.V33_DELTA_MODULE_SHA256|1",
            ),
        }.get(path, ())
...
def _counterpart_evidence(
    ...
    *,
    ...
    """Prove one committed v3.3 blob against its pinned v3.2 counterpart."""
...
def build_delta_manifest(
    ...
    *,
    ...
    v33_paths = {entry.path for entry in implementation_diff}
    overlap = inherited_paths & v33_paths
    if overlap:
        raise DeltaValidationError(
            "inherited-at-prereg blobs changed in implementation: " + ",".join(sorted(overlap))
        )
    if v33_paths != set(allow_spec.path_rules):
        unknown = sorted(v33_paths - set(allow_spec.path_rules))
        missing = sorted(set(allow_spec.path_rules) - v33_paths)
    ...
    for entry in implementation_diff:
        ...
        entry_json["classification"] = "v33_implementation_delta"
    ...
    for entry in parent_to_implementation:
        ...
        elif entry.path in v33_paths:
            classification = "v33_implementation_delta"
        else:
    ...
    if {entry["path"] for entry in classified_parent_paths} != inherited_paths | v33_paths:
...
# This mechanically inherited table records the reviewed v3.2 verifier shape.
# The exact v3.3 projection below selects only its twelve additive paths; none
# of the older shared-file rules is reachable through DEFAULT_ALLOW_SPEC.
_V32_CLONE_PATH_RULES = {
    ...
    "agent_benchmark/sec_gemma_lean_v33_acquisition.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="f37be038c5a0f0a4edd1248b5e51ff781478585a5c67d68579bde89485b6772f",
        expected_symbol_count=216,
        expected_import_inventory_sha256="5fceadd328b0c487fd0504a6e8d4c4b1383c682f99b1fa121421b84128e23220",
        expected_import_count=29,
    ),
    "agent_benchmark/sec_gemma_lean_v33_delta.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="da001b6a8a863811ee2b26140f9eb34522ccc4a1cfd894b6ee17ce67a3058979",
    ...
    "agent_benchmark/sec_gemma_lean_v33_journal.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="6ec29f203f98eabc11055f38e1ccccc23826f7e72c50be08d5bb3ce6398c0eb7",
    ...
    "agent_benchmark/sec_gemma_lean_v33_preflight.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="15a061fbef2842f6149a9aebb0f0118b02720acb77beb7e9b8e38529649344b4",
        expected_symbol_count=144,
        expected_import_inventory_sha256="5acdf49b0b764138326d1de4fd5eeb300deb8240b68bb5a5aeb7b947cbe94486",
        expected_import_count=25,
    ),
    "agent_benchmark/sec_gemma_lean_v33_source.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="e30659288c92e4edcfecb6532a6ca2b691c9eb02a9dc1c678e05133379e14fb3",
    ...
    "agent_benchmark/sec_gemma_lean_v33_transport.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="69bff8110eb7fc7f62ffe4b37c04aa1597a8c8d537e486c9605344bc07b7f4fe",
    ...
    "tests/test_sec_gemma_lean_v33_acquisition.py": PathRule(
        "A", (), (),
        ...
        expected_import_inventory_sha256="04a5536d6a688ec3add378a4b4fa1256d6569d1cf513ea7bde243afc520b3d0a",
        expected_import_count=21,
    ),
    "tests/test_sec_gemma_lean_v33_delta.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="5741fb5dd5acaaed99bf46049414ef9b2413680c8f26e6928626df8ad4372ae5",
        expected_symbol_count=49,
        expected_import_inventory_sha256="28fe9e731983e6e711a6c1202f234ef411509b1c8d64dbfce97f7f6f3b7db95a",
        expected_import_count=10,
    ),
    "tests/test_sec_gemma_lean_v33_journal.py": PathRule(
        "A", (), (),
        ...
        expected_import_inventory_sha256="eacdfdba798debc237a799cd3044db85b8d71e11cede35d2d13b72baca9d22a9",
        expected_import_count=9,
    ),
    "tests/test_sec_gemma_lean_v33_preflight.py": PathRule(
        "A", (), (),
        ...
        expected_import_inventory_sha256="974a7d7bd74cc440cb1aa1b295f7ae68df1f6c55dedf1396256b849142e59be4",
        expected_import_count=11,
    ),
    "tests/test_sec_gemma_lean_v33_source.py": PathRule(
        "A", (), (),
        expected_symbol_inventory_sha256="5a952b157f90cd3978e0d0b670e525b94846fd21057dbd164222aacd1ea91ac5",
        expected_symbol_count=73,
        expected_import_inventory_sha256="077bfd2416c290c79607bb16787eb63a77c3cd9dda2078c624cb1ca2334c1734",
        expected_import_count=12,
    ),
    "tests/test_sec_gemma_lean_v33_transport.py": PathRule(
        "A", (), (),
        ...
        expected_import_inventory_sha256="83ebd95cde503a470d10f07c50cfd858abbe56f9e770a37c91ea6327382b171e",
        expected_import_count=16,
    ),
}

# v3.3 is additive by construction: the implementation may add only these
# twelve files.  The older reviewed rules remain above solely as inert audit
# history from the v3.2 verifier clone; this exact projection is the authority
# consumed by DEFAULT_ALLOW_SPEC.
_V33_IMPLEMENTATION_PATHS = (
    "agent_benchmark/sec_gemma_lean_v33_acquisition.py",
    "agent_benchmark/sec_gemma_lean_v33_delta.py",
    "agent_benchmark/sec_gemma_lean_v33_journal.py",
    "agent_benchmark/sec_gemma_lean_v33_preflight.py",
    "agent_benchmark/sec_gemma_lean_v33_source.py",
    "agent_benchmark/sec_gemma_lean_v33_transport.py",
    "tests/test_sec_gemma_lean_v33_acquisition.py",
    "tests/test_sec_gemma_lean_v33_delta.py",
    "tests/test_sec_gemma_lean_v33_journal.py",
    "tests/test_sec_gemma_lean_v33_preflight.py",
    "tests/test_sec_gemma_lean_v33_source.py",
    "tests/test_sec_gemma_lean_v33_transport.py",
)
...
_COUNTERPART_RULE_DATA = {
    'agent_benchmark/sec_gemma_lean_v33_acquisition.py': {'counterpart_path': 'agent_benchmark/sec_gemma_lean_v32_acquisition.py',
     'expected_mechanical_counts': (12, 271, 0, 1, 2, 7, 2),
     'expected_candidate_sha256': '87762f17083fc67d1028a54854a0ba2f0527c7d63196912a1a9ea4777c7bc580',
    ...
    'agent_benchmark/sec_gemma_lean_v33_delta.py': {'counterpart_path': 'agent_benchmark/sec_gemma_lean_v32_delta.py',
     'expected_mechanical_counts': (50, 6, 12, 11, 2, 1, 7),
     'expected_candidate_sha256': None,
     'redacted_literal_assignment': None,
     'expected_redacted_candidate_sha256': None,
     'external_sha256_anchor_path': 'tests/test_sec_gemma_lean_v33_delta.py',
     'external_sha256_anchor_assignment': 'V33_DELTA_MODULE_SHA256',
     ...
     'expected_changed_import_specs': (),
     'expected_changed_symbol_evidence_sha256': 'bb869f295a781f359000e29a6f2771ba615a9ba20265a5f8ea0190697fd0c1c0'},
    'agent_benchmark/sec_gemma_lean_v33_journal.py': {'counterpart_path': 'agent_benchmark/sec_gemma_lean_v32_journal.py',
     'expected_mechanical_counts': (0, 105, 0, 0, 0, 3, 1),
     'expected_candidate_sha256': 'b5efca60b7da63a8458310b30673ce8686be8bea2c4e123e395c0d6986a0edf6',
     'redacted_literal_assignment': None,
     'expected_redacted_candidate_sha256': None,
     'external_sha256_anchor_path': None,
     'external_sha256_anchor_assignment': None,
     ...
     'expected_changed_import_specs': (),
     'expected_changed_symbol_evidence_sha256': '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945'},
    'agent_benchmark/sec_gemma_lean_v33_preflight.py': {'counterpart_path': 'agent_benchmark/sec_gemma_lean_v32_preflight.py',
     'expected_mechanical_counts': (4, 142, 0, 0, 2, 12, 3),
     'expected_candidate_sha256': 'ed6cd0a7e8df7224b0e01eb71100bf5fc205ae1ba27c1a31ce849821b24d3e89',
     'redacted_literal_assignment': None,
     'expected_redacted_candidate_sha256': None,
     'external_sha256_anchor_path': None,
     'external_sha256_anchor_assignment': None,
     ...
     'expected_changed_import_specs': (),
     'expected_changed_symbol_evidence_sha256': '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945'},
    'agent_benchmark/sec_gemma_lean_v33_source.py': {'counterpart_path': 'agent_benchmark/sec_gemma_lean_v32_source.py',
     'expected_mechanical_counts': (0, 316, 0, 15, 0, 0, 9),
     'expected_candidate_sha256': 'f4949c6166ed3b2644b8b34f8157102159461f02b96098cbfaf938dc7c1694df',
     'redacted_literal_assignment': None,
     'expected_redacted_candidate_sha256': None,
     'external_sha256_anchor_path': None,
     'external_sha256_anchor_assignment': None,
     'excluded_change_symbol_ids': (),
     'expected_changed_symbol_ids': ('function|parse_strict_master_gzip|1',),
     'expected_changed_import_specs': (),
     'expected_changed_symbol_evidence_sha256': 'b89ca1094082484d468bb7b25e95f35be44ba2e47a807b95a56f8b9c98aea569'},
    'agent_benchmark/sec_gemma_lean_v33_transport.py': {'counterpart_path': 'agent_benchmark/sec_gemma_lean_v32_transport.py',
     'expected_mechanical_counts': (5, 140, 0, 0, 0, 2, 3),
     'expected_candidate_sha256': '61b57b7fbc90b78990efae5a8bea85dbc4e1573f61428cbe3bf9a4cd92aed2d2',
     'redacted_literal_assignment': None,
     'expected_redacted_candidate_sha256': None,
     'external_sha256_anchor_path': None,
     'external_sha256_anchor_assignment': None,
     ...
     'expected_changed_import_specs': (),
     'expected_changed_symbol_evidence_sha256': '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945'},
    'tests/test_sec_gemma_lean_v33_acquisition.py': {'counterpart_path': 'tests/test_sec_gemma_lean_v32_acquisition.py',
     'expected_mechanical_counts': (11, 38, 0, 0, 1, 1, 0),
     'expected_candidate_sha256': '1150fa00946d9a3080f97d5e26cb947c0d5786b8517a5a80b3c00307a93a42a9',
     'redacted_literal_assignment': None,
     'expected_redacted_candidate_sha256': None,
     'external_sha256_anchor_path': None,
     'external_sha256_anchor_assignment': None,
     ...
     'expected_changed_import_specs': (),
     'expected_changed_symbol_evidence_sha256': '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945'},
    'tests/test_sec_gemma_lean_v33_delta.py': {'counterpart_path': 'tests/test_sec_gemma_lean_v32_delta.py',
     'expected_mechanical_counts': (15, 0, 8, 1, 1, 0, 0),
     'expected_candidate_sha256': None,
     'redacted_literal_assignment': 'V33_DELTA_MODULE_SHA256',
     'expected_redacted_candidate_sha256': '296258a42fb9c91b86a1d62032334f25162f3be4913f8d7989376dec68f21cec',
     'external_sha256_anchor_path': None,
     'external_sha256_anchor_assignment': None,
     'excluded_change_symbol_ids': ('assignment|$module.V33_DELTA_MODULE_SHA256|1',),
     'expected_changed_symbol_ids': ('assignment|$module.EXPECTED_V33_COUNTERPARTS|1',
                                     ...
                                     'function|test_checker_rejects_blank_company_scope_expansion_row_drop_and_special_case|1',
     ...
     'expected_changed_import_specs': (),
     'expected_changed_symbol_evidence_sha256': 'a669ec2a9da84baddf5ad3cfb42850d501876bcb42cdfa7c737cc08601085732'},
    'tests/test_sec_gemma_lean_v33_journal.py': {'counterpart_path': 'tests/test_sec_gemma_lean_v32_journal.py',
     'expected_mechanical_counts': (2, 43, 0, 0, 0, 0, 0),
     'expected_candidate_sha256': '95bf15aa732863ccf5e0ae904c54098356c51db5c191995228cf294c3f0f3636',
     'redacted_literal_assignment': None,
     'expected_redacted_candidate_sha256': None,
     'external_sha256_anchor_path': None,
     'external_sha256_anchor_assignment': None,
     ...
     'expected_changed_import_specs': (),
     'expected_changed_symbol_evidence_sha256': '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945'},
    'tests/test_sec_gemma_lean_v33_preflight.py': {'counterpart_path': 'tests/test_sec_gemma_lean_v32_preflight.py',
     'expected_mechanical_counts': (2, 28, 0, 0, 0, 2, 0),
     'expected_candidate_sha256': '1fdcf246e24734fa99882dbe5cce9ca3bfc7ed020fb3d30e859d4be98df62ee9',
     'redacted_literal_assignment': None,
     'expected_redacted_candidate_sha256': None,
     'external_sha256_anchor_path': None,
     'external_sha256_anchor_assignment': None,
     ...
     'expected_changed_import_specs': (),
     'expected_changed_symbol_evidence_sha256': '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945'},
    'tests/test_sec_gemma_lean_v33_source.py': {'counterpart_path': 'tests/test_sec_gemma_lean_v32_source.py',
     'expected_mechanical_counts': (2, 47, 0, 0, 0, 0, 1),
     'expected_candidate_sha256': 'ffdf317d808becbf1e17d93a1db72a87528f343cbc963d23d037420db171b0ed',
     'redacted_literal_assignment': None,
     'expected_redacted_candidate_sha256': None,
     'external_sha256_anchor_path': None,
     'external_sha256_anchor_assignment': None,
     'excluded_change_symbol_ids': (),
     'expected_changed_symbol_ids': ('function|test_blank_company_apple_targets_and_other_empty_fields_reject|1',
                                     'function|test_blank_company_non_targets_are_retained_by_general_target_predicate|1',
                                     'function|test_full_synthetic_source_seal_replays_blank_company_non_target|1',
                                     'function|test_parse_master_blank_company_rule_preserves_compact_evidence_without_predecessor_reads|1'),
     'expected_changed_import_specs': (),
     'expected_changed_symbol_evidence_sha256': '03da693fe975772dccb0ea908de91ebae3169a08818f711fb8eff43326c2705b'},
    'tests/test_sec_gemma_lean_v33_transport.py': {'counterpart_path': 'tests/test_sec_gemma_lean_v32_transport.py',
     'expected_mechanical_counts': (1, 15, 0, 0, 0, 0, 0),
     'expected_candidate_sha256': 'f9e6cfe7bad8a17ea498f64823b4adb2b7294123915f5a86f5fb4b8bfe65d176',
     'redacted_literal_assignment': None,
     'expected_redacted_candidate_sha256': None,
     'external_sha256_anchor_path': None,
     'external_sha256_anchor_assignment': None,
...
_DEFAULT_COUNTERPART_RULES = {
    ...
    for path in _V33_IMPLEMENTATION_PATHS
}
_DEFAULT_EXISTING_PATH_RULES = {
    path: _V32_CLONE_PATH_RULES[path] for path in _V33_IMPLEMENTATION_PATHS
}
