"""Dynamic, append-only SEC request journal for lean evidence v3.6.
...
JOURNAL_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-evidence-v3-6-dynamic-request-journal-v1"
)
SEALED_PREFIX_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-evidence-v3-6-sealed-prefix-v1"
)
PHASE_PLAN_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-evidence-v3-6-phase-plan-v1"
)
...
class SecGemmaLeanV36JournalError(RuntimeError):
...
def _canonical_json_bytes(value: Any) -> bytes:
    try:
        ...
        raise SecGemmaLeanV36JournalError("journal_value_invalid") from None
...
def _master_quarters(config: StageConfig) -> tuple[tuple[int, int], ...]:
    ...
    if len(result) != config.quarterly_master_count:
        raise SecGemmaLeanV36JournalError("frozen_quarter_formula_invalid")
    return tuple(result)


def _historical_roles(names: Sequence[str], start: int) -> tuple[RequestRole, ...]:
    if type(names) not in {list, tuple}:
        raise SecGemmaLeanV36JournalError("historical_plan_invalid")
    values = list(names)
    if (
...
def _complete_roles(
    ...
    if type(targets) not in {list, tuple}:
        raise SecGemmaLeanV36JournalError("complete_target_plan_invalid")
    config = STAGE_CONFIGS[stage]
    ...
    for target in targets:
        ...
        try:
            ...
            raise SecGemmaLeanV36JournalError("complete_target_plan_invalid") from None
        if (
    ...
    if (
        ...
        raise SecGemmaLeanV36JournalError(
...
def build_stage_role_plan(
    ...
    if type(stage) is not str or stage not in STAGE_CONFIGS:
        raise SecGemmaLeanV36JournalError("stage_invalid")
...
def validate_stage_role_plan(plan: StageRolePlan) -> None:
    if (
        ...
        raise SecGemmaLeanV36JournalError("role_plan_invalid")
...
class PrivateSecContact:
    ...
    def contains_echo(self, value: bytes | bytearray | memoryview | str) -> bool:
        if type(value) is str:
            ...
            raise SecGemmaLeanV36JournalError("privacy_scan_value_invalid")
        ...
        while pending:
            ...
            if type(current) in {str, bytes, bytearray, memoryview}:
                if self.contains_echo(current):
                    raise SecGemmaLeanV36JournalError("privacy_echo")
...
def _secure_root(root: Path) -> Path:
    if not isinstance(root, Path) or not root.is_absolute():
        raise SecGemmaLeanV36JournalError("journal_root_invalid")
    ...
    for component in (None, *root.parts[1:]):
        ...
        try:
            ...
            raise SecGemmaLeanV36JournalError("journal_root_invalid") from None
...
def _regular_file(path: Path) -> os.stat_result:
    try:
        ...
        raise SecGemmaLeanV36JournalError("journal_invalid") from None
    if (
        ...
        raise SecGemmaLeanV36JournalError("journal_invalid")
    return details
...
def _fsync_directory(directory: Path) -> None:
    ...
    if os.name == "nt":
        try:
            ...
            raise SecGemmaLeanV36JournalError("journal_directory_fsync_failed") from None
...
def _write_exclusive_fsynced(path: Path, payload: bytes) -> None:
    ...
    except SecGemmaLeanV36JournalError:
        raise
    except OSError:
        raise SecGemmaLeanV36JournalError(
    ...
    if transaction_markers:
        for marker in transaction_markers:
            ...
            if not 2 <= details.st_size <= _MAX_EVENT_BYTES:
                raise SecGemmaLeanV36JournalError("journal_append_incomplete")
        raise SecGemmaLeanV36JournalError("journal_append_incomplete")
...
def _replay_events(
    ...
    if not _valid_sha256(contact_fingerprint_sha256):
        raise SecGemmaLeanV36JournalError("contact_fingerprint_invalid")
    config = STAGE_CONFIGS[stage]
...
class AcquisitionJournal:
    ...
    def _append(self, event_type: str, payload: Mapping[str, Any]) -> dict[str, Any]:
        if event_type not in EVENT_TYPES or type(payload) is not dict:
            raise SecGemmaLeanV36JournalError("journal_event_invalid")
        ...
        if (
            ...
            raise SecGemmaLeanV36JournalError("journal_concurrent_change")
        _replay_events(
    ...
    def _require_phase_boundary(self, phase: str) -> None:
        ...
        if self._state.derived_terminal_code is not None:
            if not (
                ...
                raise SecGemmaLeanV36JournalError(self._state.derived_terminal_code)
        if self._state.open_role_id is not None or self._state.next_role_id is not None:
            raise SecGemmaLeanV36JournalError("phase_boundary_not_reached")
        if self._state.next_required_expansion != phase:
            raise SecGemmaLeanV36JournalError("phase_expansion_order_invalid")
    ...
    def _append_phase_plan(
        ...
        if not _valid_sha256(basis_receipt_sha256):
            raise SecGemmaLeanV36JournalError("phase_basis_receipt_invalid")
        payload = {
    ...
    def append_historical_phase(self, historical_filenames: Sequence[str]) -> str:
        ...
        if self._state.last_parse_receipt_sha256 is None:
            raise SecGemmaLeanV36JournalError("main_parse_receipt_missing")
    ...
    def record_role_intent(self, role_id: str, *, dispatch_wait_ms: int) -> str:
        ...
        if self._state.failure_code is not None or self._state.open_role_id is not None:
            raise SecGemmaLeanV36JournalError("role_intent_forbidden")
        if self._state.next_role_id != role_id:
            raise SecGemmaLeanV36JournalError("role_order_invalid")
        ...
        if type(dispatch_wait_ms) is not int or dispatch_wait_ms < minimum:
            raise SecGemmaLeanV36JournalError("dispatch_gap_too_short")
        return self._append(
            "role_intent",
            {
                "invocation_number": invocation,
    ...
    def record_response_complete(
        ...
        *,
        ...
        if self._state.open_role_id != role_id:
            raise SecGemmaLeanV36JournalError("response_role_invalid")
        role = self._state.planned_roles[self._state.role_seals]
        if type(body_bytes) is int and body_bytes == role.reservation_bytes:
            raise SecGemmaLeanV36JournalError("body_limit_exceeded")
        return self._append(
            "response_complete",
            {
                "invocation_number": invocation,
    ...
    def record_role_seal(
        ...
        *,
        ...
        invocation = self._require_owned_invocation()
        if self._state.open_role_id != role_id:
            raise SecGemmaLeanV36JournalError("seal_role_invalid")
        return self._append(
            "role_seal",
            {
                "invocation_number": invocation,
    ...
    def record_role_failure(
        ...
        *,
        ...
        if type(error_code) is not str or error_code not in ROLE_FAILURE_CODES:
            raise SecGemmaLeanV36JournalError("unsafe_error_code")
        if self._state.open_role_id != role_id:
            raise SecGemmaLeanV36JournalError("failure_role_invalid")
        return self._append(
            "role_failure",
            {
                "invocation_number": invocation,
    ...
    def close_invocation(self, *, invocation_duration_ms: int) -> str:
        ...
        if self._state.open_role_id is not None:
            raise SecGemmaLeanV36JournalError("open_intent")
        if not _nonnegative_int(invocation_duration_ms):
            raise SecGemmaLeanV36JournalError("invocation_duration_invalid")
        event = self._append(
            "invocation_clean_close",
            {
                "invocation_number": invocation,
    ...
    def seal_terminal_rejection(
        ...
        if self._state.derived_terminal_code is not None and selected != self._state.derived_terminal_code:
            raise SecGemmaLeanV36JournalError("terminal_code_mismatch")
        if self._state.active_invocation is None:
            if unclean_active_ms is not None and (
                ...
                raise SecGemmaLeanV36JournalError("unclean_duration_invalid")
            unclean = 0
        else:
            if not _nonnegative_int(unclean_active_ms):
                raise SecGemmaLeanV36JournalError("unclean_duration_required")
