"""Canonical append-only journal used by the v3.16 science attempt.
...
state-machine rules live in :mod:`sec_gemma_lean_science_v316_store`.
"""
...
JOURNAL_SCHEMA_VERSION: Final[str] = (
    "aapl-sec-gemma-lean-science-v3-16-journal-v1"
)
...
class V316JournalError(RuntimeError):
...
def _validate_json_value(value: Any) -> None:
    ...
    if value is None or type(value) in {bool, int, str}:
        if type(value) is str:
            try:
                ...
                raise V316JournalError("canonical_json_invalid") from None
        return
    ...
    if type(value) is float:
        raise V316JournalError("canonical_json_float_forbidden")
    ...
    if isinstance(value, Mapping):
        for key, item in value.items():
            if type(key) is not str:
                raise V316JournalError("canonical_json_invalid")
...
def canonical_json_bytes(value: Any) -> bytes:
    ...
    V3.16 removed the historical trailing newline from every canonical JSON
...
def sha256_bytes(value: bytes) -> str:
    if type(value) is not bytes:
        raise V316JournalError("bytes_invalid")
...
def secure_directory(path: Path) -> Path:
    ...
    if not isinstance(path, Path) or not path.is_absolute():
        raise V316JournalError("journal_directory_invalid")
    ...
    for component in path.parts[1:]:
        ...
        try:
            ...
            raise V316JournalError("journal_directory_invalid") from None
        if (
...
def regular_file(path: Path, *, code: str = "journal_invalid") -> os.stat_result:
    try:
        ...
        raise V316JournalError(code) from None
    if (
...
def fsync_directory(directory: Path) -> None:
    ...
    if os.name == "nt":
        try:
            ...
            raise V316JournalError("journal_directory_fsync_failed") from None
...
def _write_exclusive_fsynced(path: Path, payload: bytes) -> None:
    ...
    try:
        ...
        raise V316JournalError("journal_append_durability_failed") from None
    finally:
        if descriptor is not None:
            os.close(descriptor)
...
def _decode_event(path: Path, *, expected_authority_sha256: str) -> JournalEvent:
    ...
    if details.st_size <= 0 or details.st_size > MAX_EVENT_BYTES:
        raise V316JournalError("journal_invalid")
    try:
    ...
    if (
        ...
        raise V316JournalError("journal_hash_mismatch")
    return JournalEvent(
        value["sequence"],
        value["event_type"],
...
def replay_journal(root: Path, *, authority_sha256: str) -> JournalReplay:
    ...
    if not valid_sha256(authority_sha256):
        raise V316JournalError("authority_sha256_invalid")
    try:
    ...
    for entry in entries:
        if _PENDING_NAME_RE.fullmatch(entry.name) or _FAILED_NAME_RE.fullmatch(
            ...
            raise V316JournalError("journal_append_incomplete")
        match = _EVENT_NAME_RE.fullmatch(entry.name)
        if match is None:
            raise V316JournalError("journal_extra_entry")
    ...
    if [sequence for sequence, _path in parsed_names] != list(
        ...
        raise V316JournalError("journal_sequence_invalid")
    ...
    for sequence, path in parsed_names:
        ...
        if event.sequence != sequence or event.previous_event_sha256 != previous:
            raise V316JournalError("journal_chain_invalid")
        events.append(event)
...
class HashChainJournal:
    ...
    def append(self, event_type: str, payload: Mapping[str, Any]) -> JournalEvent:
        if (
            ...
            raise V316JournalError("journal_event_invalid")
        ...
        if len(encoded) > MAX_EVENT_BYTES:
            raise V316JournalError("journal_event_too_large")
        ...
        if entry_exists_nofollow(final) or entry_exists_nofollow(pending):
            raise V316JournalError("journal_destination_exists")
