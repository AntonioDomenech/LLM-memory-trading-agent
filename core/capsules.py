...
def capsule_path(symbol: str, start: str, end: str):
    ...
    p = os.path.join("data", f"capsules_{key}.jsonl"); os.makedirs(os.path.dirname(p), exist_ok=True); return p
def build_capsule(date_iso: str, symbol: str, price_row: dict, topk: List[Dict], macro: List[Dict], regime: Dict) -> Dict:
    headlines = [{"t": _canon(a.get("title",""))[:160], "s": a.get("source","")[:40]} for a in topk if a.get("title")]
    macro_h = [{"t": _canon(a.get("title",""))[:160], "s": a.get("source","")[:40]} for a in macro if a.get("title")]
    cap = {
        ...
        "regime": regime,
        "headlines": headlines[:5], "macro": macro_h[:3],
    }
