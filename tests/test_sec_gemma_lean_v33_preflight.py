...
def test_offline_identity_probe_is_exactly_tags_then_show_and_local_version() -> None:
    ...
    with pytest.raises(preflight.SecGemmaLeanV33PreflightError, match="forbidden"):
...
def test_production_trust_cannot_be_combined_with_injected_transport() -> None:
    ...
    with pytest.raises(preflight.SecGemmaLeanV33PreflightError, match="transport"):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("POST", "/api/chat", b"{}"),
        ("POST", "/api/generate", b"{}"),
        ("GET", "https://www.sec.gov/submissions/CIK0000320193.json", b""),
        ("GET", "https://query1.finance.yahoo.com/v8/finance/chart/AAPL", b""),
        ("POST", "/market", b"{}"),
        ("POST", "/performance", b"{}"),
        ("POST", "/broker", b"{}"),
        ("POST", "/trade", b"{}"),
...
def test_restricted_transport_never_delegates_forbidden_paths(
    position: int,
    method: str,
    path: str,
    body: bytes,
) -> None:
    delegate = _Delegate([b"{}", b"{}"])
    transport = preflight._RestrictedIdentityTransport(delegate)
    if position == 1:
        transport.request("GET", "/api/tags", body=b"")
    before = list(delegate.calls)

    with pytest.raises(preflight.SecGemmaLeanV33PreflightError, match="forbidden"):
        transport.request(method, path, body=body)

    assert delegate.calls == before
...
def test_tags_identity_is_unique_and_exactly_digest_pinned(payload: bytes) -> None:
    with pytest.raises(preflight.SecGemmaLeanV33PreflightError, match="tags"):
...
def test_durable_intent_makes_crash_after_first_call_nonretryable(tmp_path: Path) -> None:
    ...
    def crash_after_first() -> dict[str, Any]:
        ...
        raise preflight.SecGemmaLeanV33PreflightError("runtime_identity_invalid")

    repository = {"head_commit": IMPLEMENTATION_COMMIT, "head_tree": IMPLEMENTATION_TREE}
    with pytest.raises(preflight.SecGemmaLeanV33PreflightError, match="runtime_identity"):
    ...
    with pytest.raises(preflight.SecGemmaLeanV33PreflightError, match="already_attempted"):
        preflight._durable_identity_probe(
            checkpoint,
            repository,
            contact_fingerprint=CONTACT_SHA256,
...
@pytest.mark.parametrize(
    ...
    [
        ("branch", "wrong_branch"),
        ("status", "working_tree_not_clean"),
        ("origin", "origin_changed"),
        ("upstream_ref", "upstream_changed"),
        ("upstream_head", "head_not_at_cached_upstream"),
        ("doc_blob", "preregistration_changed"),
    ],
)
def test_repository_gate_rejects_wrong_dirty_unpushed_or_changed_state(
    tmp_path: Path,
    mutation: str,
    expected_code: str,
) -> None:
    root = _repo(tmp_path)
    values: dict[tuple[str, ...], bytes] = {
        ...
        ("rev-parse", "HEAD^{tree}"): (IMPLEMENTATION_TREE + "\n").encode(),
        (
            "rev-parse",
            "--abbrev-ref",
            "--symbolic-full-name",
        ...
        ("rev-parse", "@{upstream}"): (IMPLEMENTATION_COMMIT + "\n").encode(),
        (
            "rev-parse",
            f"HEAD:{preflight.delta.PREREG_DOC_PATH}",
        ): (preflight.delta.PREREG_DOC_BLOB + "\n").encode(),
    }
    replacements = {
        "branch": (("branch", "--show-current"), b"codex/wrong-branch\n"),
        "status": (
        ...
        "origin": (
            ...
            b"https://example.invalid/wrong.git\n",
        ),
        "upstream_ref": (
            ("rev-parse", "--abbrev-ref", "--symbolic-full-name", "@{upstream}"),
            b"origin/codex/wrong-branch\n",
        ),
        "upstream_head": (("rev-parse", "@{upstream}"), ("a" * 40 + "\n").encode()),
        "doc_blob": (
            ("rev-parse", f"HEAD:{preflight.delta.PREREG_DOC_PATH}"),
            ("b" * 40 + "\n").encode(),
        ),
    }
    ...
    def git_runner(_: Path, arguments: tuple[str, ...]) -> preflight._GitResult:
        ...
        return preflight._GitResult(0, values[arguments])

    with pytest.raises(preflight.SecGemmaLeanV33PreflightError, match=expected_code):
        preflight._verify_repository(
            root,
            git_runner=git_runner,
            delta_builder=lambda *_args, **_kwargs: {},
            delta_validator=lambda *_args, **_kwargs: {},
            source_verifier=lambda _root, _head: {},
        )
...
def _production_markers(
    ...
    nonce = "a" * 64
    intent_body = {
        "schema_version": "aapl-sec-gemma-lean-evidence-v3-3-identity-intent-v1",
    ...
    completion_body = {
        "schema_version": "aapl-sec-gemma-lean-evidence-v3-3-identity-complete-v1",
        "test_injections_used": False,
        "attempt_nonce_sha256": nonce,
...
def test_minimal_self_rehashed_forged_receipt_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(preflight.SecGemmaLeanV33PreflightError):
...
def test_extra_durable_field_and_noncanonical_artifact_bytes_reject(tmp_path: Path) -> None:
    ...
    _rehash(report)
    with pytest.raises(preflight.SecGemmaLeanV33PreflightError, match="untrusted"):
        preflight._validate_sealable_report(report)

    root = _repo(tmp_path / "encoding")
    report, manifest = _valid_report(root)
    _write_artifact(root, report, canonical=False)
    with pytest.raises(preflight.SecGemmaLeanV33PreflightError, match="committed_preflight"):
        preflight._validate_committed_preflight_for_acquisition(
            root, **_validator_kwargs(root, report, manifest)
        )
...
def test_altered_marker_delta_source_and_contact_each_reject(tmp_path: Path) -> None:
    ...
    with pytest.raises(preflight.SecGemmaLeanV33PreflightError, match="identity_probe"):
        preflight._validate_committed_preflight_for_acquisition(
            root, **_validator_kwargs(root, report, manifest)
        )
    ...
    _rehash(report)
    _write_artifact(root, report)
    with pytest.raises(preflight.SecGemmaLeanV33PreflightError, match="implementation_delta"):
        preflight._validate_committed_preflight_for_acquisition(
            root, **_validator_kwargs(root, report, manifest)
        )
    ...
    with pytest.raises(preflight.SecGemmaLeanV33PreflightError, match="executed_source"):
        preflight._validate_committed_preflight_for_acquisition(
            root,
...
def test_prereg_ancestor_and_final_git_contact_marker_rebounds_reject(tmp_path: Path) -> None:
    ...
    with pytest.raises(preflight.SecGemmaLeanV33PreflightError, match="preregistration"):
    ...
    with pytest.raises(preflight.SecGemmaLeanV33PreflightError, match="changed"):
        preflight._validate_committed_preflight_for_acquisition(root, **kwargs)
...
def test_publish_failure_leaves_orphan_unusable_and_publish_is_after_lease(
    ...
    with pytest.raises(preflight.SecGemmaLeanV33PreflightError, match="artifact_unwritable"):
    ...
    with pytest.raises(preflight.SecGemmaLeanV33PreflightError, match="publish_state"):
...
def test_test_markers_publish_marker_and_active_lock_cannot_masquerade(
    ...
    with pytest.raises(preflight.SecGemmaLeanV33PreflightError, match="active_run"):
        preflight._validate_committed_preflight_for_acquisition(
            root, **_validator_kwargs(root, report, manifest)
        )
...
def test_private_contact_is_untracked_ignored_and_only_fingerprint_escapes(
    ...
    with pytest.raises(preflight.SecGemmaLeanV33PreflightError, match="tracked"):
    ...
    with pytest.raises(preflight.SecGemmaLeanV33PreflightError, match="not_ignored"):
