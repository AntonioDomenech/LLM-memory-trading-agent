...
def test_canonical_json_and_hash_helpers_are_exact_and_redacted() -> None:
    ...
    _assert_code(
        "v315_contract_json_nonfinite", contract.canonical_json_bytes, {"x": float("nan")}
    )
    _assert_code(
        "v315_contract_json_key", contract.canonical_json_bytes, {1: "not allowed"}
    )
    _assert_code(
        "v315_contract_json_type", contract.canonical_json_bytes, {"x": object()}
    )
    _assert_code("v315_contract_hash_input", contract.sha256_bytes, "bytes required")
...
def test_generic_self_hash_is_strict_and_detached() -> None:
    ...
    _assert_code("v315_contract_self_hash_mismatch", contract.validate_self_sha256, changed)
    _assert_code("v315_contract_self_hash_present", contract.add_self_sha256, sealed)
...
def test_frozen_science_projection_is_exact_38320_bytes() -> None:
    ...
    _assert_code(
        "v315_contract_science_projection_hash",
    ...
    missing.pop("ledger")
    _assert_code(
        "v315_contract_science_projection_keys",
        contract.verify_frozen_science_projection,
        missing,
    )
...
def test_three_git_authority_families_are_exact_and_disjoint() -> None:
    assert (contract.BASE_COMMIT, contract.BASE_TREE, contract.BASE_PARENT) == (
        "3e813577f1aab4ae80d0f5f59362a46320c818fb",
        "90d4e6681acb81fc11f5abf9860df331923a907d",
    ...
    predecessor = contract.build_v314_rejection_pins()
    assert predecessor["preregistration"] == {
        "commit": "9f9598e1100e897f57dbbb2df95adeb184e6ca08",
        "tree": "46870361d13dc5ee712eda367ae42699f1eea9da",
        "path": "docs/aapl_sec_gemma_lean_science_v3_14.md",
        "git_blob_sha1": "f7776b26053040dffc5848ffd5ec84a6f7d9f7d2",
        "literal_sha256": (
            "0a1f3026aae942dca69dca2408df9f8ebfc59d3761581878fac8adac86d12b96"
        ),
        "literal_bytes": 21_988,
    }
    assert predecessor["implementation"] == {
        "commit": "c4bdc39275e1bf29425bdcad5e9ccd83f0dfafa5",
        "tree": "48c1c5ba9e8bfc02245cb8ea32b7dd22f1019323",
        "parent": predecessor["preregistration"]["commit"],
    }
    assert predecessor["preflight_failure"] == {
        "commit": contract.BASE_PARENT,
        "tree": "328306c4f853a4d036ea62fbaad3f964f7e4a37c",
        "parent": predecessor["implementation"]["commit"],
        "path": (
            "e/aapl_sec_gemma_lean_science_v3_14/DEVELOPMENT_PREFLIGHT.json"
        ),
        "git_blob_sha1": "a8c6994eafb2d21a57c63776fbef813a084edf39",
        "literal_sha256": (
    ...
    assert predecessor["rejection"]["tree"] == contract.BASE_TREE
    assert predecessor["rejection"]["parent"] == contract.BASE_PARENT
    ...
    assert predecessor["rejection"]["literal_bytes"] == 33_079
    assert predecessor["phase1_node_count"] == 323
    assert predecessor["phase1_case_sensitive_unique_node_count"] == 323
    assert predecessor["phase1_duplicate_node_count"] == 0
    assert predecessor["phase1_required_node_count"] == 22
    ...
    assert len(
        {
            ...
            contract.PREREGISTRATION_COMMIT,
            contract.V314_PREREGISTRATION_COMMIT,
            contract.V314_IMPLEMENTATION_COMMIT,
            contract.V314_PREFLIGHT_FAILURE_COMMIT,
            contract.V38_SOURCE_COMMIT,
...
def test_effect_budgets_cover_normal_pause_resume_and_preflight() -> None:
    ...
    _assert_code(
        "v315_contract_effect_counts",
        contract.validate_effect_counts,
        changed,
        route="normal_complete",
    )
    _assert_code(
        "v315_contract_effect_route",
        contract.validate_effect_counts,
        changed,
        route="best_result",
    )
...
def test_pilot_projection_has_strict_greater_than_boundary() -> None:
    ...
    _assert_code("v315_contract_pilot_durations", contract.projected_pilot_ns, exact[:4])
    _assert_code(
        "v315_contract_pilot_durations",
...
def test_only_literal_development_command_is_accepted() -> None:
    ...
    for rejected in ("confirmation", "final", "development ", None, b"development"):
        _assert_code("v315_contract_command", contract.validate_command, rejected)
...
def test_local_production_closure_and_import_upper_bound_are_exact() -> None:
    ...
    _assert_code(
        "v315_contract_local_import_duplicate",
        contract.validate_local_import_paths,
        (observed[0], observed[0]),
    )
    _assert_code(
        "v315_contract_local_import_outside_closure",
        contract.validate_local_import_paths,
        ("agent_benchmark/unpreregistered_strategy.py",),
    )
    _assert_code(
        "v315_contract_local_import_shape",
        contract.validate_local_import_paths,
...
def test_qualification_scope_schemas_runtime_and_counts_are_frozen() -> None:
    ...
    assert qualification["phases"] == ["v315", "dependencies", "requests_identity"]
...
def test_contract_manifest_is_strict_self_hashed_and_detached() -> None:
    ...
    assert manifest["v314_rejection_authority"] == (
        contract.build_v314_rejection_pins()
    )
    ...
    _assert_code(
        "v315_contract_manifest_mismatch", contract.validate_contract_manifest, changed
    )
...
def test_preregistration_implementation_and_preflight_ancestry() -> None:
    ...
    _assert_code(
        "v315_contract_preflight_ancestry",
...
def test_implementation_allowlist_and_public_paths_are_exact() -> None:
    ...
    assert contract.IMPLEMENTATION_PRODUCTION_PATHS[0].endswith("v315_contract.py")
    assert contract.IMPLEMENTATION_TEST_PATHS[0].endswith("v315_contract.py")
