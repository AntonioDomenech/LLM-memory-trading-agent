...
V34_DELTA_MODULE_SHA256 = "a2ecfbcd58f99a992ec5547129b3704da7e63cb5d48b530a5a1a0d13c884ccec"
LOGIC_PATH = "agent_benchmark/logic.py"
TEST_PATH = "tests/test_logic.py"
DOC_PATH = "docs/prereg.md"
EXPECTED_V34_COUNTERPARTS = {
    "agent_benchmark/sec_filing_content.py": "agent_benchmark/sec_filing_content.py",
    ...
    "tests/test_sec_filing_content.py": "tests/test_sec_filing_content.py",
...
def _make_counterpart_repo(tmp_path: Path) -> dict[str, Any]:
    ...
    _git(repo, "config", "user.email", "offline@example.invalid")
    _git(repo, "config", "core.autocrlf", "false")
    old_path = "agent_benchmark/v33_logic.py"
    new_path = "agent_benchmark/v34_logic.py"
    source = b'def run():\n    return "source-only"\n'
...
def test_default_git_pins_are_real_exact_objects_and_prereg_is_doc_only() -> None:
    ...
    prereg = "c82029ac1b57abb5fe875c30088e1571dd5bf571"
    prereg_tree = "27c20058eb8e8733fded5beed230684b693de328"
    base = "f411ee509870336d0d90261e8b0338c8d5220c77"
    base_tree = "e293b7a13b4e0ee3f675d0758e8b673638866341"
    doc_path = "docs/aapl_sec_gemma_lean_evidence_v3_4.md"
    doc_blob = "fd043697313d4f5155dbb8ec8c57109b97abb009"
    doc_sha256 = "b1dfa90433d137e6e5f38a20c351cb7cff403d22ad8c017e7cd9eb99feb664d9"
...
def test_default_new_file_allow_inventories_match_frozen_worktree_structure() -> None:
    repo = Path(__file__).resolve().parents[1]
    expected_paths = tuple(sorted(EXPECTED_V34_COUNTERPARTS))
    modified_paths = {
        "agent_benchmark/sec_filing_content.py",
        "tests/test_sec_filing_content.py",
    }
    expected_added_paths = tuple(sorted(set(expected_paths) - modified_paths))
    ...
    assert tuple(sorted(delta._V34_IMPLEMENTATION_PATHS)) == expected_added_paths
    assert len(delta.DEFAULT_ALLOW_SPEC.path_rules) == 14
    assert {
        path: rule.status
        for path, rule in delta.DEFAULT_ALLOW_SPEC.path_rules.items()
    } == {
        path: "M" if path in modified_paths else "A" for path in expected_paths
    }
    ...
    for path, rule in delta.DEFAULT_ALLOW_SPEC.path_rules.items():
        before = None if rule.status == "A" else _blob(repo, delta.PREREG_COMMIT, path)
...
def test_committed_counterpart_rejects_same_symbol_import_hostile_body(
    ...
    # committed-v3.3 semantic comparison.
...
def test_checker_rejects_blank_company_scope_expansion_row_drop_and_special_case(
    ...
    new_path = "agent_benchmark/sec_gemma_lean_v34_source.py"
    baseline = (
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ({"counterpart_path": "agent_benchmark/missing_v33.py"}, "Git rejected"),
    ),
)
...
def test_external_anchor_rejects_delta_body_with_stale_test_literal(
    ...
    _git(repo, "config", "user.name", "Offline Test")
    _git(repo, "config", "user.email", "offline@example.invalid")
    old_path = "agent_benchmark/sec_gemma_lean_v33_delta.py"
    new_path = "agent_benchmark/sec_gemma_lean_v34_delta.py"
    anchor_path = "tests/test_sec_gemma_lean_v34_delta.py"
    ...
    _write(
        repo,
        anchor_path,
        f'V34_DELTA_MODULE_SHA256 = "{benign_sha}"\n'.encode("ascii"),
    )
