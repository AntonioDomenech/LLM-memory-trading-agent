...
def _assert_code(code: str, function: Any, /, *args: Any, **kwargs: Any) -> None:
    with pytest.raises(runner.V310RunnerError) as caught:
...
def test_deterministic_payload_is_reproducible_with_pure_dependencies(
    ...
    assert source["v310_source_commitments_sha256"] == contract.canonical_sha256(
        {key: value for key, value in source.items() if key != "v310_source_commitments_sha256"}
    )
...
def test_public_privacy_scanner_detects_json_escaped_windows_path() -> None:
    private_windows_path = r"C:\Users\example\private\v3_10"
...
def test_complete_private_namespace_allows_qualification_paths_but_rejects_secrets(
    ...
    preflight_mode = (
        private_root / "preflight" / "qualification" / "v310" / "execution"
    )
    ...
    v310_private = str(private_root.resolve(strict=False))
    forbidden_tokens = (
        contact,
        str(root),
        str(root).replace("\\", "/"),
    ...
    runner._assert_complete_v310_private_namespace_private_free(
        store,
...
def _install_fast_orchestration_store(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from agent_benchmark import sec_gemma_lean_science_v310_journal as journal_module
    ...
    def fast_journal_append(
        ...
        if (
            ...
            raise journal_module.V310JournalError("journal_event_invalid")
        normalized = dict(payload)
        journal_module.canonical_json_bytes(normalized)
        events = getattr(self, "_v310_test_events", None)
        if events is None:
            events = list(self.replay().events)
            self._v310_test_events = events
        ...
        if len(encoded) > journal_module.MAX_EVENT_BYTES:
            raise journal_module.V310JournalError("journal_event_too_large")
        final = self.root / f"{sequence:08d}-{event_sha256}.json"
        try:
            with final.open("xb") as handle:
                handle.write(encoded)
        except OSError:
            raise journal_module.V310JournalError(
    ...
    def fast_refresh(self: Any) -> None:
        if not getattr(self, "_v310_test_fast_refresh_ready", False):
            original_refresh(self)
            self._v310_test_fast_refresh_ready = True
            return

        self._require_open()
        journal = self._journal
        if journal is None:
            raise store_module.V310StoreError("store_not_initialized")
        events = getattr(journal, "_v310_test_events", None)
        if events is None:
            events = list(journal.replay().events)
            journal._v310_test_events = events
...
def _development_test_dependencies(
    ...
    *,
    ...
    from agent_benchmark import sec_gemma_lean_science_v310_journal as journal_module
    from agent_benchmark import sec_gemma_lean_science_v310_store as store_module
    from agent_benchmark.sec_gemma_lean_science_v310_store import AttemptStore
...
def test_development_dependencies_complete_normal_route_without_external_effects(
    ...
    from agent_benchmark.sec_gemma_lean_science_v310_store import audit_attempt
...
def _expected_continuation_document(pause: dict[str, Any]) -> bytes:
    ...
    return (
        "# AAPL SEC/Gemma lean science v3.10 continuation preregistration\n\n"
...
def test_full_synthetic_pause_restart_binds_exact_continuation_commit(
    ...
    assert not (tmp_path / f"{contract.RESULT_ARTIFACT_PATH}.v310-pending").exists()
    assert not (tmp_path / f"{contract.COMPARISON_PATH}.v310-pending").exists()
...
def test_status_reads_only_safe_snapshot_and_fails_closed(
    ...
    from agent_benchmark import sec_gemma_lean_science_v310_store as store
...
def test_cli_emits_canonical_safe_result_or_fixed_rejection(
    ...
    monkeypatch.setattr(
        runner,
        "status",
        lambda root: (_ for _ in ()).throw(runner.V310RunnerError("fixed_rejection")),
    )
...
def test_comparison_update_preserves_v39_rejection_and_places_v310_after_it() -> None:
    v39_row = (
    ...
    v39_index = lines.index(v39_row)
    assert lines[v39_index] == v39_row
    assert lines[v39_index + 1].startswith(
        "| SEC/Gemma lean science v3.10 |"
    )
    ...
    _assert_code(
        ...
        existing.replace(v39_row.encode("utf-8"), b"| Missing marker |"),
        result,
    )
