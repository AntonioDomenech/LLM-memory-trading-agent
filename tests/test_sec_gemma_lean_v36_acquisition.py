...
import agent_benchmark.sec_gemma_lean_v36_acquisition as acquisition_module
import agent_benchmark.sec_gemma_lean_v36_journal as journal_module
from agent_benchmark.sec_gemma_lean_v36_acquisition import (
...
from agent_benchmark.sec_gemma_lean_v36_journal import PrivateSecContact
...
class _SourceErrorSubclass(SecGemmaLeanV36SourceError):
    pass
...
def test_source_diagnostic_is_exact_safe_and_control_flow_neutral(
    ...
    if case == "wrong_type":
        ...
        error = SecGemmaLeanV36SourceError(message)
    ...
    with pytest.raises(SecGemmaLeanV36AcquisitionError) as caught:
        acquisition.run()
...
def test_terminal_inventory_requires_exact_tagged_manifest_body_hash(
    ...
    acquisition = DiskBackedSecAcquisition(
        ...
        transport_factory=factory,
        source_adapter=_RejectingSource(
            SecGemmaLeanV36SourceError("master_boundary_incomplete")
        ),
        dispatch_ledger=ledger,
        monotonic=clock.monotonic,
    )
...
def test_import_is_side_effect_free_and_exact_names_are_exported() -> None:
    assert ROOT_NAMESPACE == "aapl_sec_gemma_lean_evidence_v3_6"
    ...
    completed = subprocess.run(
        [
            ...
            "-B",
            "-c",
            "import agent_benchmark.sec_gemma_lean_v36_acquisition",
        ],
        cwd=repository,
        check=False,
        capture_output=True,
...
def test_production_development_binds_closure_then_rechecks_and_wires_runner(
    ...
    def load_closure() -> dict[str, object]:
        ...
        return {"agent_benchmark.sec_gemma_lean_v36_source": source_adapter}
    ...
    def bind_closure(
        ...
        *,
        ...
        assert loaded_modules == {
            "agent_benchmark.sec_gemma_lean_v36_source": source_adapter
        }
...
def test_production_gate_rejects_minimal_forged_preflight_via_strict_validator(
    ...
    def reject_forgery(root: Path) -> dict[str, object]:
        ...
        raise preflight.SecGemmaLeanV36PreflightError(
...
def test_complete_fake_acquisition_restarts_without_resend_and_rejects_orphan(
    ...
    with pytest.raises(SecGemmaLeanV36AcquisitionError):
        restarted.run()
