...
import agent_benchmark.sec_gemma_lean_v31_transport as transport
...
def _error_code(payload: bytes, *, limit: int = 32) -> str:
    with pytest.raises(transport.SecGemmaLeanV31TransportError) as captured:
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            "https://www.sec.gov/Archives/edgar/full-index/1994/QTR3/master.gz",
            "quarterly_master",
            "master/1994/QTR3",
...
def test_receipt_is_deterministic_redacted_and_self_hashed(tmp_path: Path) -> None:
    ...
    with pytest.raises(transport.SecGemmaLeanV31TransportError):
