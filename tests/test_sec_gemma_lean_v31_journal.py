...
def _assert_rehashed_basis_tamper_rejected(
    ...
    *,
    ...
    try:
        with pytest.raises(SecGemmaLeanV31JournalError) as caught:
            validate_detached_journal(
...
def test_frozen_stage_formula_order_and_exact_caps() -> None:
    ...
    assert development.expected_successful_requests == 1 + 16 + 98 + 128 == 243
    assert intermediate.expected_successful_requests == 1 + 16 + 118 + 24 == 159
    assert final.expected_successful_requests == 1 + 16 + 129 + 16 == 162
    assert [role.ordinal for role in development.roles] == list(range(1, 244))
    ...
    assert development.roles[17].role_id == "master/1994/QTR3"
    assert development.roles[114].role_id == "master/2018/QTR4"
    assert development.roles[115].role_id == "complete/0000320193-00-000000"
    ...
    assert sum(role.reservation_bytes for role in development.roles) == (
        SUCCESSFUL_RESPONSE_BYTE_CAP + 243
    )
    assert sum(
...
@pytest.mark.parametrize(
    ...
    [
        ("development", 16, 128, 243),
        ("intermediate", 16, 24, 159),
        ("final", 16, 16, 162),
    ],
)
...
def test_phase_limits_and_canonical_urls_are_frozen() -> None:
    ...
    assert roles["master/1994/QTR3"].body_limit_bytes == MASTER_COMPRESSED_BODY_LIMIT
    assert (
        roles["master/1994/QTR3"].decompressed_body_limit_bytes
        == MASTER_DECOMPRESSED_BODY_LIMIT
    )
...
def test_full_formula_requires_one_intent_http_200_and_seal_per_role(
    ...
    assert phase_events[1]["role_count"] == 98
    ...
    assert journal.state.formula_requests == 99
    journal.seal_terminal_pass(checkpoint_sha256=CHECKPOINT_SHA256)
...
def test_close_directory_fsync_failure_cannot_replay_as_clean(
    ...
    with pytest.raises(SecGemmaLeanV31JournalError) as replay_caught:
