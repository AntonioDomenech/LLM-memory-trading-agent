...
from agent_benchmark import sec_gemma_lean_science_v319_runner as runner
...
def _assert_code(code: str, function: Any, /, *args: Any, **kwargs: Any) -> None:
    with pytest.raises(runner.V319RunnerError) as caught:
...
def _projection() -> SimpleNamespace:
    ...
    manifest = {
        ...
        "filename_present_count": contract.DEVELOPMENT_FILENAME_PRESENT_COUNT,
        "filename_missing_count": contract.DEVELOPMENT_FILENAME_MISSING_COUNT,
        "compatibility_manifest_sha256": _digest("compatibility-manifest"),
        "universe_sha256": _digest("universe"),
        "content_manifest_sha256": _digest("content-manifest"),
        "calendar_sessions_sha256": contract.CALENDAR_SESSIONS_SHA256,
        "universe_event_proofs_sha256": _digest("universe-event-proofs"),
    }
...
def _synthetic_sanitizer_material(
    requests: list[dict[str, Any]],
    ...
    for index, request in enumerate(requests, start=1):
        ...
        source_hash = _digest(f"source-preprocessed:{accession}")
        sanitized_hash = request["preprocessed_event_sha256"]
        removed_current = 2 if index == 1 else (1 if 2 <= index <= 4 else 0)
        removed_prior = 1 if 5 <= index <= 7 else 0
        source = {"preprocessed_event_sha256": source_hash}
        event = {
            "preprocessed_event_sha256": sanitized_hash,
        ...
        rows.append(
            {
                ...
                "sanitized_preprocessed_event_sha256": sanitized_hash,
                "removed_current_sentence_count": removed_current,
                "removed_prior_sentence_count": removed_prior,
            }
        )
    aggregate = runner.build_direction_sanitizer_aggregate(rows)
    assert aggregate["events_with_removals"] == 7
    assert (
        aggregate["removed_current_sentence_count"]
        + aggregate["removed_prior_sentence_count"]
        == 8
    )
    return sources, sanitized, aggregate
...
def _install_model_plan_fakes(
    ...
    if reverse:
        requests.reverse()
        proofs.reverse()
    sources, sanitized, aggregate = _synthetic_sanitizer_material(requests)

    monkeypatch.setattr(
        runner,
        "_build_nullable_model_bundle",
        lambda projection: (
            {"universe_sha256": _digest("universe")},
            copy.deepcopy(requests),
            copy.deepcopy(proofs),
    ...
    monkeypatch.setattr(
        ...
        lambda request, *, proof, sanitized_event: copy.deepcopy(dict(request)),
    )
    monkeypatch.setattr(
        production,
...
def _model_plan() -> runner.ModelPlan:
    ...
    for i in range(1, 76):
        ...
        requests.append(request)
        source_event = {
            "preprocessed_event_sha256": _digest(
                f"source-preprocessed:{accession}"
            )
        }
        sanitized_event = {
    ...
    sources, sanitized, sanitizer_aggregate = _synthetic_sanitizer_material(requests)
    calls = [
        replace(
            item,
            source_preprocessed_event=source,
            sanitized_event=event,
        )
        for item, source, event in zip(calls, sources, sanitized, strict=True)
    ]
    canonical_request_index = [
        {
            ...
            "preprocessed_event_sha256": item.request[
    ...
    manifest_body = {
        ...
        "universe_sha256": _digest("universe"),
        "canonical_request_index_sha256": contract.canonical_sha256(
            canonical_request_index
        ),
        "execution_index_sha256": contract.canonical_sha256(execution_index),
        "pilot_order_sha256": contract.canonical_sha256(order_index[:5]),
        "remaining_order_sha256": contract.canonical_sha256(order_index[5:]),
        "model_call_count": 75,
        "pilot_count": 5,
        "remaining_count": 70,
    }
    return runner.ModelPlan(
        model_slice={
            "model_slice_sha256": _digest("model-slice"),
            "source_preprocessed_events": copy.deepcopy(sources),
            "direction_sanitized_preprocessed_events": copy.deepcopy(sanitized),
        },
        universe={"universe_sha256": _digest("universe")},
        canonical_calls=tuple(calls),
        execution_calls=tuple(calls),
        sanitizer_aggregate=copy.deepcopy(sanitizer_aggregate),
        manifest=_with_hash(manifest_body, "model_plan_sha256"),
    )


def _synthetic_commitments(
    projection: SimpleNamespace,
    plan: runner.ModelPlan,
    *,
    canonical_requests_sha256: str | None = None,
) -> dict[str, Any]:
    byte_counts = [item.request_byte_count for item in plan.canonical_calls]
    manifest = projection.manifest
    return {
        "schema_version": runner.REQUEST_COMMITMENTS_SCHEMA_VERSION,
        "stage": "development",
        "document_count": 75,
        "record_count": 75,
        "event_count": 75,
        ...
        "pilot_count": 5,
        "remaining_count": 70,
        "filename_present_count": manifest["filename_present_count"],
        "filename_missing_count": manifest["filename_missing_count"],
        "compatibility_manifest_sha256": manifest[
            "compatibility_manifest_sha256"
        ],
        "universe_sha256": manifest["universe_sha256"],
        "content_manifest_sha256": manifest["content_manifest_sha256"],
        "calendar_sessions_sha256": manifest["calendar_sessions_sha256"],
        "universe_event_proofs_sha256": manifest[
            "universe_event_proofs_sha256"
        ],
        "documents_sha256": manifest["documents_sha256"],
        "records_sha256": manifest["records_sha256"],
        "events_sha256": manifest["event_order_sha256"],
        "preprocessed_events_sha256": _digest("preprocessed-events"),
        "canonical_requests_sha256": (
            canonical_requests_sha256 or _digest("canonical-requests")
        ),
        ...
        "pilot_order_sha256": plan.manifest["pilot_order_sha256"],
        "remaining_order_sha256": plan.manifest["remaining_order_sha256"],
        "minimum_request_byte_count": min(byte_counts),
        "maximum_request_byte_count": max(byte_counts),
        "source_order_sha256": manifest["source_order_sha256"],
        "prior_links_sha256": manifest["prior_links_sha256"],
        "set_parity": True,
...
def test_runner_builds_only_sanitized_requests_through_both_validators() -> None:
    from agent_benchmark import sec_gemma_lean_science_v319_bridge as bridge
    from agent_benchmark.sec_filing_content import normalize_filing_text
    from agent_benchmark.sec_filing_gemma_contract import CANONICAL_IDENTITY_LEXICON
    ...
    from tests.test_sec_gemma_lean_science_v319_bridge import _make_fixture

    # No validator is replaced in this test. The synthetic V3.8 authority is
    # authenticated by the real bridge, and every later transformation is the
    # real V3.19 production function.
    projection = bridge.build_streaming_science_projection(
        _make_fixture().authority
    )
    plan = runner.build_model_plan(projection)
    normalized_by_accession = {
        document["accession_number"]: normalize_filing_text(
            document["body"].decode("latin-1")
        ).text
        for document in projection.primary_documents
    }

    assert len(plan.canonical_calls) == 75
    assert len(plan.model_slice["source_preprocessed_events"]) == 75
    assert len(
        plan.model_slice["direction_sanitized_preprocessed_events"]
    ) == 75
    assert plan.manifest["direction_sanitizer_aggregate"] == dict(
        plan.sanitizer_aggregate
    )

    for call in plan.canonical_calls:
        proof = dict(call.proof)
        prior_record = proof["prior_same_form_record"]
        prior_text = (
            None
            if prior_record is None
            else normalized_by_accession[prior_record["accession_number"]]
        )
        trusted_source_hash = validate_preprocessed_event(
            call.source_preprocessed_event,
            current_normalized_text=normalized_by_accession[
                call.accession_number
            ],
        ...
        validated_sanitized = (
            contract.validate_direction_sanitized_preprocessed_event(
                call.source_preprocessed_event,
                call.sanitized_event,
                expected_source_preprocessed_event_sha256=trusted_source_hash,
            )
        )
        assert contract.validate_blinded_model_request(
            call.request,
            proof=proof,
            sanitized_event=validated_sanitized,
        ) == dict(call.request)
        inherited_bytes, inherited_ids, _payload = (
            _validate_blinded_model_request(dict(call.request), proof)
        )
        assert inherited_bytes == call.request_bytes
        assert inherited_ids == call.sentence_ids
        assert call.request["preprocessed_event_sha256"] == (
            call.sanitized_event["preprocessed_event_sha256"]
        )
        assert call.sanitized_event["source_preprocessed_event_sha256"] == (
            trusted_source_hash
        )

    # The call-order audit proves the successful values above were produced by
    # the non-injectable runner path in the preregistered order.
    source = inspect.getsource(runner._build_nullable_model_bundle)
    ordered_calls = (
...
def test_attempt_authority_binds_execution_source_science_and_exact_order() -> None:
    ...
    plan = _model_plan()
    commitments = _synthetic_commitments(projection, plan)
    execution = {
        ...
        "preflight": {
            ...
            "runtime_binding_authority_sha256": _digest(
...
def test_real_bridge_preflight_receipt_and_attempt_authority_seam(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The production bridge and preflight receipt must fit the runner exactly."""

    from agent_benchmark import sec_gemma_lean_science_v319_bridge as bridge
    from agent_benchmark import sec_gemma_lean_science_v319_preflight as preflight
    from tests.test_sec_gemma_lean_science_v319_bridge import _make_fixture
    from tests.test_sec_gemma_lean_science_v319_preflight import (
        _qualification,
        _repository_snapshot,
        _runtime_authority_fixture,
    )
    ...
    plan = runner.build_model_plan(projection)
    commitments = runner.build_preflight_request_commitments(projection)
    # This bridge fixture is structural and deliberately contains no selected
    # direction phrases. Give the preflight-schema seam the separately tested
    # frozen seven-event/eight-sentence aggregate without misrepresenting the
    # fixture's model slice as the official redacted replay.
    _sources, _sanitized, official_aggregate = _synthetic_sanitizer_material(
        [dict(item.request) for item in plan.canonical_calls]
    )
    plan_manifest_body = dict(plan.manifest)
    plan_manifest_body.pop("model_plan_sha256")
    plan_manifest_body["direction_sanitizer_aggregate"] = official_aggregate
    plan = replace(
        plan,
        sanitizer_aggregate=copy.deepcopy(official_aggregate),
        manifest=_with_hash(plan_manifest_body, "model_plan_sha256"),
    )
    commitments["direction_sanitizer_aggregate"] = copy.deepcopy(
        official_aggregate
    )
    commitments["model_plan_sha256"] = plan.manifest["model_plan_sha256"]
    # The bridge fixture authenticates synthetic V3.8 private evidence.  Keep
    # its real 41-field output untouched, but make the frozen contract pins
    # describe that authenticated synthetic authority for this offline seam.
    source_pins = contract.build_source_authority_pins()
    monkeypatch.setattr(
        contract, "build_source_authority_pins", lambda: source_pins
    )
    for field, constant_name in (
        ...
        monkeypatch.setattr(contract, constant_name, bridge_manifest[field])
    repository = _repository_snapshot()
    qualification = _qualification()
    runtime_authority = _runtime_authority_fixture()
    ...
    private_literal_sha256 = hashlib.sha256(
    ...
    assert public_artifact["counts"]["sanitizer_events_with_removals"] == 7
    assert (
        public_artifact["counts"]["sanitizer_removed_current_sentence_count"]
        + public_artifact["counts"]["sanitizer_removed_prior_sentence_count"]
        == 8
    )
    assert "preprocessed_events_sha256" not in public_artifact["aggregates"]
    assert "canonical_requests_sha256" not in public_artifact["aggregates"]
    assert "source_events_sha256" not in public_artifact["aggregates"]
    receipt = preflight._build_execution_authority_receipt(
        public_artifact=public_artifact,
        private_manifest=private_manifest,
        preflight_commit="3" * 40,
        preflight_tree="4" * 40,
        implementation_commit=repository["ancestry"]["commit"],
        implementation_tree=repository["ancestry"]["tree"],
    ...
    execution_authority = preflight._store_execution_authority(receipt)

    attempt_authority = runner.build_attempt_authority(
    ...
    assert attempt_authority == execution_authority
    assert attempt_authority["preflight"][
        "runtime_binding_authority_sha256"
    ] == runtime_authority["runtime_binding_authority_sha256"]
    assert attempt_authority["source"]["universe_sha256"] == plan.manifest[
        "universe_sha256"
    ]
    assert attempt_authority["request_order"]["model_plan_sha256"] == (
        plan.manifest["model_plan_sha256"]
    )
...
def test_production_yahoo_post_read_rejects_exact_absolute_deadline(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class Socket:
    ...
    class Headers:
        ...
        def get_all(self, key: str) -> list[str]:
            return ["2"] if key == "Content-Length" else []
    ...
    _assert_code(
        "yahoo_deadline_reached",
        runner.fetch_yahoo_once,
        contract.YAHOO_URLS[0],
        deadline_monotonic=100.0,
    )
    assert opener.timeouts == [30.0]
    assert opener.socket.timeouts == [30.0]


def test_production_yahoo_accepts_exact_declared_body_after_httpresponse_detaches_fp(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class Socket:
        def __init__(self) -> None:
    ...
    class Response:
        def __init__(self) -> None:
            self.headers = Headers()
            self.fp: Any = SimpleNamespace(raw=SimpleNamespace(_sock=socket))
            self.read_count = 0
        ...
        def read1(self, _size: int) -> bytes:
            ...
            if self.read_count != 1:
                raise AssertionError("the declared body must not need an EOF read")
            # This mirrors http.client.HTTPResponse._close_conn after the
            # final Content-Length byte is consumed.
            self.fp = None
            return b"{}"
    ...
    body, metadata = runner.fetch_yahoo_once(
        contract.YAHOO_URLS[0], deadline_monotonic=100.0
    )

    assert body == b"{}"
    assert metadata["declared_content_length"] == 2
    assert response.read_count == 1
    assert socket.timeouts == [30.0]


def test_production_yahoo_narrows_live_socket_before_every_read(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class Clock:
        def __init__(self) -> None:
    ...
    class Response:
        def __init__(self) -> None:
            ...
            self.chunks = iter(((5.0, b"{"), (10.0, b"}"), (0.0, b"")))
        ...
        def read1(self, _size: int) -> bytes:
            advance, chunk = next(self.chunks)
            clock.value += advance
            return chunk
    ...
    assert body == b"{}"
    assert metadata["network_requests"] == 1
    assert socket.timeouts == [30.0, 25.0]
    assert all(timeout > 0.0 for timeout in socket.timeouts)
...
class _ManualMonotonicClock:
    ...
    def advance(self, seconds: int) -> None:
        self.value_ns += seconds * 1_000_000_000
...
class _DeadlineResponse:
    def __init__(
        ...
        *,
        ...
        body_bytes = sum(len(chunk) for _advance, chunk in chunks)
        self.headers = {
    ...
    def iter_content(self, *, chunk_size: int) -> Any:
        ...
        for seconds, chunk in self.chunks:
...
class _DeadlineSession:
    ...
    def request(self, method: str, endpoint: str, **kwargs: Any) -> Any:
        self.calls.append((method, endpoint, copy.deepcopy(kwargs)))
        self.clock.advance(self.request_seconds)
        self.response.url = endpoint
        return self.response


def test_production_generation_deadline_permits_pause_scale_but_stops_trickle(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    long_clock = _ManualMonotonicClock()
    monkeypatch.setattr(runner.time, "monotonic_ns", long_clock.monotonic_ns)
    long_response = _DeadlineResponse(
        clock=long_clock,
        chunks=[(0, b"{}")],
    )
    long_session = _DeadlineSession(
        response=long_response,
        clock=long_clock,
        request_seconds=610,
    )

    body, http, duration_ns = runner._raw_generation_request(
        long_session, request_bytes=b"synthetic-request"
    )

    assert body == b"{}"
    assert http["http_status"] == 200
    assert duration_ns == 610_000_000_000
    assert contract.pilot_pause_required([duration_ns] * 5) is True
    assert len(long_session.calls) == 1
    method, _endpoint, kwargs = long_session.calls[0]
    assert method == "POST"
    assert kwargs["allow_redirects"] is False
    assert kwargs["stream"] is True
    assert sum(kwargs["timeout"]) == pytest.approx(
        runner.MODEL_GENERATION_ABSOLUTE_TIMEOUT_SECONDS
    )
    assert long_response.socket.timeouts
    assert long_response.closed is True

    trickle_clock = _ManualMonotonicClock()
    monkeypatch.setattr(
        runner.time, "monotonic_ns", trickle_clock.monotonic_ns
    )
    trickle_response = _DeadlineResponse(
        clock=trickle_clock,
        chunks=[(300, b"{"), (300, b" "), (301, b"}")],
    )
    trickle_session = _DeadlineSession(
        response=trickle_response,
        clock=trickle_clock,
    )

    _assert_code(
        "ollama_generation_deadline_reached",
        runner._raw_generation_request,
        trickle_session,
        request_bytes=b"synthetic-request",
    )
    assert len(trickle_session.calls) == 1
    assert trickle_response.socket.timeouts == pytest.approx(
        [900.0, 600.0, 300.0]
    )
    assert trickle_response.closed is True


def test_production_probe_deadline_is_absolute_and_never_retries(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from agent_benchmark.sec_filing_gemma_ollama import OLLAMA_SHOW_ENDPOINT

    success_clock = _ManualMonotonicClock()
    monkeypatch.setattr(
        runner.time, "monotonic_ns", success_clock.monotonic_ns
    )
    success_response = _DeadlineResponse(
        clock=success_clock,
        chunks=[(1, b"{}")],
    )
    success_session = _DeadlineSession(
        response=success_response,
        clock=success_clock,
    )

    body, http = runner._runtime_probe_request(success_session, kind="show")

    assert body == b"{}"
    assert http["response_url"] == OLLAMA_SHOW_ENDPOINT
    assert len(success_session.calls) == 1
    method, endpoint, kwargs = success_session.calls[0]
    assert (method, endpoint) == ("POST", OLLAMA_SHOW_ENDPOINT)
    assert type(kwargs["data"]) is bytes
    assert sum(kwargs["timeout"]) == pytest.approx(
        runner.RUNTIME_PROBE_ABSOLUTE_TIMEOUT_SECONDS
    )
    assert success_response.closed is True

    timeout_clock = _ManualMonotonicClock()
    monkeypatch.setattr(
        runner.time, "monotonic_ns", timeout_clock.monotonic_ns
    )
    timeout_response = _DeadlineResponse(
        clock=timeout_clock,
        chunks=[(61, b"{}")],
    )
    timeout_session = _DeadlineSession(
        response=timeout_response,
        clock=timeout_clock,
    )

    _assert_code(
        "runtime_probe_deadline_reached",
        runner._runtime_probe_request,
        timeout_session,
        kind="version",
    )
    assert len(timeout_session.calls) == 1
    assert timeout_response.socket.timeouts == pytest.approx([60.0])
    assert timeout_response.closed is True
...
def test_deterministic_payload_is_reproducible_with_pure_dependencies(
    ...
    assert source["v319_source_commitments_sha256"] == contract.canonical_sha256(
        {key: value for key, value in source.items() if key != "v319_source_commitments_sha256"}
    )
...
def test_forbidden_old_validator_sentinel_is_retained_and_triggered(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from agent_benchmark import sec_gemma_online_risk_overlay_production as production

    # Install every harmless production fake first.  The predecessor installed
    # the sentinel first and then accidentally overwrote it here, so the test
    # could not prove that the forbidden predecessor validators stayed unused.
    _install_deterministic_fakes(monkeypatch)
    triggered: list[str] = []
    ...
    def forbidden(name: str) -> Any:
        def sentinel(*_args: Any, **_kwargs: Any) -> Any:
            ...
            raise AssertionError(f"forbidden predecessor validator executed: {name}")

        return sentinel

    for name in ("_validate_universe_proof", "_validate_blinded_model_request"):
        monkeypatch.setattr(production, name, forbidden(name))

    source = inspect.getsource(runner.build_deterministic_payload)
    assert "_validate_universe_proof" not in source
    assert "_validate_blinded_model_request" not in source
    stage_slice, semantic = _deterministic_inputs()
    runner.build_deterministic_payload(
        stage_slice=stage_slice,
        semantic_payload=semantic,
    )
    assert triggered == []

    # The exact sentinels are still installed after the production-shaped call;
    # explicitly invoking them proves they were retained rather than replaced.
    for name in ("_validate_universe_proof", "_validate_blinded_model_request"):
        with pytest.raises(AssertionError, match=name):
            getattr(production, name)({})
    assert triggered == ["_validate_universe_proof", "_validate_blinded_model_request"]
...
def test_real_bridge_payloads_evaluator_and_terminal_material_replay_offline() -> None:
    """Exercise the inherited evaluator without inventing a passing result."""
    ...
    # Start with the authenticated 75-document bridge fixture and the real
    # V3.19 model-plan builder.  No SEC, Yahoo, Ollama, or other transport is
    # reachable from this pure offline construction.
    projection = bridge.build_streaming_science_projection(
        _make_fixture().authority
    )
    plan = runner.build_model_plan(projection)

    event_by_request: dict[str, str] = {}
    ordered_response_events: list[str] = []
    for item in plan.execution_calls:
        event_sha = _digest(
            f"offline-real-evaluator-response:{item.execution_ordinal}"
        )
        event_by_request[item.request["request_sha256"]] = event_sha
        ordered_response_events.append(event_sha)
    guard = _guard(
        "full",
        ordered_response_events,
        seed="offline-real-evaluator",
    )
    aggregate = runner.build_runtime_aggregate_guard(
        segment_guards=[guard],
        execution_request_sha256s=[
    ...
    latency = runner.build_latency_receipt(
        plan=plan,
        durations_ns_by_request_sha256={
            item.request["request_sha256"]: 1
            for item in plan.execution_calls[:5]
        },
    )

    # A real Ollama response envelope with an authenticated non-normal finish
    # produces an intentionally neutral semantic row.  This proves plumbing;
    # it cannot be mistaken for model evidence or a profitable result.
    output = '{"offline_fixture":"authenticated neutral completion"}'
    response_bytes = json.dumps(
        {
            "model": "gemma4:12b",
            "created_at": "2026-07-11T10:11:12.123456789Z",
    ...
    for item in plan.canonical_calls:
        ...
        sealed[request_sha] = {
            ...
            "duration_ns": item.execution_ordinal,
            "response_event_sha256": event_by_request[request_sha],
        }
        guards[request_sha] = guard
    semantic = runner.build_semantic_payload(
        plan=plan,
        sealed_calls_by_request_sha256=sealed,
        segment_guard_by_request_sha256=guards,
        runtime_aggregate=aggregate,
    ...
    assert {
        ...
        for row in semantic["semantic_extraction_rows"]
    } == {"invalid"}

    # Build one compact, deterministic market-shaped fixture through the real
    # inherited stage-slice constructor.  Sparse structural rows keep this
    # test offline and small; no performance claim is made from their values.
    sessions = {
        item.request["availability_session"] for item in plan.canonical_calls
    }
    for year in range(2000, 2019):
        for month in range(1, 13):
            sessions.add(f"{year:04d}-{month:02d}-15")
    sessions.update(
        {
            "2000-01-03",
            "2005-01-03",
    ...
    ordered_sessions = sorted(
        ...
        if "2000-01-03" <= session <= "2018-12-31"
    )
    rows_by_symbol: dict[str, list[dict[str, Any]]] = {}
    for symbol_ordinal, symbol in enumerate(YAHOO_SYMBOLS, start=1):
        market_rows: list[dict[str, Any]] = []
        for row_ordinal, session in enumerate(ordered_sessions, start=1):
            close = 20.0 + symbol_ordinal + row_ordinal / 1_000.0
            market_rows.append(
                {
                    "session": session,
                    "available": True,
                    "values": {
                        ...
                        "low": (close * 0.99).hex(),
                        "close": close.hex(),
                        "volume": float(1_000_000 + row_ordinal).hex(),
                        "adjusted_close": (close * 0.95).hex(),
                    },
                }
            )
        rows_by_symbol[symbol] = market_rows
    stage_slice = _build_stage_slice(
        plan=runner._v319_market_plan(),
        rows_by_symbol=rows_by_symbol,
        model_requests=[dict(item.request) for item in plan.canonical_calls],
        universe_event_proofs=[
            dict(item.proof) for item in plan.canonical_calls
        ],
    )

    # Leave all four production seams real: deterministic payload minting,
    # stage-input construction, inherited evaluator evaluate+validate replay,
    # and private terminal material construction.
    deterministic = runner.build_deterministic_payload(
        stage_slice=stage_slice,
        semantic_payload=semantic,
        plan=plan,
    )
    science = runner.evaluate_deterministic_science(
        semantic_payload=semantic,
        deterministic_payload=deterministic,
    )
    terminal = runner.build_private_terminal_material(
        authority={"offline_fixture_kind": "structural_no_effect"},
        projection=projection,
        plan=plan,
        ...
        invocation_parent="1" * 40,
        invocation_parent_kind="preflight",
    )

    assert len(deterministic["feature_rows"]) == 75
    assert science["evaluation"]["stage_input_bundle_sha256"] == science[
        "stage_input_bundle"
    ]["stage_input_bundle_sha256"]
    assert science["evaluation"]["gate_report"]["passed"] is False
    assert terminal["science_summary"]["gate_report"]["passed"] is False
    assert terminal["private_terminal_material_sha256"] == (
        contract.canonical_sha256(
            {
                key: value
                for key, value in terminal.items()
                if key != "private_terminal_material_sha256"
            }
        )
    )
...
def test_public_privacy_scanner_detects_json_escaped_windows_path() -> None:
    private_windows_path = r"C:\Users\example\private\v3_19"
...
def test_private_namespace_privacy_scans_every_relative_path_name(
    tmp_path: Path, entry_kind: str
) -> None:
    private_root = tmp_path / "private"
    private_root.mkdir()
    contact = "private-contact@example.invalid"
    leaked_name = contact.upper()
    if entry_kind == "file":
        (private_root / f"{leaked_name}.json").write_bytes(b'{"safe":true}')
    else:
        (private_root / leaked_name).mkdir()

    _assert_code(
        "private_namespace_privacy_failed",
        runner.assert_private_namespace_private_free,
        private_root,
        forbidden_tokens=(contact,),
    )


@pytest.mark.parametrize("injected_kind", ("contact", "v38_private_path"))
def test_complete_private_namespace_allows_qualification_paths_but_rejects_secrets(
    tmp_path: Path, injected_kind: str
) -> None:
    root = tmp_path.resolve()
    private_root = root / contract.PRIVATE_NAMESPACE
    preflight_mode = (
        private_root / "preflight" / "qualification" / "v319" / "execution"
    )
    development_root = private_root / "development"
    preflight_mode.mkdir(parents=True)
    development_root.mkdir()
    junit_path = preflight_mode / "junit.xml.partial"
    qualification_intent = {
        ...
        "private_preflight_root": str(private_root / "preflight"),
    }
    (preflight_mode / "intent.json").write_bytes(
        contract.canonical_json_bytes(qualification_intent)
    )
    (development_root / "safe.json").write_bytes(b'{"sealed":true}\n')

    contact = "synthetic-private-contact@example.invalid"
    v38_private = str(
        (root / "data/aapl_sec_gemma_lean_evidence_v3_8").resolve(strict=False)
    )
    v319_private = str(private_root.resolve(strict=False))
    forbidden_tokens = (
        contact,
        str(root),
        str(root).replace("\\", "/"),
    ...
    store = SimpleNamespace(
        ...
        privacy_scan_locked_payloads=lambda: {},
    )

    runner._assert_complete_v319_private_namespace_private_free(
        store,
    ...
    injected = contact if injected_kind == "contact" else v38_private
    (preflight_mode / "injected.json").write_bytes(
        contract.canonical_json_bytes({"injected": injected})
    )
    _assert_code(
        "private_namespace_privacy_failed",
        runner._assert_complete_v319_private_namespace_private_free,
...
def _install_fast_orchestration_store(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from agent_benchmark import sec_gemma_lean_science_v319_journal as journal_module
    from agent_benchmark import sec_gemma_lean_science_v319_store as store_module
    ...
    def fast_journal_append(
        ...
        if (
            ...
            raise journal_module.V319JournalError("journal_event_invalid")
        normalized = dict(payload)
        journal_module.canonical_json_bytes(normalized)
        events = getattr(self, "_v319_test_events", None)
        if events is None:
            events = list(self.replay().events)
            self._v319_test_events = events
        ...
        if len(encoded) > journal_module.MAX_EVENT_BYTES:
            raise journal_module.V319JournalError("journal_event_too_large")
        final = self.root / f"{sequence:08d}-{event_sha256}.json"
        try:
            with final.open("xb") as handle:
                handle.write(encoded)
        except OSError:
            raise journal_module.V319JournalError(
    ...
    def fast_refresh(self: Any) -> None:
        if not getattr(self, "_v319_test_fast_refresh_ready", False):
            original_refresh(self)
            self._v319_test_fast_refresh_ready = True
            return

        self._require_open()
        journal = self._journal
        if journal is None:
            raise store_module.V319StoreError("store_not_initialized")
        events = getattr(journal, "_v319_test_events", None)
        if events is None:
            events = list(journal.replay().events)
            journal._v319_test_events = events
        ...
        try:
            ...
            recovery_replay = journal_module.JournalReplay(
                self.authority_sha256,
                tuple(self._recovery_journal.replay().events),
            )
            recovery_machine = store_module._replay_recovery_state(
                recovery_replay, machine
            )
            (
                terminal_candidate,
                terminal_evidence,
                pause_candidate,
            ) = store_module._audit_publication_candidates(
                machine,
                self.root / store_module.PUBLICATION_CANDIDATE_DIRECTORY,
            )
            store_module._audit_publication_links(
                machine,
                recovery_machine,
                terminal_candidate,
                pause_candidate,
...
def _development_test_dependencies(
    ...
    *,
    ...
    from agent_benchmark import sec_gemma_lean_science_v319_journal as journal_module
    from agent_benchmark import sec_gemma_lean_science_v319_store as store_module
    from agent_benchmark.sec_gemma_lean_science_v319_store import AttemptStore
    ...
    plan = _model_plan()
    commitments = _synthetic_commitments(
        projection,
        plan,
        canonical_requests_sha256=_digest(
            "orchestration-canonical-requests"
        ),
    )
    authority = {
        "plan": contract.CONTRACT_MANIFEST_SHA256,
    ...
    comparison_before = (
        b"| Approach | Information | Development | LLM | Decision |\n"
        b"| SEC/Gemma lean science v3.18 | frozen predecessor | No | No | Preserved |\n"
    )
    invocation_commits = {
    ...
    def authenticate_execution(root: Path, descendant: bool) -> dict[str, Any]:
        ...
        comparison_path = root / Path(contract.COMPARISON_PATH)
        comparison_path.parent.mkdir(parents=True, exist_ok=True)
        if not comparison_path.exists():
            comparison_path.write_bytes(comparison_before)
    ...
    def publish_result(root: Path, value: dict[str, Any]) -> None:
        ...
        result_path = root / Path(contract.RESULT_ARTIFACT_PATH)
        result_path.parent.mkdir(parents=True, exist_ok=True)
        result_path.write_bytes(contract.canonical_json_bytes(value))
        comparison_path = root / Path(contract.COMPARISON_PATH)
        comparison_path.write_bytes(
            runner.build_comparison_update(comparison_before, value)
        )
    ...
    def runtime_binding(
        ...
        assert root.is_absolute()
        assert supplied_authority == authority
        trace["runtime_bindings"].append(
            (checkpoint_name, invocation_kind)
        )
        return {
            "head": invocation_commits[trace["invocation_parent_kind"]],
            "tree": authority["implementation"]["tree"],
            "clean_state_sha256": _digest(
                f"clean:{invocation_kind}"
            ),
            "repository_runtime_manifest_sha256": _digest(
                "repository-runtime"
            ),
            "execution_dependency_manifest_sha256": _digest(
                "execution-dependencies"
            ),
            "loaded_code_manifest_sha256": _digest(
                f"loaded-code:{checkpoint_name}"
            ),
            "runtime_binding_authority_sha256": _digest(
                "runtime-authority"
            ),
            "route_argv_sha256": _digest(f"argv:{invocation_kind}"),
            "process_environment_sha256": _digest("process-environment"),
            "interpreter_identity_sha256": _digest("interpreter-identity"),
        }
    ...
    dependencies = runner.DevelopmentDependencies(
        ...
        read_comparison=lambda root: comparison_before,
...
def _continuation_order_trace(
    ...
    continuation_path = tmp_path / Path(
    ...
    original_authorize = AttemptStore.authorize_continuation

    def authorize_continuation(self: Any, **kwargs: Any) -> Any:
        event = original_authorize(self, **kwargs)
        order.append("continuation_authorized")
        return event

    monkeypatch.setattr(
        AttemptStore, "authorize_continuation", authorize_continuation
    )

    original_begin_request = AttemptStore.begin_request

    def begin_request(self: Any, **kwargs: Any) -> Any:
        intent = original_begin_request(self, **kwargs)
        if (
            kwargs.get("effect_kind") == "identity"
            and kwargs.get("segment_id") == "continuation"
            and "first_ollama_intent" not in order
        ):
            order.append("first_ollama_intent")
        return intent

    monkeypatch.setattr(AttemptStore, "begin_request", begin_request)

    original_binding = dependencies.runtime_binding
    ...
    def runtime_binding(
        ...
        value = original_binding(
            root,
            supplied_authority,
            checkpoint_name,
            invocation_kind,
        )
        if invocation_kind == "continuation" and checkpoint_name in {
            "attempt_open",
            "model_continuation_pre",
        }:
            order.append(checkpoint_name)
        return value

    dependencies = replace(dependencies, runtime_binding=runtime_binding)
    result = runner.run_development(
        tmp_path,
        dependencies=dependencies,
        continuation_permission_sha256=_digest(
            "fresh-explicit-continuation-permission"
        ),
    )
...
def test_continuation_authorized_precedes_attempt_open_binding(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    order = _continuation_order_trace(monkeypatch, tmp_path)
    assert order.index("authenticate_continuation") < order.index(
        "continuation_authorized"
    ) < order.index("attempt_open")


def test_continuation_attempt_open_precedes_model_continuation_pre(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    order = _continuation_order_trace(monkeypatch, tmp_path)
    assert order.index("attempt_open") < order.index("model_continuation_pre")


def test_model_continuation_pre_precedes_first_ollama_intent(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    order = _continuation_order_trace(monkeypatch, tmp_path)
    assert order.index("model_continuation_pre") < order.index(
        "first_ollama_intent"
    )
...
def test_development_dependencies_complete_normal_route_without_external_effects(
    ...
    from agent_benchmark.sec_gemma_lean_science_v319_store import audit_attempt
    ...
    publication_count = len(trace["result_publications"])
    recovered = runner.recover_publication(tmp_path, dependencies=dependencies)
    assert recovered == result
    assert len(trace["result_publications"]) == publication_count
    from agent_benchmark.sec_gemma_lean_science_v319_store import AttemptStore

    with AttemptStore.open(
        tmp_path / contract.PRIVATE_DEVELOPMENT_NAMESPACE,
        authority=authority,
    ) as audited:
        assert audited.latest_recovery_terminal is not None
        assert (
            audited.latest_recovery_terminal.payload["target_kind"]
            == "terminal_bound"
        )
    runtime_count = len(trace["runtime_bindings"])
    repeated = runner.recover_publication(tmp_path, dependencies=dependencies)
    assert repeated == result
    assert len(trace["result_publications"]) == publication_count
    assert len(trace["runtime_bindings"]) == runtime_count


def test_production_route_ignores_configured_custom_yahoo_fetcher(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    ...
    def forbidden_custom_fetcher(_url: str):
        raise AssertionError(
            "a configured test fetcher cannot authorize production transport"
        )

    production_dependencies = replace(
        dependencies, yahoo_fetch=forbidden_custom_fetcher
    )
    production_calls: list[tuple[str, float]] = []

    def production_fetch(
        url: str, *, deadline_monotonic: float
    ) -> tuple[bytes, dict[str, Any]]:
        production_calls.append((url, deadline_monotonic))
        return b'{"synthetic":"production-deadline-path"}', {
            "network_requests": 1,
            "retries": 0,
            "redirects": 0,
        }
    ...
    monkeypatch.setattr(
        runner,
        "build_production_development_dependencies",
        lambda: production_dependencies,
    )
    monkeypatch.setattr(runner, "fetch_yahoo_once", production_fetch)

    result = runner.run_development(tmp_path)

    assert result["status"] == "passed"
    assert [url for url, _deadline in production_calls] == list(
        contract.YAHOO_URLS
    )
    assert all(type(deadline) is float for _url, deadline in production_calls)
    assert trace["yahoo"] == []
...
def test_development_dependencies_pause_after_five_without_opening_outputs(
    ...
    pause_publication_count = len(trace["pause_publications"])
    recovered = runner.recover_publication(tmp_path, dependencies=dependencies)
    assert recovered == result
    assert len(trace["pause_publications"]) == pause_publication_count
    from agent_benchmark.sec_gemma_lean_science_v319_store import AttemptStore
    ...
    with AttemptStore.open(
        ...
        assert audited.latest_recovery_terminal is not None
        assert audited.latest_recovery_terminal.payload["target_kind"] == "pause"
    runtime_count = len(trace["runtime_bindings"])
    repeated = runner.recover_publication(tmp_path, dependencies=dependencies)
    assert repeated == result
    assert len(trace["pause_publications"]) == pause_publication_count
...
def test_durable_pause_requires_exact_artifact_and_republishes_only_when_missing(
    ...
    pause_reopen_calls: list[Path] = []

    def swapped_pause_on_path_reopen(self: Path) -> bytes:
        if self == pause_path:
            pause_reopen_calls.append(self)
            return b'{"attacker":"swap-restored-before-final-lstat"}'
        return original_read_bytes(self)

    monkeypatch.setattr(Path, "read_bytes", swapped_pause_on_path_reopen)
    second = runner.run_development(tmp_path, dependencies=dependencies)
    assert second == first
    assert pause_reopen_calls == []
    assert trace["pause_publications"] == [first]
...
def test_pause_candidate_recovery_binds_before_publish_and_restarts_verification_only(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    from agent_benchmark.sec_gemma_lean_science_v319_journal import replay_journal
    from agent_benchmark.sec_gemma_lean_science_v319_store import AttemptStore
    ...
    original_commit_pause = AttemptStore.commit_planned_pause
    pause_crash = {"pending": True}

    def crash_before_pause_event(self: Any, **kwargs: Any) -> Any:
        if pause_crash["pending"]:
            pause_crash["pending"] = False
            raise KeyboardInterrupt("crash before bound pause event")
        return original_commit_pause(self, **kwargs)

    monkeypatch.setattr(
        AttemptStore, "commit_planned_pause", crash_before_pause_event
    )
    with pytest.raises(KeyboardInterrupt, match="before bound pause event"):
        runner.run_development(tmp_path, dependencies=dependencies)
    assert trace["pause_publications"] == []

    original_finish = AttemptStore.finish_recovery
    finish_crash = {"pending": True}

    def crash_after_pause_publication(self: Any, **kwargs: Any) -> Any:
        if finish_crash["pending"]:
            finish_crash["pending"] = False
            raise KeyboardInterrupt("crash after bound pause publication")
        return original_finish(self, **kwargs)

    monkeypatch.setattr(AttemptStore, "finish_recovery", crash_after_pause_publication)
    with pytest.raises(KeyboardInterrupt, match="after bound pause publication"):
        runner.recover_publication(tmp_path, dependencies=dependencies)
    assert len(trace["pause_publications"]) == 1

    pause = runner.recover_publication(tmp_path, dependencies=dependencies)
    assert pause["status"] == "paused_for_justification"
    assert len(trace["pause_publications"]) == 1
    runtime_count = len(trace["runtime_bindings"])
    assert runner.recover_publication(tmp_path, dependencies=dependencies) == pause
    assert len(trace["pause_publications"]) == 1
    assert len(trace["runtime_bindings"]) == runtime_count

    recovery = replay_journal(
        tmp_path
        / contract.PRIVATE_DEVELOPMENT_NAMESPACE
        / "publication_recovery_journal",
        authority_sha256=contract.canonical_sha256(authority),
    )
    assert [
        event.payload["target_kind"]
    ...
    terminals = [
        event
        for event in recovery.events
        if event.event_type == "recovery_terminal"
    ]
    assert len(terminals) == 1
    assert terminals[0].payload["ordinal"] == 2
...
def test_continuation_document_and_git_blobs_are_bound_to_exact_bytes(
    ...
    original_read_bytes = Path.read_bytes
    continuation_reopen_calls: list[Path] = []

    def swapped_continuation_on_path_reopen(self: Path) -> bytes:
        if self == continuation_path:
            continuation_reopen_calls.append(self)
            return expected + b"attacker-swap"
        return original_read_bytes(self)

    monkeypatch.setattr(
        Path, "read_bytes", swapped_continuation_on_path_reopen
    )
    ...
    assert continuation_reopen_calls == []
    monkeypatch.setattr(Path, "read_bytes", original_read_bytes)
    assert receipt == {
...
def test_full_synthetic_pause_restart_binds_exact_continuation_commit(
    ...
    assert not (tmp_path / f"{contract.RESULT_ARTIFACT_PATH}.v319-pending").exists()
    assert not (tmp_path / f"{contract.COMPARISON_PATH}.v319-pending").exists()

    restart_permission_sha256 = _digest(
        "fresh-explicit-continuation-restart-permission"
    )
    result = runner.run_development(
        tmp_path,
        dependencies=dependencies,
        continuation_permission_sha256=restart_permission_sha256,
    )
    assert result["status"] == "passed"
    assert result["terminal_store_status"] == "completed"
    ...
    assert "yahoo_body_bytes" not in result["effect_report"]
    assert b"yahoo_body_bytes" not in contract.canonical_json_bytes(result)
    assert trace["authenticate_descendant"] == [False, True, True, True, True]
    ...
    assert (
        completed_snapshot.continuation_permission_sha256
        == restart_permission_sha256
    )
    result_publication_count = len(trace["result_publications"])
    assert runner.recover_publication(tmp_path, dependencies=dependencies) == result
    assert len(trace["result_publications"]) == result_publication_count
    from agent_benchmark.sec_gemma_lean_science_v319_store import AttemptStore
    ...
    with AttemptStore.open(
        ...
        terminals = audited.recovery_terminals()
        assert [item.target_kind for item in terminals] == [
            "pause",
            "terminal_bound",
        ]
        intents = audited.recovery_intents()
        assert intents[-1].previous_recovery_terminal_sha256 == (
            terminals[0].recovery_terminal_sha256
        )


def test_paused_resumed_local_failure_after_restart_is_retryable_and_unpublished(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    from agent_benchmark.sec_gemma_lean_science_v319_store import audit_attempt
    ...
    _assert_code(
        "development_worker_failed",
        runner.run_development,
        tmp_path,
        dependencies=dependencies,
        continuation_permission_sha256=_digest(
            "fresh-failure-route-restart-permission"
        ),
    )
    assert trace["result_publications"] == []
    assert [
        item
        for item in trace["runtime_bindings"]
        if item == ("evaluation_pre", "continuation")
    ] == [("evaluation_pre", "continuation")]
    assert not any(
        checkpoint == "evaluation_post"
        for checkpoint, _kind in trace["runtime_bindings"]
    )
    ...
    assert snapshot.terminal_code is None
...
def test_development_dependencies_terminalize_first_transport_failure_once(
    ...
    assert interrupted.terminal_code == "request_outcome_unknown"
    assert interrupted.yahoo_intent_count == 1
    assert interrupted.yahoo_response_count == 0
    assert trace["result_publications"] == []

    result = runner.recover_publication(
        tmp_path, dependencies=dependencies
    )

    assert result["status"] == "indeterminate"
    assert result["terminal_store_status"] == "indeterminate"
    assert result["terminal_code"] == "request_outcome_unknown"
    ...
    assert trace["result_publications"] == [result]
    assert trace["runtime_bindings"][-3:] == [
        ("publication_recovery_pre", "publication_recovery"),
        ("publication_recovery_pre", "publication_recovery"),
        ("publication_recovery_pre", "publication_recovery"),
    ]
    snapshot = audit_attempt(
    ...
    assert snapshot.terminal_code == "request_outcome_unknown"
    assert snapshot.yahoo_intent_count == 1
    assert snapshot.yahoo_response_count == 0
    from agent_benchmark.sec_gemma_lean_science_v319_journal import (
        replay_journal,
    )
    ...
    assert len(trace["result_publications"]) == publication_count
    assert len(trace["runtime_bindings"]) == runtime_count
    repeated_recovery = replay_journal(
        tmp_path
        / contract.PRIVATE_DEVELOPMENT_NAMESPACE
        / "publication_recovery_journal",
        authority_sha256=contract.canonical_sha256(authority),
    )
    assert repeated_recovery.events == recovery.events


def test_publication_recovery_survives_every_private_crash_and_demotes_unbound_candidate(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    from agent_benchmark.sec_gemma_lean_science_v319_journal import replay_journal
    from agent_benchmark.sec_gemma_lean_science_v319_store import AttemptStore
    ...
    original_persist = AttemptStore.persist_terminal_candidate
    persist_crash = {"pending": True}

    def crash_before_candidate_persistence(self: Any, candidate: Any) -> Any:
        if persist_crash["pending"]:
            persist_crash["pending"] = False
            raise KeyboardInterrupt("crash before candidate persistence")
        return original_persist(self, candidate)

    monkeypatch.setattr(
        AttemptStore,
        "persist_terminal_candidate",
        crash_before_candidate_persistence,
    )
    with pytest.raises(KeyboardInterrupt, match="before candidate persistence"):
        runner.recover_publication(tmp_path, dependencies=dependencies)

    original_append_terminal = AttemptStore.append_attempt_terminal

    def crash_before_terminal(self: Any, **_kwargs: Any) -> Any:
        if self.terminal_candidate_abandoned:
            raise KeyboardInterrupt("crash after candidate abandonment")
        raise KeyboardInterrupt("crash after candidate persistence")

    monkeypatch.setattr(
        AttemptStore, "append_attempt_terminal", crash_before_terminal
    )
    with pytest.raises(KeyboardInterrupt, match="after candidate persistence"):
        runner.recover_publication(tmp_path, dependencies=dependencies)
    with pytest.raises(KeyboardInterrupt, match="after candidate abandonment"):
        runner.recover_publication(tmp_path, dependencies=dependencies)
    ...
    def crash_after_publication(self: Any, **kwargs: Any) -> Any:
        if finish_crash["pending"]:
            finish_crash["pending"] = False
            raise KeyboardInterrupt("crash after exact publication")
        return original_finish(self, **kwargs)

    monkeypatch.setattr(AttemptStore, "finish_recovery", crash_after_publication)
    with pytest.raises(KeyboardInterrupt, match="after exact publication"):
        runner.recover_publication(tmp_path, dependencies=dependencies)
    assert len(trace["result_publications"]) == 1
    comparison_path = tmp_path / Path(contract.COMPARISON_PATH)
    assert comparison_path.read_bytes() != dependencies.read_comparison(tmp_path)

    result = runner.recover_publication(tmp_path, dependencies=dependencies)
    assert result["status"] == "indeterminate"
    assert result["terminal_code"] == "unbound_terminal_candidate"
    assert len(trace["result_publications"]) == 1
    runtime_count = len(trace["runtime_bindings"])
    repeated = runner.recover_publication(tmp_path, dependencies=dependencies)
    assert repeated == result
    ...
    intents = [
        event for event in recovery.events if event.event_type == "recovery_intent"
    ]
    terminals = [
        event
        for event in recovery.events
    ...
    assert [event.payload["ordinal"] for event in intents] == [1, 2, 3, 4, 5]
    assert [event.payload["target_kind"] for event in intents] == [
    ...
    assert intents[0].payload["candidate_sha256"] == intents[1].payload[
        "candidate_sha256"
    ]
    assert {
        (
            event.payload["public_artifact_sha256"],
            event.payload["public_artifact_bytes"],
            event.payload["comparison_before_sha256"],
            event.payload["comparison_after_sha256"],
        )
        for event in intents[2:]
    } == {
        (
            intents[2].payload["public_artifact_sha256"],
            intents[2].payload["public_artifact_bytes"],
            intents[2].payload["comparison_before_sha256"],
            intents[2].payload["comparison_after_sha256"],
        )
    }
    assert len(terminals) == 1
    assert terminals[0].payload["ordinal"] == 5
    assert terminals[0].payload["target_kind"] == "terminal_demoted_bound"
    assert terminals[0].payload["outcome"] == "published"
    with AttemptStore.open(
        tmp_path / contract.PRIVATE_DEVELOPMENT_NAMESPACE,
        authority=authority,
    ) as audited:
        candidate = audited.terminal_candidate
        assert candidate is not None
        assert result["terminal_material_sha256"] != (
            candidate.terminal_material_sha256
        )
    result_bytes = contract.canonical_json_bytes(result)
    assert contract.sha256_bytes(result_bytes) == intents[2].payload[
        "public_artifact_sha256"
    ]
    assert contract.sha256_bytes(comparison_path.read_bytes()) == intents[
        2
    ].payload["comparison_after_sha256"]


def test_regular_file_bytes_never_reopens_path_under_swap_restore_adversary(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """A checked path must be read only through its already-open descriptor."""

    path = tmp_path / "authenticated-publication.json"
    original = b"authenticated-original-bytes"
    attacker = b"same-length-attacker-content"
    assert len(attacker) == len(original)
    path.write_bytes(original)
    original_read_bytes = Path.read_bytes
    reopen_calls: list[Path] = []

    def swap_restore_on_reopen(self: Path) -> bytes:
        if self == path:
            # Model the classic race: the attacker swaps the pathname after
            # lstat, a vulnerable pathname read opens the other inode, and the
            # original pathname is restored before the final lstat.
            reopen_calls.append(self)
            return attacker
        return original_read_bytes(self)

    monkeypatch.setattr(Path, "read_bytes", swap_restore_on_reopen)
    assert runner._regular_file_bytes(
        path, code="publication_recovery_artifact_invalid"
    ) == original
    runner._assert_exact_regular_file(
        path, original, code="public_artifact_unwritable"
    )
    assert reopen_calls == []


def test_regular_file_bytes_rejects_changed_path_identity_after_descriptor_read(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    path = tmp_path / "authenticated-publication.json"
    replacement = tmp_path / "replacement-publication.json"
    path.write_bytes(b"authenticated-original-bytes")
    replacement.write_bytes(b"different-replacement-bytes")
    original_lstat = Path.lstat
    target_calls = {"count": 0}

    def changed_identity_on_final_lstat(self: Path):
        if self == path:
    ...
    monkeypatch.setattr(Path, "lstat", changed_identity_on_final_lstat)
    _assert_code(
        "publication_recovery_artifact_invalid",
        runner._regular_file_bytes,
        path,
        code="publication_recovery_artifact_invalid",
    )


def test_result_publisher_reads_comparison_through_stable_descriptor(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    comparison_base = (
        b"| Approach | Information | Development | LLM | Decision |\n"
    ...
    comparison_path = tmp_path / Path(contract.COMPARISON_PATH)
    comparison_path.parent.mkdir(parents=True, exist_ok=True)
    comparison_path.write_bytes(comparison_base)
    result = {
    ...
    monkeypatch.setattr(
        runner,
        "_git_bytes",
        lambda root, *arguments: comparison_base,
    )
    original_read_bytes = Path.read_bytes
    comparison_reopen_calls: list[Path] = []

    def swapped_comparison_on_path_reopen(self: Path) -> bytes:
        if self == comparison_path:
            comparison_reopen_calls.append(self)
            return b"attacker comparison restored before final lstat\n"
        return original_read_bytes(self)

    monkeypatch.setattr(Path, "read_bytes", swapped_comparison_on_path_reopen)
    runner.publish_result_artifacts(tmp_path, result)
    assert comparison_reopen_calls == []
    expected_after = runner.build_comparison_update(comparison_base, result)
    assert runner._regular_file_bytes(
        comparison_path, code="comparison_unreadable"
    ) == expected_after
    result_path = tmp_path / Path(contract.RESULT_ARTIFACT_PATH)
    assert runner._regular_file_bytes(
        result_path, code="public_artifact_unwritable"
...
def test_development_consumes_only_pushed_gated_authority(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    from agent_benchmark import sec_gemma_lean_science_v319_preflight as preflight

    authority = {"schema_version": "pushed-f313-authority"}
    order: list[str] = []

    def gate(root: Path) -> None:
        assert root == tmp_path
        order.append("pushed_f313_gate")

    def load(root: Path) -> dict[str, str]:
        assert root == tmp_path
        order.append("load_pushed_authority")
        return authority

    monkeypatch.setattr(preflight, "authenticate_execution_preflight", gate)
    monkeypatch.setattr(preflight, "load_execution_authority", load)
    monkeypatch.setattr(
        preflight,
    ...
    monkeypatch.setattr(
        ...
        lambda _root: (_ for _ in ()).throw(
            AssertionError("development must not consume recovery authority")
        ),
    )
    dependencies = runner.build_production_development_dependencies()
    assert dependencies.authenticate_execution(tmp_path, False) is authority
    assert order == ["pushed_f313_gate", "load_pushed_authority"]

    order.clear()

    def rejected_gate(_root: Path) -> None:
        order.append("pushed_f313_gate")
        raise RuntimeError("synthetic gate rejection")

    monkeypatch.setattr(preflight, "authenticate_execution_preflight", rejected_gate)
    blocked = runner.build_production_development_dependencies()
    with pytest.raises(RuntimeError, match="synthetic gate rejection"):
        blocked.authenticate_execution(tmp_path, False)
    assert order == ["pushed_f313_gate"]


def test_pushed_result_gate_uses_candidate_material_schema_with_plan_equals_plan(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from agent_benchmark import sec_gemma_lean_science_v319_preflight as preflight

    plan = SimpleNamespace(identity=object())
    projection = object()
    authority = {"schema_version": "identity-spy-authority"}
    expected_plan = plan
    deterministic = {"schema_version": "identity-spy-deterministic"}
    public_summary = {"gate_report": {"passed": True, "failed_checks": []}}
    calls: list[str] = []

    evidence = {"schema_version": "identity-spy-private-material"}
    terminal_material_sha256 = _digest("identity-spy-terminal-material")
    result = {
        "terminal_store_status": "indeterminate",
        "terminal_code": "request_outcome_unknown",
    ...
    result_bytes = contract.canonical_json_bytes(result)
    comparison_before = b"# identity-spy comparison\n"
    comparison_after = b"# identity-spy comparison\nrecovered\n"
    candidate_body = {
        "schema_version": (
            "aapl-sec-gemma-lean-science-v3-19-"
            "terminal-evidence-candidate-v1"
        ),
        "authority_sha256": _digest("identity-spy-authority"),
        "stage": "development",
        "attempt_id": contract.DEVELOPMENT_ATTEMPT_ID,
        "candidate_sequence": 1,
        "journal_head_before_candidate_sha256": _digest(
            "identity-spy-candidate-head"
        ),
        "proposed_status": "indeterminate",
        "proposed_terminal_code": "request_outcome_unknown",
        "evidence": evidence,
        "terminal_material_sha256": terminal_material_sha256,
        "public_result_sha256": contract.sha256_bytes(result_bytes),
        "public_result_bytes": len(result_bytes),
        "comparison_before_sha256": contract.sha256_bytes(
            comparison_before
        ),
    ...
    candidate = SimpleNamespace(
        ...
        public_result_sha256=contract.sha256_bytes(result_bytes),
        public_result_bytes=len(result_bytes),
        comparison_before_sha256=contract.sha256_bytes(comparison_before),
        comparison_after_sha256=contract.sha256_bytes(comparison_after),
        comparison_after_bytes=len(comparison_after),
        evidence=evidence,
        candidate=candidate_mapping,
        public_result=result_bytes,
        comparison_before=comparison_before,
        comparison_after=comparison_after,
    )
    receipt = SimpleNamespace(
        **candidate.__dict__,
        event_sha256=_digest("identity-spy-terminal-event"),
        status="indeterminate",
        terminal_code="request_outcome_unknown",
    )
    snapshot = SimpleNamespace(
        status="indeterminate",
        terminal_code="request_outcome_unknown",
        journal_head_sha256=receipt.event_sha256,
    )
    assert preflight._validate_pushed_terminal_candidate_binding(
        result=result,
        snapshot=snapshot,
        receipt=receipt,
        candidate=candidate,
        committed_result_bytes=result_bytes,
        parent_comparison_bytes=comparison_before,
        committed_comparison_bytes=comparison_after,
    ) == evidence

    def commitments(candidate: object) -> dict[str, str]:
        assert candidate is projection
        calls.append("commitments")
        return {"sha256": _digest("identity-spy-commitments")}

    def model_plan(candidate: object) -> object:
        assert candidate is projection
        calls.append("plan")
        return plan

    def attempt_authority(**kwargs: object) -> dict[str, str]:
        assert kwargs["execution_authority"] is authority
        assert kwargs["projection"] is projection
        assert kwargs["plan"] is plan
        calls.append("authority")
        return authority

    def runtime_evidence(_store: object, *, plan: object):
        assert plan is expected_plan
        calls.append("runtime_evidence")
        return ({"guard": True}, {"aggregate": True}, {"latency": True}, {})

    def stage_slice(_store: object, *, plan: object) -> dict[str, bool]:
        assert plan is expected_plan
        calls.append("stage_slice")
        return {"stage": True}

    def generation_batch(_store: object, *, plan: object) -> dict[str, object]:
        assert plan is expected_plan
        calls.append("generation_batch")
        return {}

    def semantic_payload(**kwargs: object) -> dict[str, bool]:
        assert kwargs["plan"] is expected_plan
        calls.append("semantic")
        return {"semantic": True}

    def deterministic_payload(**kwargs: object) -> dict[str, str]:
        assert kwargs["plan"] is expected_plan
        calls.append("deterministic")
        return deterministic

    def private_material(**kwargs: object) -> dict[str, object]:
        assert kwargs["plan"] is expected_plan
        assert kwargs["deterministic_payload"] is deterministic
        calls.append("material")
        return {"science_summary": public_summary}

    monkeypatch.setattr(runner, "build_preflight_request_commitments", commitments)
    monkeypatch.setattr(runner, "build_model_plan", model_plan)
    monkeypatch.setattr(runner, "build_attempt_authority", attempt_authority)
    monkeypatch.setattr(runner, "rebuild_runtime_evidence", runtime_evidence)
    monkeypatch.setattr(runner, "_open_stage_slice", stage_slice)
    monkeypatch.setattr(runner, "_open_generation_batch", generation_batch)
    monkeypatch.setattr(runner, "build_semantic_payload", semantic_payload)
    monkeypatch.setattr(runner, "build_deterministic_payload", deterministic_payload)
    monkeypatch.setattr(
        runner,
        "evaluate_deterministic_science",
        lambda **_kwargs: {"evaluation": {"gate_report": {"passed": True}}},
    )
    monkeypatch.setattr(
        runner, "build_public_science_summary", lambda _evaluation: public_summary
    )
    monkeypatch.setattr(runner, "build_private_terminal_material", private_material)

    dependencies = preflight.build_production_pushed_result_dependencies()
    context = dependencies.rebuild_attempt_context(authority, projection)
    assert context["authority"] is authority
    assert context["plan"] is plan
    replayed = dependencies.replay_completed_terminal(
        store=object(),
        authority=authority,
        projection=projection,
        plan=context["plan"],
        effect_report={},
        invocation_parent="a" * 40,
        invocation_parent_kind="preflight",
    )
    assert replayed == {"science_summary": public_summary}
    assert calls == [
        "commitments",
        "plan",
        "authority",
...
def test_status_reads_only_safe_snapshot_and_fails_closed(
    ...
    from agent_benchmark import sec_gemma_lean_science_v319_preflight as preflight
    from agent_benchmark import sec_gemma_lean_science_v319_store as store
...
def test_cli_emits_canonical_safe_result_or_fixed_rejection(
    ...
    monkeypatch.setattr(
        runner,
        "status",
        lambda root: (_ for _ in ()).throw(runner.V319RunnerError("fixed_rejection")),
    )
    ...
    monkeypatch.setattr(
        runner, "recover_publication", lambda root: recovered
    )
    assert (
        runner.main(
            ["recover-publication", "--repo-root", str(tmp_path)]
        )
        == 0
    )
    assert capsys.readouterr().out == (
        contract.canonical_json_bytes(recovered).decode("utf-8") + "\n"
    )
    assert (
        runner.main(
            [
                ...
                "--repo-root",
                str(tmp_path),
                "--continuation-permission-sha256",
                "a" * 64,
            ]
        )
        == 2
    ...
    assert json.loads(capsys.readouterr().out) == {
        "status": "rejected",
        "code": "recovery_permission_argument_forbidden",
    }
...
def test_comparison_update_preserves_prior_rows_and_appends_v319_after_v318() -> None:
    v39_row = (
        "| SEC/Gemma lean science v3.9 | Frozen donor | Rejected | No | "
        "Preserved |"
    )
    v310_row = (
        "| SEC/Gemma lean science v3.10 | Frozen donor | Rejected | No | "
        "Preserved |"
    )
    v311_row = (
        "| SEC/Gemma lean science v3.11 | Frozen donor | Rejected | No | "
        "Preserved |"
    )
    v315_row = (
        "| SEC/Gemma lean science v3.15 | Frozen donor | Rejected | No | "
        "Preserved |"
    )
    v316_row = (
        "| SEC/Gemma lean science v3.16 | Frozen donor | Rejected | No | "
        "Preserved |"
    )
    v317_row = (
        "| SEC/Gemma lean science v3.17 | Frozen donor | Rejected | No | "
        "Preserved |"
    )
    v318_row = (
        "| SEC/Gemma lean science v3.18 | Frozen donor | Rejected | No | "
        "Preserved |"
    )
    existing = (
        ...
        f"{v317_row}\n"
        f"{v318_row}\n"
        "| Later unrelated row | Frozen | Frozen | No | Frozen |\n"
    ).encode("utf-8")
    result = {
        "status": "failed_gate",
        "failed_gate_count": 1,
        "terminal_code": "failed_gate",
    }
    updated = runner.build_comparison_update(existing, result)
    lines = updated.decode("utf-8").splitlines()
    assert lines[:9] == [
        "| Approach | Design | Development | Real money | Decision |",
        "|---|---|---|---|---|",
        v39_row,
    ...
    v318_index = lines.index(v318_row)
    assert lines[v318_index + 1].startswith(
        "| SEC/Gemma lean science v3.19 |"
    )
    assert lines[v318_index + 2] == (
        "| Later unrelated row | Frozen | Frozen | No | Frozen |"
    )
    assert runner.build_comparison_update(updated, result) == updated

    _assert_code(
        "comparison_marker_invalid",
        runner.build_comparison_update,
        existing.replace(v318_row.encode("utf-8"), b"| Missing marker |"),
        result,
    )
