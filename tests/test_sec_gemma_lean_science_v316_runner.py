...
from agent_benchmark import sec_gemma_lean_science_v316_contract as contract
from agent_benchmark import sec_gemma_lean_science_v316_runner as runner
...
def _assert_code(code: str, function: Any, /, *args: Any, **kwargs: Any) -> None:
    with pytest.raises(runner.V316RunnerError) as caught:
...
def test_real_bridge_preflight_receipt_and_attempt_authority_seam(
    ...
    from agent_benchmark import sec_gemma_lean_science_v316_bridge as bridge
    from agent_benchmark import sec_gemma_lean_science_v316_preflight as preflight
    from tests.test_sec_gemma_lean_science_v316_bridge import _make_fixture
    from tests.test_sec_gemma_lean_science_v316_preflight import (
        _qualification,
...
def test_deterministic_payload_is_reproducible_with_pure_dependencies(
    ...
    assert source["v316_source_commitments_sha256"] == contract.canonical_sha256(
        {key: value for key, value in source.items() if key != "v316_source_commitments_sha256"}
    )
...
def test_real_bridge_payloads_evaluator_and_terminal_material_replay_offline() -> None:
    ...
    # V3.16 model-plan builder.  No SEC, Yahoo, Ollama, or other transport is
...
def test_public_privacy_scanner_detects_json_escaped_windows_path() -> None:
    private_windows_path = r"C:\Users\example\private\v3_16"
...
def test_complete_private_namespace_allows_qualification_paths_but_rejects_secrets(
    ...
    preflight_mode = (
        private_root / "preflight" / "qualification" / "v316" / "execution"
    )
    ...
    v316_private = str(private_root.resolve(strict=False))
    forbidden_tokens = (
        contact,
        str(root),
        str(root).replace("\\", "/"),
    ...
    runner._assert_complete_v316_private_namespace_private_free(
        store,
...
def _install_fast_orchestration_store(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from agent_benchmark import sec_gemma_lean_science_v316_journal as journal_module
    from agent_benchmark import sec_gemma_lean_science_v316_store as store_module
    ...
    def fast_journal_append(
        ...
        if (
            ...
            raise journal_module.V316JournalError("journal_event_invalid")
        normalized = dict(payload)
        journal_module.canonical_json_bytes(normalized)
        events = getattr(self, "_v316_test_events", None)
        if events is None:
            events = list(self.replay().events)
            self._v316_test_events = events
        ...
        if len(encoded) > journal_module.MAX_EVENT_BYTES:
            raise journal_module.V316JournalError("journal_event_too_large")
        final = self.root / f"{sequence:08d}-{event_sha256}.json"
        try:
            with final.open("xb") as handle:
                handle.write(encoded)
        except OSError:
            raise journal_module.V316JournalError(
    ...
    def fast_refresh(self: Any) -> None:
        if not getattr(self, "_v316_test_fast_refresh_ready", False):
            original_refresh(self)
            self._v316_test_fast_refresh_ready = True
            return

        self._require_open()
        journal = self._journal
        if journal is None:
            raise store_module.V316StoreError("store_not_initialized")
        events = getattr(journal, "_v316_test_events", None)
        if events is None:
            events = list(journal.replay().events)
            journal._v316_test_events = events
...
def _development_test_dependencies(
    ...
    *,
    ...
    from agent_benchmark import sec_gemma_lean_science_v316_journal as journal_module
    from agent_benchmark import sec_gemma_lean_science_v316_store as store_module
    from agent_benchmark.sec_gemma_lean_science_v316_store import AttemptStore
    ...
    comparison_before = (
        ...
        b"| SEC/Gemma lean science v3.15 | frozen predecessor | No | No | Preserved |\n"
    )
    invocation_commits = {
...
def test_development_dependencies_complete_normal_route_without_external_effects(
    ...
    from agent_benchmark.sec_gemma_lean_science_v316_store import audit_attempt
...
def test_pause_candidate_recovery_binds_before_publish_and_restarts_verification_only(
    ...
    from agent_benchmark.sec_gemma_lean_science_v316_journal import replay_journal
    from agent_benchmark.sec_gemma_lean_science_v316_store import AttemptStore
...
def _expected_continuation_document(pause: dict[str, Any]) -> bytes:
    ...
    return (
        "# AAPL SEC/Gemma lean science v3.16 continuation preregistration\n\n"
...
def test_full_synthetic_pause_restart_binds_exact_continuation_commit(
    ...
    assert not (tmp_path / f"{contract.RESULT_ARTIFACT_PATH}.v316-pending").exists()
    assert not (tmp_path / f"{contract.COMPARISON_PATH}.v316-pending").exists()
...
def test_development_dependencies_terminalize_first_transport_failure_once(
    ...
    from agent_benchmark.sec_gemma_lean_science_v316_journal import (
        replay_journal,
    )
...
def test_pushed_result_gate_uses_candidate_material_schema_with_plan_equals_plan(
    ...
    candidate_body = {
        "schema_version": (
            "aapl-sec-gemma-lean-science-v3-16-"
...
def test_status_reads_only_safe_snapshot_and_fails_closed(
    ...
    from agent_benchmark import sec_gemma_lean_science_v316_preflight as preflight
    from agent_benchmark import sec_gemma_lean_science_v316_store as store
...
def test_cli_emits_canonical_safe_result_or_fixed_rejection(
    ...
    monkeypatch.setattr(
        runner,
        "status",
        lambda root: (_ for _ in ()).throw(runner.V316RunnerError("fixed_rejection")),
    )
...
def test_comparison_update_preserves_prior_rows_and_appends_v316_after_v315() -> None:
    v39_row = (
    ...
    v315_index = lines.index(v315_row)
    assert lines[v315_index + 1].startswith(
        "| SEC/Gemma lean science v3.16 |"
    )
    assert lines[v315_index + 2] == (
        "| Later unrelated row | Frozen | Frozen | No | Frozen |"
    )
    ...
    _assert_code(
        ...
        existing.replace(v315_row.encode("utf-8"), b"| Missing marker |"),
        result,
    )
