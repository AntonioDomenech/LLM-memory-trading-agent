...
import agent_benchmark.sec_gemma_lean_v31_acquisition as acquisition_module
import agent_benchmark.sec_gemma_lean_v31_journal as journal_module
from agent_benchmark.sec_gemma_lean_v31_acquisition import (
...
from agent_benchmark.sec_gemma_lean_v31_journal import PrivateSecContact
...
class _FakeSource:
    ...
    def build_compact_checkpoint(self, **kwargs: object) -> dict[str, object]:
        ...
        body = {
            ...
            "main_parse_receipt_sha256": manifests[0][
    ...
    def rehydrate_compact_stage(
        ...
        for manifest in role_manifests:
            ...
            assert hashlib.sha256(payload).hexdigest() == manifest["body_sha256"]
            del payload
        receipt = {
...
def test_file_lock_conflict_is_nonblocking_and_body_oserror_is_preserved(
    ...
    with acquisition_module._locked_file(
        ...
        with pytest.raises(SecGemmaLeanV31AcquisitionError) as caught:
            ledger.dispatch(
                purpose="conflict",
                first_in_invocation=True,
...
def test_import_is_side_effect_free_and_exact_names_are_exported() -> None:
    assert ROOT_NAMESPACE == "aapl_sec_gemma_lean_evidence_v3_1"
    ...
    completed = subprocess.run(
        [
            ...
            "-B",
            "-c",
            "import agent_benchmark.sec_gemma_lean_v31_acquisition",
        ],
        cwd=repository,
        check=False,
        capture_output=True,
...
def test_production_development_binds_closure_then_rechecks_and_wires_runner(
    ...
    def load_closure() -> dict[str, object]:
        ...
        return {"agent_benchmark.sec_gemma_lean_v31_source": source_adapter}
    ...
    def bind_closure(
        ...
        *,
        ...
        assert loaded_modules == {
            "agent_benchmark.sec_gemma_lean_v31_source": source_adapter
        }
...
def test_production_gate_rejects_minimal_forged_preflight_via_strict_validator(
    ...
    def reject_forgery(root: Path) -> dict[str, object]:
        ...
        raise preflight.SecGemmaLeanV31PreflightError(
...
def test_pre_dispatch_deadline_gates_roll_invocations_and_stop_before_stage_cap(
    ...
    assert 1 < len(transports[0].calls) < 99
...
def test_complete_fake_acquisition_restarts_without_resend_and_rejects_orphan(
    ...
    assert len(transports[0].calls) == 99
    assert len(set(transports[0].calls)) == 99
    assert len(promotions) == 99
    assert all(
    ...
    assert result.journal_state.formula_requests == 99
    assert result.journal_state.lifetime_intents == 99
    assert result.journal_state.http_200_responses == 99
    assert result.journal_state.role_seals == 99
    ...
    with pytest.raises(SecGemmaLeanV31AcquisitionError):
        restarted.run()
...
def test_full_size_three_stage_orchestrator_rehearsal_uses_disk_prior_rehydrate(
    ...
    assert [len(transport.calls) for transport in transports] == [243, 159, 162]
    assert all(
    ...
    assert len(final.blob_paths) == 243 + 159 + 162
    assert all(
