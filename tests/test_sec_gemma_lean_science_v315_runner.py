...
def test_real_bridge_preflight_receipt_and_attempt_authority_seam(
    ...
    from agent_benchmark import sec_gemma_lean_science_v315_preflight as preflight
    from tests.test_sec_gemma_lean_science_v315_bridge import _make_fixture
    from tests.test_sec_gemma_lean_science_v315_preflight import (
        _qualification,
...
def test_deterministic_payload_is_reproducible_with_pure_dependencies(
    ...
    assert source["v315_source_commitments_sha256"] == contract.canonical_sha256(
        {key: value for key, value in source.items() if key != "v315_source_commitments_sha256"}
    )
...
def test_real_bridge_payloads_evaluator_and_terminal_material_replay_offline() -> None:
    ...
    # V3.15 model-plan builder.  No SEC, Yahoo, Ollama, or other transport is
...
def test_public_privacy_scanner_detects_json_escaped_windows_path() -> None:
    private_windows_path = r"C:\Users\example\private\v3_15"
...
def test_complete_private_namespace_allows_qualification_paths_but_rejects_secrets(
    ...
    preflight_mode = (
        private_root / "preflight" / "qualification" / "v315" / "execution"
    )
    ...
    v315_private = str(private_root.resolve(strict=False))
    forbidden_tokens = (
        contact,
        str(root),
        str(root).replace("\\", "/"),
    ...
    runner._assert_complete_v315_private_namespace_private_free(
        store,
...
def _install_fast_orchestration_store(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from agent_benchmark import sec_gemma_lean_science_v315_journal as journal_module
    ...
    def fast_journal_append(
        ...
        if (
            ...
            raise journal_module.V315JournalError("journal_event_invalid")
        normalized = dict(payload)
        journal_module.canonical_json_bytes(normalized)
        events = getattr(self, "_v315_test_events", None)
        if events is None:
            events = list(self.replay().events)
            self._v315_test_events = events
        ...
        if len(encoded) > journal_module.MAX_EVENT_BYTES:
            raise journal_module.V315JournalError("journal_event_too_large")
        final = self.root / f"{sequence:08d}-{event_sha256}.json"
        try:
            with final.open("xb") as handle:
                handle.write(encoded)
        except OSError:
            raise journal_module.V315JournalError(
    ...
    def fast_refresh(self: Any) -> None:
        if not getattr(self, "_v315_test_fast_refresh_ready", False):
            original_refresh(self)
            self._v315_test_fast_refresh_ready = True
            return

        self._require_open()
        journal = self._journal
        if journal is None:
            raise store_module.V315StoreError("store_not_initialized")
        events = getattr(journal, "_v315_test_events", None)
        if events is None:
            events = list(journal.replay().events)
            journal._v315_test_events = events
...
def _development_test_dependencies(
    ...
    *,
    ...
    from agent_benchmark import sec_gemma_lean_science_v315_journal as journal_module
    from agent_benchmark import sec_gemma_lean_science_v315_store as store_module
    from agent_benchmark.sec_gemma_lean_science_v315_store import AttemptStore
    ...
    comparison_before = (
        ...
        b"| SEC/Gemma lean science v3.14 | frozen predecessor | No | No | Preserved |\n"
    )
    invocation_commits = {
...
def test_development_dependencies_complete_normal_route_without_external_effects(
    ...
    from agent_benchmark.sec_gemma_lean_science_v315_store import audit_attempt
...
def test_pause_candidate_recovery_binds_before_publish_and_restarts_verification_only(
    ...
    from agent_benchmark.sec_gemma_lean_science_v315_journal import replay_journal
    from agent_benchmark.sec_gemma_lean_science_v315_store import AttemptStore
...
def _expected_continuation_document(pause: dict[str, Any]) -> bytes:
    ...
    return (
        "# AAPL SEC/Gemma lean science v3.15 continuation preregistration\n\n"
...
def test_full_synthetic_pause_restart_binds_exact_continuation_commit(
    ...
    assert not (tmp_path / f"{contract.RESULT_ARTIFACT_PATH}.v315-pending").exists()
    assert not (tmp_path / f"{contract.COMPARISON_PATH}.v315-pending").exists()
...
def test_pushed_result_gate_uses_candidate_material_schema_with_plan_equals_plan(
    ...
    candidate_body = {
        "schema_version": (
            "aapl-sec-gemma-lean-science-v3-15-"
...
def test_status_reads_only_safe_snapshot_and_fails_closed(
    ...
    from agent_benchmark import sec_gemma_lean_science_v315_preflight as preflight
    from agent_benchmark import sec_gemma_lean_science_v315_store as store
...
def test_cli_emits_canonical_safe_result_or_fixed_rejection(
    ...
    monkeypatch.setattr(
        runner,
        "status",
        lambda root: (_ for _ in ()).throw(runner.V315RunnerError("fixed_rejection")),
    )
...
def test_comparison_update_preserves_prior_rows_and_appends_v315_after_v314() -> None:
    v39_row = (
    ...
    v314_row = (
        "| SEC/Gemma lean science v3.14 | Frozen donor | Rejected | No | "
        "Preserved |"
    )
    existing = (
    ...
    v314_index = lines.index(v314_row)
    assert lines[v314_index + 1].startswith(
        "| SEC/Gemma lean science v3.15 |"
    )
    assert lines[v314_index + 2] == (
        "| Later unrelated row | Frozen | Frozen | No | Frozen |"
    )
    ...
    _assert_code(
        ...
        existing.replace(v314_row.encode("utf-8"), b"| Missing marker |"),
        result,
    )
