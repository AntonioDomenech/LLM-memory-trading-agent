...
import agent_benchmark.sec_gemma_lean_v32_transport as transport
...
def _error_code(payload: bytes, *, limit: int = 32) -> str:
    with pytest.raises(transport.SecGemmaLeanV32TransportError) as captured:
...
def test_receipt_is_deterministic_redacted_and_self_hashed(tmp_path: Path) -> None:
    ...
    with pytest.raises(transport.SecGemmaLeanV32TransportError):
