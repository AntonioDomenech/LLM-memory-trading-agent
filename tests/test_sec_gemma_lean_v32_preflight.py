...
def test_offline_identity_probe_is_exactly_tags_then_show_and_local_version() -> None:
    ...
    with pytest.raises(preflight.SecGemmaLeanV32PreflightError, match="forbidden"):
...
def test_production_trust_cannot_be_combined_with_injected_transport() -> None:
    ...
    with pytest.raises(preflight.SecGemmaLeanV32PreflightError, match="transport"):
...
def test_tags_identity_is_unique_and_exactly_digest_pinned(payload: bytes) -> None:
    with pytest.raises(preflight.SecGemmaLeanV32PreflightError, match="tags"):
...
def test_durable_intent_makes_crash_after_first_call_nonretryable(tmp_path: Path) -> None:
    ...
    def crash_after_first() -> dict[str, Any]:
        ...
        raise preflight.SecGemmaLeanV32PreflightError("runtime_identity_invalid")

    repository = {"head_commit": IMPLEMENTATION_COMMIT, "head_tree": IMPLEMENTATION_TREE}
    with pytest.raises(preflight.SecGemmaLeanV32PreflightError, match="runtime_identity"):
    ...
    with pytest.raises(preflight.SecGemmaLeanV32PreflightError, match="already_attempted"):
        preflight._durable_identity_probe(
            checkpoint,
            repository,
            contact_fingerprint=CONTACT_SHA256,
...
def test_repository_gate_rejects_wrong_dirty_unpushed_or_changed_state(
    ...
    with pytest.raises(preflight.SecGemmaLeanV32PreflightError, match=expected_code):
...
def _production_markers(
    ...
    nonce = "a" * 64
    intent_body = {
        "schema_version": "aapl-sec-gemma-lean-evidence-v3-2-identity-intent-v1",
    ...
    completion_body = {
        "schema_version": "aapl-sec-gemma-lean-evidence-v3-2-identity-complete-v1",
        "test_injections_used": False,
        "attempt_nonce_sha256": nonce,
...
def test_minimal_self_rehashed_forged_receipt_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(preflight.SecGemmaLeanV32PreflightError):
...
def test_extra_durable_field_and_noncanonical_artifact_bytes_reject(tmp_path: Path) -> None:
    ...
    _rehash(report)
    with pytest.raises(preflight.SecGemmaLeanV32PreflightError, match="untrusted"):
        preflight._validate_sealable_report(report)

    root = _repo(tmp_path / "encoding")
    report, manifest = _valid_report(root)
    _write_artifact(root, report, canonical=False)
    with pytest.raises(preflight.SecGemmaLeanV32PreflightError, match="committed_preflight"):
        preflight._validate_committed_preflight_for_acquisition(
            root, **_validator_kwargs(root, report, manifest)
        )
...
def test_altered_marker_delta_source_and_contact_each_reject(tmp_path: Path) -> None:
    ...
    with pytest.raises(preflight.SecGemmaLeanV32PreflightError, match="identity_probe"):
        preflight._validate_committed_preflight_for_acquisition(
            root, **_validator_kwargs(root, report, manifest)
        )
    ...
    _rehash(report)
    _write_artifact(root, report)
    with pytest.raises(preflight.SecGemmaLeanV32PreflightError, match="implementation_delta"):
        preflight._validate_committed_preflight_for_acquisition(
            root, **_validator_kwargs(root, report, manifest)
        )
    ...
    with pytest.raises(preflight.SecGemmaLeanV32PreflightError, match="executed_source"):
        preflight._validate_committed_preflight_for_acquisition(
            root,
...
def test_prereg_ancestor_and_final_git_contact_marker_rebounds_reject(tmp_path: Path) -> None:
    ...
    with pytest.raises(preflight.SecGemmaLeanV32PreflightError, match="preregistration"):
    ...
    with pytest.raises(preflight.SecGemmaLeanV32PreflightError, match="changed"):
        preflight._validate_committed_preflight_for_acquisition(root, **kwargs)
...
def test_publish_failure_leaves_orphan_unusable_and_publish_is_after_lease(
    ...
    with pytest.raises(preflight.SecGemmaLeanV32PreflightError, match="artifact_unwritable"):
    ...
    with pytest.raises(preflight.SecGemmaLeanV32PreflightError, match="publish_state"):
...
def test_test_markers_publish_marker_and_active_lock_cannot_masquerade(
    ...
    with pytest.raises(preflight.SecGemmaLeanV32PreflightError, match="active_run"):
        preflight._validate_committed_preflight_for_acquisition(
            root, **_validator_kwargs(root, report, manifest)
        )
...
def test_private_contact_is_untracked_ignored_and_only_fingerprint_escapes(
    ...
    with pytest.raises(preflight.SecGemmaLeanV32PreflightError, match="tracked"):
    ...
    with pytest.raises(preflight.SecGemmaLeanV32PreflightError, match="not_ignored"):
