...
import agent_benchmark.sec_gemma_lean_v38_source as source_module
...
def test_q1_q2_master_authority_is_mandatory_and_q1_must_match() -> None:
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="quarter evidence set"):
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="target set differs"):
...
def test_submissions_snapshot_accepts_unoccupied_inclusive_bounds_and_layout() -> None:
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="repeats"):
        parse_submissions_snapshot(duplicate, {history_name: history})

    duplicate_key = main.replace(b'"cik":320193', b'"cik":320193,"cik":320193')
    with pytest.raises(SecGemmaLeanV38SourceError, match="duplicate JSON object key"):
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="zero/one flag"):
        parse_submissions_snapshot(
...
def test_strict_master_gzip_and_exact_apple_projection() -> None:
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="trailing|concatenated"):
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="repeated raw row"):
        parse_strict_master_gzip(repeated, year=2024, quarter=1)
    malformed = gzip.compress(
        (MASTER_HEADER + "\n320193|APPLE|10-Q|2024-02-01\n").encode(),
        mtime=0,
    )
    with pytest.raises(SecGemmaLeanV38SourceError, match="five fields"):
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="line-control"):
...
def test_blank_company_apple_targets_and_other_empty_fields_reject() -> None:
    ...
    for line, message in empty_cases:
        with pytest.raises(SecGemmaLeanV38SourceError, match=message):
            parse_strict_master_gzip(
                _master_payload([line]), year=2024, quarter=1
            )
...
def _synthetic_full_rehearsal_inputs() -> tuple[
    ...
    # The observed Q1 filing is an ordinary member.  Its evidence boundary is
    # earlier than the exact supported-history threshold, so v3.8 seals null
    # availability and stage values without any row-specific exception.
    observed_q1 = _row(
        "0000320193-94-000002",
        "1994-01-26",
        primary="",
    ...
    # delayed beyond the pinned calendar.  The remaining 72 rows produce the
    # unchanged minimum admitted development D of 72.
...
def test_synthetic_full_size_three_stage_rehearsal_replay_and_prefix() -> None:
    ...
    assert development.seal["counts"] == {
        ...
        "U": 72,
        "D": 72,
        "exact_acceptance_U": 72,
        "successful_request_formula": 245,
    }
    assert development.session_dates == EXPECTED_SESSIONS
    assert [
    ...
    assert observed["availability_session"] is None
    assert observed["stage_assignment"] is None
    assert detached_replay_stage_source(development)["exact_seal_json_match"] is True
    projection = build_legacy_science_projection(development)
    assert len(projection.documents) == 72
    assert len(projection.documents) <= STAGE_SOURCE_CONFIGS["development"].d_max
    assert all(
        item.accession_number != "0000320193-94-000002"
...
def test_cross_stage_rejects_each_frozen_prefix_drift_kind(
    ...
    with pytest.raises(SecGemmaLeanV38SourceError):
...
def test_authoritative_calendar_is_exact_and_compact_identity_is_sealed() -> None:
    ...
    experiment = calendar["experiment_calendar"]
    history = calendar["availability_search_calendar"]
    assert calendar == {
        "schema_version": (
            "aapl-sec-gemma-lean-v38-authoritative-calendar-receipt-v1"
        ),
        "experiment_calendar": {
        ...
        "availability_search_calendar": {
            "calendar_id": "nyse_trading_session_dates_1998_01_01_2026_07_10_v1",
            "start": "1998-01-01",
            "end": "2026-07-10",
            "session_count": 7173,
            "sessions_sha256": (
        ...
        "suffix_relation": {
            ...
            "availability_prefix_session_count": 504,
            "experiment_suffix_start_index": 504,
            "experiment_suffix_session_count": 6669,
            "experiment_suffix_sessions_sha256": experiment["sessions_sha256"],
            "experiment_suffix_sessions_canonical_json_sha256": experiment[
                "sessions_canonical_json_sha256"
            ],
        },
        "availability_semantics": {
            ...
            "exact_boundary_start": "1997-12-31",
            ...
            "experiment_stage_start": "2000-01-01",
        },
        "authoritative_calendar_receipt_sha256": calendar[
            "authoritative_calendar_receipt_sha256"
        ],
    }
    calendar_body = deepcopy(calendar)
    calendar_hash = calendar_body.pop("authoritative_calendar_receipt_sha256")
    assert calendar_hash == _canonical_hash(calendar_body)
    assert len(source_module.EXPECTED_MARKET_HISTORY_SESSIONS) == 7173
    assert source_module.EXPECTED_MARKET_HISTORY_SESSIONS[504:] == EXPECTED_SESSIONS
    assert history["session_count"] - experiment["session_count"] == 504
    assert source_module.validate_market_history_session_calendar(
        source_module.EXPECTED_MARKET_HISTORY_SESSIONS
    )["sessions"] == list(source_module.EXPECTED_MARKET_HISTORY_SESSIONS)
    assert source_module._conservative_availability_session("1994-01-26") is None
    assert source_module._conservative_availability_session("1997-12-30") is None
    assert (
        source_module._conservative_availability_session("1997-12-31")
        == "1998-01-02"
    )
    assert (
        source_module._conservative_availability_session("1998-01-02")
        == "1998-01-05"
    )
    assert (
        source_module._conservative_availability_session("1998-01-16")
        == "1998-01-20"
    )
    assert (
        source_module._conservative_availability_session("1999-12-30")
        == "1999-12-31"
    )
    assert (
        source_module._conservative_availability_session("1999-12-31")
        == "2000-01-03"
    )
    assert source_module._conservative_availability_session("2026-07-10") is None
    delayed = _row(
        "7777777777-94-000777",
        "1994-02-01",
        change="1999-12-31",
    )
    delayed_source = reconcile_complete_submission(
        _target_for_row(delayed),
        _complete_submission(delayed),
        session_dates=EXPECTED_SESSIONS,
        acquisition_stage="development",
    )
    assert delayed_source.seal_row["availability_session"] == "2000-01-03"
    assert delayed_source.seal_row["stage_assignment"] == "development"
    main, _, rows, responses = _synthetic_full_rehearsal_inputs()
    prestage_rows = deepcopy(rows)
    prestage_accession = "0000320193-94-000002"
    prestage_row = next(
        row
        for row in prestage_rows
        if row["accessionNumber"] == prestage_accession
    )
    prestage_row["dateOfFilingDateChange"] = "1998-01-02"
    ordered_development = sorted(
        (
            row
            for row in prestage_rows
    ...
    for number in range(16):
        group = ordered_development[number * 8 : (number + 1) * 8]
        name = f"CIK0000320193-submissions-{number + 1:03d}.json"
        prestage_historical[name] = json.dumps(
            _columns(group), separators=(",", ":")
        ).encode()
    prestage_responses = dict(responses)
    prestage_responses[prestage_accession] = _complete_submission(prestage_row)
    prestage_bundle = build_stage_source_seal(
        stage="development",
        main_submissions_payload=main,
        historical_submissions_payloads=prestage_historical,
        master_gzip_payloads=_master_set("development", prestage_rows),
        complete_submission_payloads={
            str(row["accessionNumber"]): prestage_responses[
    ...
    prestage_seal_row = next(
        row
        for row in prestage_bundle.seal["target_rows"]
        if row["accession_number"] == prestage_accession
    )
    assert prestage_seal_row["availability_session"] == "1998-01-05"
    assert prestage_seal_row["stage_assignment"] is None
    assert prestage_bundle.seal["counts"]["U"] == 72
    assert prestage_bundle.seal["counts"]["D"] == 72
    assert prestage_accession not in prestage_bundle.seal["u_accessions"]
    assert prestage_accession not in prestage_bundle.seal["d_accessions"]
    assert all(
        item.accession_number != prestage_accession
        for item in build_legacy_science_projection(prestage_bundle).documents
    )
    target = development.master_reconciliation.targets[0]
    source = development.complete_sources[0]
    with pytest.raises(SecGemmaLeanV38SourceError, match="authoritative"):
        reconcile_complete_submission(
            target,
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="authoritative"):
        reconcile_complete_submission(
            target,
            source.complete_response,
            session_dates=source_module.EXPECTED_MARKET_HISTORY_SESSIONS,
            acquisition_stage="development",
        )

    for section, field, replacement in (
        ("experiment_calendar", "session_count", 6670),
        ("availability_search_calendar", "sessions_sha256", "sha256:" + "0" * 64),
        ("suffix_relation", "experiment_suffix_start_index", 503),
        ("availability_semantics", "exact_boundary_start", "1997-12-30"),
    ):
        forged = deepcopy(final.seal)
        forged["session_calendar"][section][field] = replacement
        composite = forged["session_calendar"]
        composite.pop("authoritative_calendar_receipt_sha256")
        composite["authoritative_calendar_receipt_sha256"] = _canonical_hash(
            composite
        )
        _reseal(forged)
        with pytest.raises(SecGemmaLeanV38SourceError, match="calendar"):
            validate_stage_source_seal(forged)

    d81_rows = deepcopy(development.seal["target_rows"])
    promoted = 0
    for row in d81_rows:
        if row["stage_assignment"] is None and row["availability_session"] is None:
            row["stage_assignment"] = "development"
            row["availability_session"] = "2018-12-31"
            promoted += 1
            if promoted == 9:
                break
    assert promoted == 9
    with pytest.raises(SecGemmaLeanV38SourceError, match="Frozen admitted D"):
        source_module._assemble_stage_seal_metadata(
            config=development.config,
            snapshot=development.submissions_snapshot,
            reconciliation=development.master_reconciliation,
            target_rows=d81_rows,
            current_complete_accessions=development.current_complete_accessions,
            prior_seal=None,
            pre_master_upper_bound=development.seal["pre_master_upper_bound"],
        )
...
def test_pre_master_submissions_bounds_are_rejection_only_and_exact() -> None:
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="I upper bound"):
...
def test_prior_raw_chain_is_replayed_before_subtraction_and_deep_snapshotted() -> None:
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="authenticate|replay"):
        derive_complete_submission_plan(
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="prior bundle"):
        derive_complete_submission_plan(
            final.master_reconciliation,
...
def test_cross_stage_prior_hash_and_exact_p_carry_are_not_forgeable() -> None:
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="hash/full chain"):
...
def test_u_d_and_science_use_availability_order_while_p_uses_filing_order() -> None:
    ...
    _reseal(forged)
    with pytest.raises(SecGemmaLeanV38SourceError, match="U/D"):
        validate_stage_source_seal(forged)
...
def test_final_completed_year_gate_is_mandatory_and_reconstructed() -> None:
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="year 2000"):
        validate_completed_year_source_coverage(rows)

    forged = deepcopy(final.seal)
    forged["completed_year_source_coverage"]["years"][0]["10-K"] = 0
    _reseal(forged)
    with pytest.raises(SecGemmaLeanV38SourceError, match="completed-year source gate"):
        validate_stage_source_seal(forged)

    wrong_stage = deepcopy(development.seal)
    wrong_stage["completed_year_source_coverage"] = {"forged": True}
    _reseal(wrong_stage)
    with pytest.raises(SecGemmaLeanV38SourceError, match="only in the final"):
...
def test_plain_header_form_ascii_horizontal_whitespace_is_family_neutral() -> None:
    valid_cases = (
        ("5555555555-06-000101", "2006-03-15", "10-K"),
        ("5555555555-06-000102", "2006-08-15", "10-Q"),
    )
    for accession, filed, form in valid_cases:
        row = _row(accession, filed, form=form, primary="synthetic-general.htm")
        sec_tagged = _complete_submission(row, include_filename=False)
        tagged_form = f"<CONFORMED-SUBMISSION-TYPE>{form}".encode("ascii")
        ims_tagged = (
            b"general synthetic preamble\n"
            + sec_tagged.replace(b"<SEC-DOCUMENT>", b"<IMS-DOCUMENT>", 1)
            .replace(b"<SEC-HEADER>", b"<IMS-HEADER>", 1)
            .replace(b"</SEC-HEADER>", b"</IMS-HEADER>", 1)
            + b"</IMS-DOCUMENT>\ngeneral synthetic trailer\n"
        )
        for family, tagged_response in (
            ("SEC", sec_tagged),
            ("IMS", ims_tagged),
        ):
            assert tagged_response.count(f"<{family}-HEADER>".encode("ascii")) == 1
            assert tagged_response.count(
                f"</{family}-HEADER>".encode("ascii")
            ) == 1
            other_family = "IMS" if family == "SEC" else "SEC"
            assert f"<{other_family}-HEADER>".encode("ascii") not in tagged_response
            for indentation, separator in (
                (b"", b" "),
                (b"\t", b"\t"),
                (b" \t ", b" \t  "),
            ):
                ...
                candidate = tagged_response.replace(tagged_form, plain_form, 1)
                assert source_module._exact_header_sgml_form(
                    candidate,
                    expected_form=form,
                ) == form

    invalid_row = _row(
        "6666666666-07-000103",
        "2007-05-30",
        form="10-K",
        primary="synthetic-invalid.htm",
    )
    sec_tagged = _complete_submission(invalid_row, include_filename=False)
    tagged_form = b"<CONFORMED-SUBMISSION-TYPE>10-K"
    ims_tagged = (
        b"invalid-matrix synthetic preamble\n"
        + sec_tagged.replace(b"<SEC-DOCUMENT>", b"<IMS-DOCUMENT>", 1)
        .replace(b"<SEC-HEADER>", b"<IMS-HEADER>", 1)
        .replace(b"</SEC-HEADER>", b"</IMS-HEADER>", 1)
        + b"</IMS-DOCUMENT>\ninvalid-matrix synthetic trailer\n"
    )
    invalid_lines = (
        b"CONFORMED SUBMISSION TYPE:10-K",
        b"CONFORMED SUBMISSION TYPE:\x0b10-K",
        b"CONFORMED SUBMISSION TYPE:\x0c10-K",
        b"CONFORMED SUBMISSION TYPE:\r10-K",
        b"CONFORMED SUBMISSION TYPE:\n10-K",
        b"CONFORMED SUBMISSION TYPE:\xa010-K",
        b"CONFORMED SUBMISSION TYPE:\xe2\x80\x8310-K",
        b"CONFORMED SUBMISSION TYPE: 10-K\t",
        b"CONFORMED SUBMISSION TYPE: 10-k",
    ...
    for tagged_response in (sec_tagged, ims_tagged):
        for invalid_line in invalid_lines:
            with pytest.raises(SecGemmaLeanV38SourceError, match="form"):
                source_module._exact_header_sgml_form(
                    tagged_response.replace(tagged_form, invalid_line, 1),
                    expected_form="10-K",
                )

    full_row = _row(
        "7777777777-08-000104",
        "2008-04-30",
        form="10-Q",
        primary="synthetic-reconciliation.htm",
    )
    original_sec = _complete_submission(full_row).replace(
        b"<CONFORMED-SUBMISSION-TYPE>10-Q",
        b"\t CONFORMED SUBMISSION TYPE:\t \t10-Q",
        1,
    )
    source = reconcile_complete_submission(
        _target_for_row(full_row),
        original_sec,
        session_dates=EXPECTED_SESSIONS,
        acquisition_stage="development",
    )
    assert source.complete_response == original_sec
    assert source.complete_response_sha256 == (
        "sha256:" + hashlib.sha256(original_sec).hexdigest()
    )
    start = source.seal_row["extracted_text"]["start_byte"]
    end = source.seal_row["extracted_text"]["end_byte"]
    expected_start = original_sec.index(b"<TEXT>") + len(b"<TEXT>")
    expected_end = original_sec.index(b"</TEXT>", expected_start)
    assert (start, end) == (expected_start, expected_end)
    assert original_sec[start:end] == source.extracted_primary
    assert source.seal_row["extracted_text"]["sha256"] == (
        "sha256:" + hashlib.sha256(source.extracted_primary).hexdigest()
    )
    ...
    assert original_sec not in published
    assert source.extracted_primary not in published
    assert b"AAPL filing evidence" not in published


def test_legacy_ims_header_form_accepts_horizontal_whitespace_only() -> None:
    cases = (
        ("0000912057-00-023450", "2000-05-30", "10-K"),
        ("0000912057-00-023451", "2000-06-01", "10-Q"),
    )
    for accession, filed, form in cases:
        row = _row(accession, filed, form=form, primary="legacy-primary.htm")
        base = _complete_submission(row, include_filename=False)
        tagged_form = f"<CONFORMED-SUBMISSION-TYPE>{form}".encode("ascii")
        ims_tagged = (
            b"legacy preamble\n"
            + base.replace(b"<SEC-DOCUMENT>", b"<IMS-DOCUMENT>", 1)
            .replace(b"<SEC-HEADER>", b"<IMS-HEADER>", 1)
            .replace(b"</SEC-HEADER>", b"</IMS-HEADER>", 1)
            + b"</IMS-DOCUMENT>\nlegacy trailer\n"
        )
        ...
        for indentation, separator in (
            ...
            ims_plain = ims_tagged.replace(tagged_form, plain_form, 1)
            assert source_module._exact_header_sgml_form(
                ims_plain,
                expected_form=form,
            ) == form
    ...
    invalid_lines = (
        ...
        b"CONFORMED SUBMISSION TYPE: 10-K ",
        b"CONFORMED SUBMISSION TYPE: 10-k",
        b"\x0bCONFORMED SUBMISSION TYPE: 10-K",
    ...
    for invalid_line in invalid_lines:
        with pytest.raises(SecGemmaLeanV38SourceError, match="form"):
            source_module._exact_header_sgml_form(
                ims.replace(tagged_form, invalid_line, 1),
                expected_form="10-K",
            )


def test_full_synthetic_source_seal_replays_legacy_ims_original_bytes() -> None:
    main, historical, rows, responses = _synthetic_full_rehearsal_inputs()
    development_rows = [
        row for row in rows if str(row["filingDate"]) <= "2018-12-31"
    ]
    candidate = next(
        row
        for row in development_rows
        if str(row["filingDate"]) >= "2000-01-01" and row["form"] == "10-K"
    )
    accession = str(candidate["accessionNumber"])
    form = str(candidate["form"])
    acceptance = source_module.parse_submissions_acceptance_datetime(
        str(candidate["acceptanceDateTime"])
    ).strftime("%Y%m%d%H%M%S")
    original_sec = _complete_submission(candidate, include_filename=False)
    original_ims = (
        b"synthetic legacy transport preamble\n"
        + original_sec.replace(b"<SEC-DOCUMENT>", b"<IMS-DOCUMENT>", 1)
        .replace(b"<SEC-HEADER>", b"<IMS-HEADER>", 1)
        .replace(b"</SEC-HEADER>", b"</IMS-HEADER>", 1)
        .replace(
            f"<ACCEPTANCE-DATETIME>{acceptance}\n".encode("ascii"),
        ...
        .replace(
            f"<CONFORMED-SUBMISSION-TYPE>{form}".encode("ascii"),
            b"  CONFORMED SUBMISSION TYPE:  \t " + form.encode("ascii"),
            1,
        )
        .replace(
            b"<TEXT>",
            b"<TEXT><IMS-HEADER>literal filing text</IMS-HEADER>",
            1,
        )
        + b"</IMS-DOCUMENT>\nsynthetic legacy transport trailer\n"
    )
    responses[accession] = original_ims
    bundle = build_stage_source_seal(
        stage="development",
        main_submissions_payload=main,
        historical_submissions_payloads=historical,
    ...
    source = bundle.sources_by_accession[accession]
    assert source.complete_response == original_ims
    assert source.complete_response_sha256 == (
        "sha256:" + hashlib.sha256(original_ims).hexdigest()
    )
    start = source.seal_row["extracted_text"]["start_byte"]
    end = source.seal_row["extracted_text"]["end_byte"]
    assert original_ims[start:end] == source.extracted_primary
    assert b"<IMS-HEADER>literal filing text</IMS-HEADER>" in (
        source.extracted_primary
    )
    assert source.seal_row["primary_selection"]["sgml_filename_missing"] is True
    assert b"synthetic legacy transport preamble" not in bundle.seal_json
    assert source.extracted_primary not in bundle.seal_json
    replay = detached_replay_stage_source(bundle)
    assert replay["stage_source_seal_sha256"] == bundle.stage_source_seal_sha256
    assert replay["replayed_stage_source_seal_sha256"] == (
        bundle.stage_source_seal_sha256
...
def test_raw_header_and_all_document_form_sequence_topology_must_be_exact() -> None:
    ...
    for duplicate_response in duplicate_fields:
        ...
        # behavior; singular raw fields are a v3.8 adapter rule only.
...
def test_gzip_corrupt_truncated_and_both_size_caps(
    ...
    corrupt[-8] ^= 1
    with pytest.raises(SecGemmaLeanV38SourceError, match="CRC|gzip"):
        parse_strict_master_gzip(bytes(corrupt), year=2024, quarter=1)
    with pytest.raises(SecGemmaLeanV38SourceError, match="truncated|CRC"):
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="compressed byte cap"):
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="decompressed byte cap"):
        parse_strict_master_gzip(payload, year=2024, quarter=1)
...
def test_master_exact_set_rejects_missing_unexpected_and_forged_evidence() -> None:
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="set is not exact"):
        reconcile_master_exact_set(
            development.submissions_snapshot,
            evidence[:-1],
            "development",
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="set differs"):
        reconcile_master_exact_set(
            development.submissions_snapshot,
            missing,
            "development",
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="authenticate"):
        reconcile_master_exact_set(
            development.submissions_snapshot,
            forged,
            "development",
...
def test_compact_replay_rejects_blob_receipt_evidence_and_callback_tampering() -> None:
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="hash/length"):
        detached_replay(
            "development",
            replay.checkpoint,
            manifests,
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="evidence differs"):
        detached_replay(
            "development",
            evidence_checkpoint,
            mismatched_evidence,
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="hash/bindings"):
        detached_replay(
            "development",
            replay.checkpoint,
            tampered_receipt,
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="failed safely"):
        detached_replay(
            "development",
            replay.checkpoint,
            manifests,
...
def test_compact_replay_rejects_missing_orphan_duplicate_and_misordered_roles() -> None:
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="missing required role"):
        detached_replay(
            "development",
            missing_checkpoint,
            missing,
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="orphan"):
        detached_replay(
            "development",
            orphan_checkpoint,
            orphan,
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="duplicated"):
        detached_replay(
            "development",
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="misordered|mismatched"):
        detached_replay(
            "development",
...
def test_compact_replay_rejects_resealed_checkpoint_and_recursive_prior_blob_tamper() -> None:
    ...
    with pytest.raises(SecGemmaLeanV38SourceError, match="source seal differs"):
        detached_replay(
            "development",
...
def test_acceptance_representation_semantics_are_exact_and_dst_hostile() -> None:
    exact_cases = (
        ("20320115123456", "2032-01-15T12:34:56-05:00", 0),
        ("20320715123456", "2032-07-15T12:34:56-04:00", 0),
        ("2032-01-15T17:34:56Z", "2032-01-15T12:34:56-05:00", 0),
        ("2032-07-15T16:34:56.0Z", "2032-07-15T12:34:56-04:00", 0),
        (
            "2032-01-15T02:34:56.000000000Z",
            "2032-01-14T21:34:56-05:00",
            0,
        ),
        ("2024-03-10T02:30:00Z", "2024-03-09T21:30:00-05:00", 0),
        ("2024-11-03T01:30:00.000Z", "2024-11-02T21:30:00-04:00", 0),
    )
    for raw, expected, expected_fold in exact_cases:
        parsed = source_module.parse_submissions_acceptance_datetime(raw)
        assert parsed.isoformat() == expected
        assert parsed.fold == expected_fold

    for raw, word in (
        ("20240310023000", "nonexistent"),
        ("20241103013000", "ambiguous"),
    ):
        with pytest.raises(source_module.SecPointInTimeError, match=word):
            source_module.parse_submissions_acceptance_datetime(raw)

    first_fold = source_module.parse_submissions_acceptance_datetime(
        "2024-11-03T05:30:00.000Z"
    )
    second_fold = source_module.parse_submissions_acceptance_datetime(
        "2024-11-03T06:30:00.000000000Z"
    )
    assert first_fold.strftime("%Y%m%d%H%M%S") == "20241103013000"
    assert second_fold.strftime("%Y%m%d%H%M%S") == "20241103013000"
    assert first_fold.utcoffset() != second_fold.utcoffset()
    assert (first_fold.fold, second_fold.fold) == (0, 1)
    assert second_fold.timestamp() - first_fold.timestamp() == 3600

    without_fraction = source_module.parse_submissions_acceptance_datetime(
        "2032-02-15T18:19:20Z"
    )
    for zero_count in range(1, 10):
        with_fraction = source_module.parse_submissions_acceptance_datetime(
            f"2032-02-15T18:19:20.{('0' * zero_count)}Z"
        )
        assert with_fraction == without_fraction

    malformed = (
        "2032-02-15T18:19:20.100000000Z",
        "2032-02-15T18:19:20.000000001Z",
        "2032-02-15t18:19:20Z",
        "2032-02-15T18:19:20z",
        "2032-02-15T18:19:20+00:00",
        " 2032-02-15T18:19:20Z",
        "2032-02-15T18:19:20Z ",
        "2032-02-15 T18:19:20Z",
        "2032-02-15T18:19Z",
        "2032-02-15T18:19:20.Z",
        "2032-02-15T18:19:20.0000000000Z",
        "2032-02-15T18:19:20Zextra",
        "2032-02-30T18:19:20Z",
        "2032-02-15T24:19:20Z",
        "2032-02-15T18:19:60Z",
        b"2032-02-15T18:19:20Z",
        None,
        20320215181920,
    )
    for raw in malformed:
        with pytest.raises(
            source_module.SecPointInTimeError,
            match="acceptanceDateTime|SEC acceptance",
        ):
            source_module.parse_submissions_acceptance_datetime(raw)  # type: ignore[arg-type]


def test_utc_acceptance_reconciles_to_eastern_header_across_date_rollover(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    direct_cases = (
        (
            "8888888888-08-000101",
            "2008-01-15",
            "2008-01-15T17:34:56Z",
            "20080115123456",
            "10-K",
        ),
        (
            "8888888888-08-000102",
            "2008-07-15",
            "2008-07-15T16:34:56.000000000Z",
            "20080715123456",
            "10-Q",
        ),
        (
            "8888888888-08-000103",
            "2008-02-14",
            "2008-02-15T02:34:56.000Z",
            "20080214213456",
            "10-Q",
        ),
    )
    for accession, filed, raw_acceptance, normalized, form in direct_cases:
        row = _row(
            accession,
            filed,
            form=form,
            primary="synthetic-time-semantics.htm",
        )
        row["acceptanceDateTime"] = raw_acceptance
        response = _complete_submission(row)
        source = reconcile_complete_submission(
            _target_for_row(row),
            response,
        ...
        prefix = source.seal_row["frozen_prefix"]
        acceptance = prefix["acceptance"]
        assert acceptance["submissions_source"] == raw_acceptance
        assert acceptance["header_source"] == normalized
        assert acceptance["normalized_et"] == normalized
        assert acceptance["exact"] is True
        assert prefix["availability_not_before_date"] == filed
        assert prefix["availability_session"] == next(
            value for value in EXPECTED_SESSIONS if value > filed
        )
        assert f"<ACCEPTANCE-DATETIME>{normalized}\n".encode("ascii") in response
        assert source.complete_response == response
        assert source.complete_response_sha256 == (
    ...
    main, historical, rows, responses = _synthetic_full_rehearsal_inputs()
    compact_row_source = next(
        row
        for row in rows
        if row["filingDate"] == "2008-02-01" and row["form"] == "10-K"
    )
    compact_accession = str(compact_row_source["accessionNumber"])
    compact_raw_acceptance = "2008-02-02T02:34:56.000000000Z"
    compact_normalized = "20080201213456"
    compact_row_source["acceptanceDateTime"] = compact_raw_acceptance
    updated_historical: dict[str, bytes] = {}
    containing_historical_name: str | None = None
    for name, payload in historical.items():
        columns = json.loads(payload.decode("utf-8"))
        if compact_accession in columns["accessionNumber"]:
            position = columns["accessionNumber"].index(compact_accession)
            columns["acceptanceDateTime"][position] = compact_raw_acceptance
            containing_historical_name = name
        updated_historical[name] = json.dumps(
            columns,
            separators=(",", ":"),
        ).encode("utf-8")
    assert containing_historical_name is not None
    responses[compact_accession] = _complete_submission(compact_row_source)
    monkeypatch.setitem(
        globals(),
        "_synthetic_full_rehearsal_inputs",
        lambda: (main, updated_historical, rows, responses),
    )
    stage_output, replay_input, blobs = _build_compact_stage(
        "development",
        prior_output=None,
        prior_replay=None,
    )
    compact_row = next(
        row
        for row in stage_output.seal["target_rows"]
        if row["accession_number"] == compact_accession
    )
    compact_prefix = compact_row["frozen_prefix"]
    assert compact_prefix["acceptance"] == {
        "submissions_source": compact_raw_acceptance,
        "header_source": compact_normalized,
        "normalized_et": compact_normalized,
        "submissions_missing": False,
        "header_missing": False,
        "exact": True,
    }
    assert compact_prefix["availability_not_before_date"] == "2008-02-01"
    assert replay_input.checkpoint["expected_stage_source_seal_sha256"] == (
    ...
    assert json.loads(checkpoint_json) == replay_input.checkpoint

    historical_manifest = next(
        manifest
        for manifest in replay_input.role_manifests
        if manifest["role_id"]
        == f"submissions/historical/{containing_historical_name}"
    )
    complete_manifest = next(
        manifest
        for manifest in replay_input.role_manifests
        if manifest["role_id"] == f"complete/{compact_accession}"
    )
    assert compact_raw_acceptance.encode("ascii") in blobs[
        str(historical_manifest["blob_name"])
    ]
    compact_complete = blobs[str(complete_manifest["blob_name"])]
    assert (
        f"<ACCEPTANCE-DATETIME>{compact_normalized}\n".encode("ascii")
        in compact_complete
    )
    assert compact_row["complete_response"]["sha256"] == (
        "sha256:" + hashlib.sha256(compact_complete).hexdigest()
    )
    replayed = rehydrate_compact_stage(
        "development",
        replay_input.checkpoint,
        replay_input.role_manifests,
        blobs.__getitem__,
    )
    replayed_row = next(
        row
        for row in replayed.stage_output.seal["target_rows"]
        if row["accession_number"] == compact_accession
    )
    assert replayed_row == compact_row
    assert replayed.receipt["exact_source_seal_match"] is True
    assert detached_replay(
        "development",
        replay_input.checkpoint,
        replay_input.role_manifests,
...
def test_acceptance_mismatch_and_missingness_contract_is_unchanged() -> None:
    row = _row(
        "6666666666-10-000201",
        "2010-06-15",
        form="10-Q",
        primary="synthetic-missingness.htm",
    )
    row["acceptanceDateTime"] = "2010-06-15T19:20:21.000Z"
    normalized = "20100615152021"
    response = _complete_submission(row)
    header_line = f"<ACCEPTANCE-DATETIME>{normalized}\n".encode("ascii")
    assert header_line in response

    mismatched = response.replace(
        header_line,
        b"<ACCEPTANCE-DATETIME>20100615152022\n",
        1,
    )
    with pytest.raises(
        SecGemmaLeanV38SourceError,
        match="Acceptance values do not reconcile lexically",
    ):
        reconcile_complete_submission(
            _target_for_row(row),
            mismatched,
    ...
    without_header = response.replace(header_line, b"", 1)
    submissions_only = reconcile_complete_submission(
        _target_for_row(row),
        without_header,
        session_dates=EXPECTED_SESSIONS,
        acquisition_stage="development",
    ).seal_row["frozen_prefix"]["acceptance"]
    assert submissions_only == {
        "submissions_source": row["acceptanceDateTime"],
        "header_source": None,
        "normalized_et": normalized,
        "submissions_missing": False,
        "header_missing": True,
        "exact": True,
    }

    no_submissions_row = deepcopy(row)
    no_submissions_row["acceptanceDateTime"] = ""
    header_only = reconcile_complete_submission(
        _target_for_row(no_submissions_row),
        response,
        session_dates=EXPECTED_SESSIONS,
        acquisition_stage="development",
    ).seal_row["frozen_prefix"]["acceptance"]
    ...
    both_missing = reconcile_complete_submission(
        _target_for_row(no_submissions_row),
        without_header,
        session_dates=EXPECTED_SESSIONS,
        acquisition_stage="development",
    ...
    malformed_submissions = deepcopy(row)
    malformed_submissions["acceptanceDateTime"] = (
        "2010-06-15T19:20:21.000000001Z"
    )
    with pytest.raises(
        SecGemmaLeanV38SourceError,
        match="malformed acceptanceDateTime",
    ):
        _target_for_row(malformed_submissions)

    malformed_header = response.replace(
        header_line,
        b"<ACCEPTANCE-DATETIME>20100230152021\n",
        1,
    )
    with pytest.raises(
        SecGemmaLeanV38SourceError,
        match="Complete submission SGML failed strict parsing",
    ):
        reconcile_complete_submission(
            _target_for_row(row),
            malformed_header,
