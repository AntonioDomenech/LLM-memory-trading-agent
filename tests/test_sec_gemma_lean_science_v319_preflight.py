...
from tests import test_sec_gemma_lean_science_v319_bridge as bridge_fixtures
...
def _direction_sanitizer_aggregate() -> dict[str, object]:
    return {
        "sanitizer_schema_version": contract.DIRECTION_SANITIZER_SCHEMA_VERSION,
        "event_count": 75,
        "events_with_removals": 7,
        "removed_current_sentence_count": 5,
        "removed_prior_sentence_count": 3,
        "empty_current_count": 0,
        "empty_prior_count": 0,
        "source_events_sha256": SHA64["source_events"],
        "sanitized_events_sha256": SHA64["sanitized_events"],
        "removal_counts_sha256": SHA64["removal_counts"],
    }


def _assert_code(expected_code: str, function, *args, **kwargs) -> None:
    with pytest.raises(preflight.V319PreflightError) as captured:
...
def _bridge_manifest() -> dict[str, object]:
    body = {
        "schema_version": "aapl-sec-gemma-lean-science-v3-19-streaming-bridge-v1",
        "stage": "development",
        "document_count": 75,
...
def _commitments() -> dict[str, object]:
    return {
        ...
        "direction_sanitizer_aggregate": _direction_sanitizer_aggregate(),
        ...
        repository_rows.append(
            {
                ...
                "role": role,
                "owning_revision": (
                    implementation_commit if role == "v319" else contract.BASE_COMMIT
                ),
...
def _write_qualification_private_state(
    ...
    *,
    ...
    ancestry = repository["ancestry"]
    phase_nodes = {
        contract.QUALIFICATION_PHASE_V319: [
            *preflight._RUNTIME_REQUIRED_PHASE1_NODE_IDS,
            *(
                f"{v319_node_path}::test_synthetic_{index:03d}"
                for index in range(
...
def test_phase2_rejects_case_insensitive_node_id_collapsing() -> None:
    ...
    # semantics now see 654 unique IDs and one duplicate, so V3.19 rejects it.
    nodes[-1] = nodes[-2]
    digest = hashlib.sha256("\n".join(nodes).encode("utf-8")).hexdigest()
...
def test_isolated_qualifier_bootstrap_matches_frozen_literal_and_imports_pinned_pytest() -> None:
    ...
    assert len(bootstrap) == contract.QUALIFICATION_BOOTSTRAP_BYTES == 5_482
...
def test_isolated_qualifier_executes_exact_windows_recovery_nodes_in_controlled_environment(
    tmp_path: Path,
) -> None:
    root = Path(__file__).resolve().parents[1]
    runtime = _qualification_runtime_fixture()
    environment, profile = preflight._qualification_environment(runtime)
    controlled = contract.QUALIFICATION_CONTROLLED_PYTEST_ENVIRONMENT
    controlled_names = tuple(name for name, _authority in controlled)
    expected_controlled = (
    ...
    assert controlled == expected_controlled
    assert len(environment) == len(profile) == 16
    assert set(environment) == set(contract.QUALIFICATION_ENVIRONMENT_NAMES)
    assert set(controlled_names).isdisjoint(environment)
    assert len(contract.QUALIFICATION_PYTEST_ENVIRONMENT_NAMES) == 22
    assert set(contract.QUALIFICATION_PYTEST_ENVIRONMENT_NAMES) == {
        *environment,
        *controlled_names,
    }
    ...
    assert qualification["launch_names"] == list(
        contract.QUALIFICATION_ENVIRONMENT_NAMES
    )
    assert qualification["controlled_pytest_additions"] == [
        {"name": name, "authority": authority}
        for name, authority in expected_controlled
    ]
    assert qualification["pytest_names"] == list(
        contract.QUALIFICATION_PYTEST_ENVIRONMENT_NAMES
    )
    redacted_profile = contract.canonical_json_bytes(profile).decode("ascii")
    assert environment["TEMP"] not in redacted_profile
    assert str(
        Path(environment["SYSTEMROOT"]) / "System32" / "cmd.exe"
    ) not in redacted_profile

    forbidden_shared_recovery_symbols = frozenset(
        {
            ...
            "_build_publication_recovery_worker_request",
            "_publication_recovery_worker_payload",
            "_supervise_publication_recovery_worker",
            "_recover_production_publication_from_entry",
            "recover_production_publication",
        }
    )
    observed_forbidden: list[tuple[str, str]] = []
    for relative in contract.IMPLEMENTATION_PRODUCTION_PATHS:
        ...
        for node in ast.walk(tree):
            ...
            observed_forbidden.extend(
                (relative, value)
                for value in values
                if value in forbidden_shared_recovery_symbols
            )
    assert observed_forbidden == []

    nodes = [
        "tests/test_sec_gemma_online_risk_overlay_production.py::test_recovery_supervisor_uses_private_stdin_and_accepts_only_envelope",
        "tests/test_sec_gemma_online_risk_overlay_production.py::test_recovery_private_request_restores_exact_git_host_environment",
    ]
    dispatch = [
        str(root)
        if token == contract.QUALIFICATION_CANONICAL_ROOT_TOKEN
    ...
    collection_stdout: bytes | None = None
    execution_stdout: bytes | None = None
    execution_xml: bytes | None = None
    for mode in contract.QUALIFICATION_MODES:
        junit = tmp_path / f"{mode}.xml"
        argv = [
            runtime["executable_path"],
            *contract.QUALIFICATION_PYTHON_FLAGS,
            "-c",
        ...
        if mode == "collection":
            ...
            execution_stdout = completed.stdout
            execution_xml = junit.read_bytes()
    assert collection_stdout is not None
    assert execution_stdout is not None
    assert execution_xml is not None
    assert preflight._qualification_collection_node_ids(collection_stdout) == nodes
    expected_hash = hashlib.sha256("\n".join(nodes).encode("utf-8")).hexdigest()
    assert preflight._qualification_executed_node_evidence(
        execution_xml, nodes
    ) == (2, expected_hash, nodes)
    assert preflight._qualification_junit_counts(
        execution_xml, execution_stdout
    ) == {
        "passed_count": 2,
        "failed_count": 0,
        "error_count": 0,
        ...
        **{
            **dependencies.__dict__,
            ...
            "run_qualification": lambda _root: (_ for _ in ()).throw(
                preflight.V319PreflightError("stop_after_reservation")
            ),
        }
    )
    _assert_code(
        ...
        **{
            **dependencies.__dict__,
            ...
            "authenticate_source": lambda _root: (_ for _ in ()).throw(
                preflight.V319PreflightError("stop_after_authority_creation")
            ),
        }
    )
    _assert_code(
...
def test_junit_counts_accepts_exact_pytest_long_duration_clock_suffix() -> None:
    node_ids = [
        f"tests/test_synthetic.py::test_case_{index:03d}"
    ...
    assert preflight._qualification_junit_counts(
        xml_payload,
        b"323 passed in 301.03s (0:05:01)\r\n",
    ) == expected
    assert preflight._qualification_junit_counts(
        xml_payload,
        b"323 passed in 301.03s\r\n",
    ) == expected
    ...
    invalid_summaries = (
        ...
        b"323 passed in 301.03s(0:05:01)\r\n",
        b"323 passed in 301.03s 0:05:01\r\n",
        b"323 passed in 301.03s (0:5:01)\r\n",
        b"323 passed in 301.03s (0:05:1)\r\n",
        b"323 passed in 301.03s (0:60:01)\r\n",
        b"323 passed in 301.03s (0:05:60)\r\n",
        b"323 passed in 301.03s (0:05:01) trailing\r\n",
    )
    for stdout in invalid_summaries:
        _assert_code(
...
def test_deadline_overrun_completion_fails_without_mutating_self_hashed_result() -> None:
    ...
    completed = preflight._qualification_completion_receipt(
        phase=contract.QUALIFICATION_PHASE_V319,
        mode="execution",
        result=result,
...
def test_private_qualification_replay_rejects_internally_consistent_wrong_selector(
    ...
    report = _write_qualification_private_state(
        root,
        repository,
        v319_node_path="tests/test_unrelated.py",
    )
    _assert_code(
        "execution_preflight_qualification_invalid",
        preflight._validate_qualification_private_state,
...
def test_v319_local_import_upper_bound_covers_static_and_fake_runtime_paths(
    tmp_path: Path,
) -> None:
    ...
    for index, relative in enumerate(contract.IMPLEMENTATION_PRODUCTION_PATHS):
        ...
        if index == 0:
            body += (
                "import importlib\n"
                "from . import sec_gemma_lean_science_v319_bridge\n"
    ...
    observed = preflight._audit_v319_local_import_upper_bound(
        root,
        head_commit=head,
    ...
    _assert_code(
        ...
        preflight._audit_v319_local_import_upper_bound,
        root,
        head_commit=head,
    ...
    dynamic_source.write_text(
        "import importlib\n"
        "module_name = 'agent_benchmark.sec_gemma_lean_science_v319_bridge'\n"
...
def test_real_v319_production_import_calls_are_literal_and_upper_bound_closes(
    tmp_path: Path,
) -> None:
    root = Path(__file__).resolve().parents[1]
    production_paths = (
    ...
    expected_literals = (*expected_local_modules, "numpy")
    assert production_paths == contract.IMPLEMENTATION_PRODUCTION_PATHS
    assert len(expected_local_modules) == len(set(expected_local_modules)) == 44
    assert len(expected_literals) == len(set(expected_literals)) == 45
    ...
    for path in production_paths:
        ...
        committed_sources[path] = completed.stdout
        assert (root / path).read_bytes() == completed.stdout

    observed_literals: list[str] = []
    missing_or_nonliteral: list[tuple[str, int]] = []
    recognized_call_names = {"__import__", "import_module", "run_module"}
    for path, source in committed_sources.items():
        ...
        for node in ast.walk(tree):
            ...
            if function_name not in recognized_call_names:
                continue
            if not node.args:
                missing_or_nonliteral.append((path, node.lineno))
                continue
            first = node.args[0]
            if not isinstance(first, ast.Constant) or type(first.value) is not str:
                missing_or_nonliteral.append((path, node.lineno))
                continue
            observed_literals.append(first.value)

    assert missing_or_nonliteral == []
    assert len(observed_literals) == 45
    assert Counter(observed_literals) == Counter(expected_literals)
    assert all(count == 1 for count in Counter(observed_literals).values())

    expected_runtime_paths = tuple(
        contract.LOCAL_IMPORT_PACKAGE_BOOTSTRAP_PATH
        if module_name == "agent_benchmark"
        else module_name.replace(".", "/") + ".py"
        for module_name in expected_local_modules
    )
    assert len(expected_runtime_paths) == len(set(expected_runtime_paths)) == 44
    assert len(contract.LOCAL_IMPORT_ALLOWED_PATHS) == 44
    assert set(expected_runtime_paths) == set(contract.LOCAL_IMPORT_ALLOWED_PATHS)
    observed_paths = preflight._audit_v319_local_import_upper_bound(
        root,
        head_commit=head,
        runtime_observed_paths=expected_runtime_paths,
    )
    assert len(observed_paths) == len(set(observed_paths)) == 44
    assert observed_paths == sorted(
        observed_paths, key=lambda value: value.encode("utf-8")
    )
    assert len(observed_paths) == len(contract.LOCAL_IMPORT_ALLOWED_PATHS)
    assert set(observed_paths) == set(contract.LOCAL_IMPORT_ALLOWED_PATHS)

    synthetic_root = tmp_path / "detached-variable-import"
    synthetic_root.mkdir()
    _git(synthetic_root, "init")
    _git(synthetic_root, "config", "user.name", "Synthetic Test")
    _git(synthetic_root, "config", "user.email", "synthetic@example.invalid")
    package = synthetic_root / "agent_benchmark"
    package.mkdir()
    (package / "__init__.py").write_bytes(b"")
    for index, path in enumerate(production_paths):
        target = synthetic_root / path
        if index == 0:
            target.write_bytes(
                b"import importlib\n"
                b"module_name = "
                b"'agent_benchmark.sec_gemma_lean_science_v319_bridge'\n"
                b"importlib.import_module(module_name)\n"
            )
        else:
            target.write_bytes(b"from __future__ import annotations\n")
    _git(synthetic_root, "add", "--", "agent_benchmark")
    _git(synthetic_root, "commit", "-m", "variable-driven local import")
    synthetic_head = _git(synthetic_root, "rev-parse", "HEAD")
    _git(synthetic_root, "checkout", "--detach", synthetic_head)
    _assert_code(
        "preflight_local_import_dynamic_unresolved",
        preflight._audit_v319_local_import_upper_bound,
        synthetic_root,
        head_commit=synthetic_head,
        runtime_observed_paths=expected_runtime_paths,
    )
...
def test_validators_reject_scope_count_parity_and_effect_changes() -> None:
    ...
    changed_commitments = _commitments()
    changed_commitments["direction_sanitizer_aggregate"][
        "events_with_removals"
    ] = 6
    _assert_code(
        "preflight_direction_sanitizer_aggregate_invalid",
        preflight.validate_request_commitments,
        changed_commitments,
    )
...
def test_commitment_validator_requires_frozen_sanitizer_counts_on_runner_projection_output() -> None:
    fixture = bridge_fixtures._make_fixture()
    projection = bridge.build_streaming_science_projection(fixture.authority)
    ...
    assert commitments["direction_sanitizer_aggregate"][
        "events_with_removals"
    ] == 0
    _assert_code(
        "preflight_direction_sanitizer_aggregate_invalid",
    ...
    commitments["direction_sanitizer_aggregate"] = (
        _direction_sanitizer_aggregate()
    )
...
def test_real_bridge_runner_and_preflight_authority_seam(
    ...
    plan_manifest_body["direction_sanitizer_aggregate"] = (
        _direction_sanitizer_aggregate()
    )
    plan_manifest = {
        **plan_manifest_body,
        "model_plan_sha256": contract.canonical_sha256(plan_manifest_body),
    }
    commitments["model_plan_sha256"] = plan_manifest["model_plan_sha256"]
    plan = replace(
        plan,
        sanitizer_aggregate=MappingProxyType(
            _direction_sanitizer_aggregate()
        ),
        manifest=MappingProxyType(plan_manifest),
    )
    frozen_pins = contract.build_source_authority_pins()
    monkeypatch.setattr(contract, "build_source_authority_pins", lambda: frozen_pins)
    for field, name in (
...
def test_run_preflight_seals_private_content_addressed_and_redacted_public(
    ...
    sanitizer_counts = {
        "sanitizer_event_count": 75,
        "sanitizer_events_with_removals": 7,
        "sanitizer_removed_current_sentence_count": 5,
        "sanitizer_removed_prior_sentence_count": 3,
        "sanitizer_empty_current_count": 0,
        "sanitizer_empty_prior_count": 0,
    }
    assert {
        key: artifact["counts"][key] for key in sanitizer_counts
    } == sanitizer_counts
    assert all(
        type(artifact["counts"][key]) is int
        for key in sanitizer_counts
    )
    ...
    assert private_only_ordered_hashes.isdisjoint(artifact["aggregates"])
    assert all(
        key.encode("ascii") not in public_payload
        for key in private_only_ordered_hashes
    )
    ...
    assert private_value["direction_sanitizer_aggregate"] == (
        _direction_sanitizer_aggregate()
    )
    assert private_value["request_commitments"][
...
def test_pushed_execution_authority_replays_public_private_and_source_inventory(
    ...
    _git(work, "commit", "-m", "synthetic v3.19 implementation")
    ...
    _git(work, "commit", "-m", "synthetic v3.19 preflight evidence")
...
def test_private_tree_token_split_allows_bound_paths_only(tmp_path: Path) -> None:
    ...
    v319 = str(root / contract.PRIVATE_NAMESPACE).encode("ascii")
    split = preflight._private_tree_forbidden_tokens(
        root,
        (contact, str(root).encode("ascii"), v38, v319),
    )
...
def _make_result_gate_repo(
    ...
    *,
    ...
    comparison = (
        ...
        "| SEC/Gemma lean science v3.18 | Frozen | Rejected | No | Preserved |\n"
    ).encode("utf-8")
    ...
    v319 = root / contract.PRIVATE_NAMESPACE
    v38 = root / preflight.V38_PRIVATE_ROOT
    v319.mkdir(parents=True)
    v38.mkdir(parents=True)
    (v319 / "synthetic.json").write_bytes(
        contract.canonical_json_bytes(
            {"sealed": True, "bound_private_path": str(v319)}
        )
    )
...
@pytest.mark.parametrize("secret_kind", ("contact", "v38_path"))
def test_pushed_result_private_tree_allows_v319_path_but_rejects_secrets(
    tmp_path: Path,
    secret_kind: str,
) -> None:
    fixture = _make_result_gate_repo(tmp_path)
...
def test_publication_recovery_accepts_only_exact_idempotent_partial_bytes(
    ...
    result_pending = root / f"{contract.RESULT_ARTIFACT_PATH}.v319-pending"
    result_pending.write_bytes(result_bytes)
    comparison_path = root / contract.COMPARISON_PATH
    derived = runner.build_comparison_update(fixture["comparison_before"], public)
    pending = root / f"{contract.COMPARISON_PATH}.v319-pending"
...
def test_publication_recovery_accepts_only_exact_pause_pending_bytes(
    ...
    pause_path = root / contract.PAUSE_ARTIFACT_PATH
    pause_pending = root / f"{contract.PAUSE_ARTIFACT_PATH}.v319-pending"
