...
def test_canonical_json_and_hash_helpers_are_exact_and_redacted() -> None:
    ...
    _assert_code(
        "v316_contract_json_nonfinite", contract.canonical_json_bytes, {"x": float("nan")}
    )
    _assert_code(
        "v316_contract_json_key", contract.canonical_json_bytes, {1: "not allowed"}
    )
    _assert_code(
        "v316_contract_json_type", contract.canonical_json_bytes, {"x": object()}
    )
    _assert_code("v316_contract_hash_input", contract.sha256_bytes, "bytes required")
...
def test_generic_self_hash_is_strict_and_detached() -> None:
    ...
    _assert_code("v316_contract_self_hash_mismatch", contract.validate_self_sha256, changed)
    _assert_code("v316_contract_self_hash_present", contract.add_self_sha256, sealed)
...
def test_frozen_science_projection_is_exact_38320_bytes() -> None:
    ...
    _assert_code(
        "v316_contract_science_projection_hash",
    ...
    missing.pop("ledger")
    _assert_code(
        "v316_contract_science_projection_keys",
        contract.verify_frozen_science_projection,
        missing,
    )
...
def test_three_git_authority_families_are_exact_and_disjoint() -> None:
    assert (contract.BASE_COMMIT, contract.BASE_TREE, contract.BASE_PARENT) == (
        "7e124c309356fbba8c43868fd46c23ceb09137dd",
        "6e1822bcb509d0abd318404f3c81e415a3a1a12c",
    ...
    predecessor = contract.build_v315_rejection_pins()
    assert predecessor["preregistration"] == {
        "commit": "3955520674a3b6ccfa29cf9fc6593e110917e437",
        "tree": "c07f5893b235b145914cb115e1888fb360d59b34",
        "path": "docs/aapl_sec_gemma_lean_science_v3_15.md",
        "git_blob_sha1": "0d00607ac6a8fc548987c038e951f0c831437097",
        "literal_sha256": (
            "48af36ce45277be93fdb0af6e2a5173be725f8305f03ad956dec8e32cce37d65"
        ),
        "literal_bytes": 24_926,
    }
    assert predecessor["implementation"] == {
        "commit": "c3a83f1e676d3c8c80d0224954d2e9ccc95ba30a",
        "tree": "2206d5dddcddf1122c05ea167ceffccd03b3cc34",
        "parent": predecessor["preregistration"]["commit"],
    }
    assert predecessor["preflight_failure"] == {
        "commit": contract.BASE_PARENT,
        "tree": "4f49c1e2402674c193f24b1328c31b8ce6278805",
        "parent": predecessor["implementation"]["commit"],
        "path": (
            "e/aapl_sec_gemma_lean_science_v3_15/DEVELOPMENT_PREFLIGHT.json"
        ),
        "git_blob_sha1": "0a6781d8cb8d9b8eb59245608dddee75ee9cf21b",
        "literal_sha256": (
    ...
    assert predecessor["rejection"]["literal_bytes"] == 34_082
    assert predecessor["phase1_node_count"] == 324
    assert predecessor["phase1_case_sensitive_unique_node_count"] == 324
    assert predecessor["phase1_duplicate_node_count"] == 0
    assert predecessor["phase1_required_node_count"] == 23
    ...
    assert len(
        {
            ...
            contract.PREREGISTRATION_COMMIT,
            contract.V315_PREREGISTRATION_COMMIT,
            contract.V315_IMPLEMENTATION_COMMIT,
            contract.V315_PREFLIGHT_FAILURE_COMMIT,
            contract.V38_SOURCE_COMMIT,
...
def test_effect_budgets_cover_normal_pause_resume_and_preflight() -> None:
    ...
    _assert_code(
        "v316_contract_effect_counts",
        contract.validate_effect_counts,
        changed,
        route="normal_complete",
    )
    _assert_code(
        "v316_contract_effect_route",
        contract.validate_effect_counts,
        changed,
        route="best_result",
    )
...
def test_pilot_projection_has_strict_greater_than_boundary() -> None:
    ...
    _assert_code("v316_contract_pilot_durations", contract.projected_pilot_ns, exact[:4])
    _assert_code(
        "v316_contract_pilot_durations",
...
def test_only_literal_development_command_is_accepted() -> None:
    ...
    for rejected in ("confirmation", "final", "development ", None, b"development"):
        _assert_code("v316_contract_command", contract.validate_command, rejected)
...
def test_local_production_closure_and_import_upper_bound_are_exact() -> None:
    ...
    _assert_code(
        "v316_contract_local_import_duplicate",
        contract.validate_local_import_paths,
        (observed[0], observed[0]),
    )
    _assert_code(
        "v316_contract_local_import_outside_closure",
        contract.validate_local_import_paths,
        ("agent_benchmark/unpreregistered_strategy.py",),
    )
    _assert_code(
        "v316_contract_local_import_shape",
        contract.validate_local_import_paths,
...
def test_qualification_scope_schemas_runtime_and_counts_are_frozen() -> None:
    ...
    assert qualification["phases"] == ["v316", "dependencies", "requests_identity"]
...
def test_contract_manifest_is_strict_self_hashed_and_detached() -> None:
    ...
    assert manifest["v315_rejection_authority"] == (
        contract.build_v315_rejection_pins()
    )
    ...
    _assert_code(
        "v316_contract_manifest_mismatch", contract.validate_contract_manifest, changed
    )
...
def test_preregistration_implementation_and_preflight_ancestry() -> None:
    ...
    _assert_code(
        "v316_contract_preflight_ancestry",
...
def test_implementation_allowlist_and_public_paths_are_exact() -> None:
    ...
    assert contract.IMPLEMENTATION_PRODUCTION_PATHS[0].endswith("v316_contract.py")
    assert contract.IMPLEMENTATION_TEST_PATHS[0].endswith("v316_contract.py")
