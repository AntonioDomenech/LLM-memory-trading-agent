...
from agent_benchmark.sec_gemma_lean_science_v319_bridge import (
...
def _request_with_sentences(
    ...
    *,
    ...
    request = _blinded_request(
        proof,
        marker=marker,
        request_bytes=payload,
    )
    request["supplied_sentence_ids"] = [
        sentence["id"] for sentence in sentences
    ]
    return request


def _direction_sanitized_event_for_sentences(
    sentences: list[dict[str, str]],
    *,
    marker: str = "0",
) -> dict[str, Any]:
    """Build one exact private sanitized envelope for request-gate tests."""

    current_count = sum(item["id"].startswith("C") for item in sentences)
    prior_count = sum(item["id"].startswith("P") for item in sentences)
    model_payload = build_extractor_model_payload(sentences)
    body = {
        "schema_version": contract.DIRECTION_SANITIZED_EVENT_SCHEMA_VERSION,
        "sanitizer_schema_version": contract.DIRECTION_SANITIZER_SCHEMA_VERSION,
        "source_preprocessed_event_sha256": hashlib.sha256(
            f"bridge-source-{marker}".encode("ascii")
        ).hexdigest(),
        "source_sentence_count": len(sentences),
        "removed_current_sentence_count": 0,
        "removed_prior_sentence_count": 0,
        "retained_current_sentence_count": current_count,
        "retained_prior_sentence_count": prior_count,
        "sentences": copy.deepcopy(sentences),
        "sentences_sha256": contract.canonical_sha256(sentences),
        "model_payload": model_payload,
        "model_payload_sha256": contract.canonical_sha256(model_payload),
    }
    return {
        **body,
...
def _sentences_from_request(request: dict[str, Any]) -> list[dict[str, str]]:
    decoded = json.loads(request["request_bytes"].decode("utf-8", errors="strict"))
    user_payload = json.loads(decoded["messages"][1]["content"])
    return copy.deepcopy(user_payload["sentences"])


def _safe_sentences_for_proof(proof: Any) -> list[dict[str, str]]:
    sentences = [{"id": "C0001", "text": "Safe public business sentence."}]
    if proof["prior_same_form_record"] is not None:
        sentences.append(
...
def _validate_bound_blinded_request(
    value: dict[str, Any],
    *,
    proof: Any,
) -> dict[str, Any]:
    """Enter the V3.19 gate with a real sanitized-event binding.

    Malformed/noncanonical request bytes deliberately receive a valid safe
    envelope, so the new binding gate rejects them instead of the fixture
    failing before the contract is called.
    """

    try:
        sentences = _sentences_from_request(value)
        canonical_request = True
    except Exception:
        sentences = _safe_sentences_for_proof(proof)
        canonical_request = False
    sanitized_event = _direction_sanitized_event_for_sentences(
        sentences,
        marker=value.get("request_sha256", "0"),
    )
    bound = copy.deepcopy(value)
    if canonical_request:
        # Ordinary canonical fixtures are bound exactly.  Tests that need to
        # exercise a stale outer field mutate it after this helper is built.
        bound["preprocessed_event_sha256"] = sanitized_event[
            "preprocessed_event_sha256"
        ]
    validated = contract.validate_blinded_model_request(
        bound,
        proof=proof,
...
def _model_slice_material(
    proofs: list[Any],
    primary_documents: tuple[Any, ...],
) -> tuple[
    list[dict[str, Any]],
    list[dict[str, Any]],
    list[dict[str, Any]],
]:
    """Build the three request/source/sanitized sequences bound to proofs."""

    normalized_text_by_accession = {
    ...
    for proof in proofs:
        current = proof["current_record"]
        current_text = normalized_text_by_accession[current["accession_number"]]
        prior = proof["prior_same_form_record"]
        prior_text = (
            None
            if prior is None
            else normalized_text_by_accession[prior["accession_number"]]
        )
        ...
        request_bytes = contract.canonical_json_bytes(sanitized["model_payload"])
        request = {
            "schema_version": contract.BLINDED_MODEL_REQUEST_SCHEMA_VERSION,
            "accession_number": current["accession_number"],
            "form": current["form"],
    ...
    return requests, source_events, sanitized_events
...
def test_proof_rejects_a_self_consistent_wrong_top_level_authority_hash(
    ...
    _assert_contract_code(
        "v319_contract_universe_event_proof_identity",
...
def test_proof_rejects_duplicate_compatibility_membership(
    ...
    _assert_contract_code(
        "v319_contract_proof_compatibility_duplicate",
        contract.validate_nullable_universe_event_proof,
...
def test_blinded_request_rejects_current_private_locator_bytes(
    ...
    assert _validate_bound_blinded_request(valid, proof=proof) == valid
    ...
    _assert_contract_code(
        ...
        _validate_bound_blinded_request,
        leaked,
        proof=proof,
    )
...
def test_blinded_request_rejects_every_current_source_and_proof_identity_token(
    ...
    for token in sorted(tokens):
        payload = contract.canonical_json_bytes(
            build_extractor_model_payload(
                [{"id": "C0001", "text": f"safe {token} sentence"}]
            )
        )
        leaked = _blinded_request(proof, marker="2", request_bytes=payload)
        with pytest.raises(contract.ContractViolation) as captured:
            _validate_bound_blinded_request(leaked, proof=proof)
        assert captured.value.code == (
            "v319_contract_blinded_request_private_identity"
        ), token
        ...
        payload = contract.canonical_json_bytes(
            build_extractor_model_payload(
                [
                    ...
                    {"id": "P0001", "text": f"safe {token} sentence"},
                ]
            )
        )
...
def test_blinded_request_rejects_structured_current_and_prior_missingness(
    ...
    assert _validate_bound_blinded_request(safe, proof=proof) == safe
...
def test_blinded_request_rejects_plain_text_identity_outcome_and_missingness(
    ...
    expected_code = (
        "v319_contract_blinded_request_sanitized_event"
        if (
            not leaked_text.isascii()
            or contract._blinded_text_has_security_direction(
                {"id": "C0001", "text": leaked_text}
            )
        )
        else "v319_contract_blinded_request_private_identity"
    )
    _assert_contract_code(
        expected_code,
        _validate_bound_blinded_request,
...
def test_blinded_request_plain_text_guard_preserves_unrelated_business_language(
    ...
    for safe_text in safe_texts:
        ...
        assert _validate_bound_blinded_request(safe, proof=proof) == safe, safe_text


def test_blinded_request_allows_undirected_accounting_value_and_rejects_local_market_direction(
    nullable_projection: tuple[_Fixture, ScienceBridgeProjection],
) -> None:
    _, result = nullable_projection
    proof = next(
        item
        for item in result.universe_event_proofs
        if item["prior_same_form_record"] is None
    )

    allowed = (
        "Stock value supports compensation accounting.",
        "Lower operating costs affected stock compensation.",
        "Stock; lower business costs.",
    )
    forbidden = (
        "Shares rose.",
        "Shares moved very sharply lower.",
        "Market rose.",
        "Security gained.",
        "Stock was up.",
        "Stock value rose.",
        "Stock prices were materially lower.",
        "Higher share price.",
        "Lower market value.",
    )

    for index, text in enumerate(allowed, start=100):
        request = _request_with_sentences(
            proof,
            marker=str(index),
            sentences=[{"id": "C0001", "text": text}],
        )
        assert _validate_bound_blinded_request(
            request, proof=proof
        ) == request

    for index, text in enumerate(forbidden, start=200):
        request = _request_with_sentences(
            proof,
            marker=str(index),
            sentences=[{"id": "C0001", "text": text}],
...
def test_canonical_request_cap_proves_tight_wrapper_maximum(
    nullable_projection: tuple[_Fixture, ScienceBridgeProjection],
) -> None:
    _, result = nullable_projection
    proof = next(
    ...
    ids = [
        *(f"C{index:04d}" for index in range(1, 37)),
        *(f"P{index:04d}" for index in range(1, 37)),
    ]

    def payload_for(text: str) -> bytes:
        return contract.canonical_json_bytes(
            build_extractor_model_payload(
                [{"id": sentence_id, "text": text} for sentence_id in ids]
            )
        )

    empty_payload = payload_for("")
    assert len(empty_payload) == 10_197

    total_text_characters = 72 * 220
    contributions: list[int] = []
    for codepoint in range(128):
        payload = payload_for(chr(codepoint) * 220)
        delta = len(payload) - len(empty_payload)
        assert delta % total_text_characters == 0
        contributions.append(delta // total_text_characters)

    assert max(contributions) == 7
    assert contributions[0] == 7

    sentences = [
        {"id": sentence_id, "text": "\x00" * 220}
    ...
    assert contract.MODEL_INPUT_MAX_BYTES == 20_000
    assert contract.MODEL_REQUEST_MAX_BYTES == 128 * 1024 == 131_072
    assert len("\n".join(item["text"] for item in sentences).encode()) == 15_911
    assert len(payload) == 121_077
    assert contract.MODEL_REQUEST_MAX_BYTES - len(payload) == 9_995

    decoded = json.loads(payload)
    replayed = contract._exact_blinded_sentences(
        decoded,
        request_bytes=payload,
        supplied_sentence_ids=ids,
        has_prior=True,
    )
    assert replayed == tuple(sentences)

    identity_tokens = contract._blinded_request_identity_tokens(proof)
    assert not contract._sentences_violate_frozen_preprocessor(
        replayed,
        identity_tokens=identity_tokens,
    )
    assert not contract._blinded_request_contains_private_identity(
        payload,
        proof,
        supplied_sentence_ids=ids,
    )
    ...
    oversized = _blinded_request(
        proof,
        marker="301",
        request_bytes=b"x" * 131_073,
    )
    _assert_contract_code(
        "v319_contract_blinded_request_sanitized_event",
        _validate_bound_blinded_request,
...
def test_security_direction_single_linker_matrix_is_complete_and_bounded(
    nullable_projection: tuple[_Fixture, ScienceBridgeProjection],
) -> None:
    _, result = nullable_projection
    proof = next(
    ...
    regex = contract._BLINDED_TEXT_SECURITY_DIRECTION_RE
    ...
    assert set(linker_cases) == {
        "are", "closed", "had", "has", "have", "is",
        "moved", "moving", "traded", "was", "were",
    }
    ...
    for index, text in enumerate(forbidden, start=400):
        assert regex.search(text.casefold()) is not None, text
        request = _request_with_sentences(
            proof,
            marker=str(index),
            sentences=[{"id": "C0001", "text": text}],
    ...
    for index, text in enumerate(allowed, start=500):
        assert regex.search(text.casefold()) is None, text
        request = _request_with_sentences(
            proof,
            marker=str(index),
            sentences=[{"id": "C0001", "text": text}],
...
def test_model_slice_excludes_bytes_but_request_hash_binds_them(
    ...
    requests, source_events, sanitized_events = _model_slice_material(
        proofs,
        result.primary_documents,
    )
    model_slice = contract.build_nullable_model_slice(
        requests,
        proofs,
        source_events,
        sanitized_events,
    ...
    changed_requests = requests[1:] + requests[:1]
    changed_proofs = proofs[1:] + proofs[:1]
    changed_sources = source_events[1:] + source_events[:1]
    changed_sanitized = sanitized_events[1:] + sanitized_events[:1]
    changed_slice = contract.build_nullable_model_slice(
        changed_requests,
        changed_proofs,
        changed_sources,
        changed_sanitized,
...
def test_model_slice_rejects_wrong_request_proof_order_and_count(
    ...
    source_events[0], source_events[1] = source_events[1], source_events[0]
    sanitized_events[0], sanitized_events[1] = (
        sanitized_events[1],
        sanitized_events[0],
    )
    _assert_contract_code(
        "v319_contract_blinded_request_identity",
    ...
    _assert_contract_code(
        "v319_contract_model_slice_count",
        contract.build_nullable_model_slice,
        requests[:-1],
        proofs[:-1],
        source_events[:-1],
