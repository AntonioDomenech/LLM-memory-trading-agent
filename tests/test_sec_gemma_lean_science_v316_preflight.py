...
from tests import test_sec_gemma_lean_science_v316_bridge as bridge_fixtures
...
def _assert_code(expected_code: str, function, *args, **kwargs) -> None:
    with pytest.raises(preflight.V316PreflightError) as captured:
...
def _bridge_manifest() -> dict[str, object]:
    body = {
        "schema_version": "aapl-sec-gemma-lean-science-v3-16-streaming-bridge-v1",
        "stage": "development",
        "document_count": 75,
        ...
        repository_rows.append(
            {
                ...
                "role": role,
                "owning_revision": (
                    implementation_commit if role == "v316" else contract.BASE_COMMIT
                ),
...
def _write_qualification_private_state(
    ...
    *,
    ...
    ancestry = repository["ancestry"]
    phase_nodes = {
        contract.QUALIFICATION_PHASE_V316: [
            *preflight._RUNTIME_REQUIRED_PHASE1_NODE_IDS,
            *(
                f"{v316_node_path}::test_synthetic_{index:03d}"
                for index in range(
...
def test_phase2_rejects_case_insensitive_node_id_collapsing() -> None:
    ...
    # semantics now see 654 unique IDs and one duplicate, so V3.16 rejects it.
    nodes[-1] = nodes[-2]
    digest = hashlib.sha256("\n".join(nodes).encode("utf-8")).hexdigest()
        ...
        **{
            **dependencies.__dict__,
            ...
            "run_qualification": lambda _root: (_ for _ in ()).throw(
                preflight.V316PreflightError("stop_after_reservation")
            ),
        }
    )
    _assert_code(
        ...
        **{
            **dependencies.__dict__,
            ...
            "authenticate_source": lambda _root: (_ for _ in ()).throw(
                preflight.V316PreflightError("stop_after_authority_creation")
            ),
        }
    )
    _assert_code(
...
def test_deadline_overrun_completion_fails_without_mutating_self_hashed_result() -> None:
    ...
    completed = preflight._qualification_completion_receipt(
        phase=contract.QUALIFICATION_PHASE_V316,
        mode="execution",
        result=result,
...
def test_private_qualification_replay_rejects_internally_consistent_wrong_selector(
    ...
    report = _write_qualification_private_state(
        root,
        repository,
        v316_node_path="tests/test_unrelated.py",
    )
    _assert_code(
        "execution_preflight_qualification_invalid",
        preflight._validate_qualification_private_state,
...
def test_v316_local_import_upper_bound_covers_static_and_fake_runtime_paths(
    tmp_path: Path,
) -> None:
    ...
    for index, relative in enumerate(contract.IMPLEMENTATION_PRODUCTION_PATHS):
        ...
        if index == 0:
            body += (
                "import importlib\n"
                "from . import sec_gemma_lean_science_v316_bridge\n"
    ...
    observed = preflight._audit_v316_local_import_upper_bound(
        root,
        head_commit=head,
    ...
    _assert_code(
        ...
        preflight._audit_v316_local_import_upper_bound,
        root,
        head_commit=head,
    ...
    dynamic_source.write_text(
        "import importlib\n"
        "module_name = 'agent_benchmark.sec_gemma_lean_science_v316_bridge'\n"
...
def test_real_v316_production_import_calls_are_literal_and_upper_bound_closes(
    tmp_path: Path,
) -> None:
    root = Path(__file__).resolve().parents[1]
    production_paths = (
    ...
    observed_paths = preflight._audit_v316_local_import_upper_bound(
        root,
        head_commit=head,
    ...
    for index, path in enumerate(production_paths):
        ...
        if index == 0:
            target.write_bytes(
                b"import importlib\n"
                b"module_name = "
                b"'agent_benchmark.sec_gemma_lean_science_v316_bridge'\n"
...
def test_pushed_execution_authority_replays_public_private_and_source_inventory(
    ...
    _git(work, "commit", "-m", "synthetic v3.16 implementation")
    ...
    _git(work, "commit", "-m", "synthetic v3.16 preflight evidence")
...
def test_private_tree_token_split_allows_bound_paths_only(tmp_path: Path) -> None:
    ...
    v316 = str(root / contract.PRIVATE_NAMESPACE).encode("ascii")
    split = preflight._private_tree_forbidden_tokens(
        root,
        (contact, str(root).encode("ascii"), v38, v316),
    )
...
def _make_result_gate_repo(
    ...
    *,
    ...
    comparison = (
        ...
        "| SEC/Gemma lean science v3.15 | Frozen | Rejected | No | Preserved |\n"
    ).encode("utf-8")
    ...
    v316 = root / contract.PRIVATE_NAMESPACE
    v38 = root / preflight.V38_PRIVATE_ROOT
    v316.mkdir(parents=True)
    v38.mkdir(parents=True)
    (v316 / "synthetic.json").write_bytes(
        contract.canonical_json_bytes(
            {"sealed": True, "bound_private_path": str(v316)}
        )
    )
...
@pytest.mark.parametrize("secret_kind", ("contact", "v38_path"))
def test_pushed_result_private_tree_allows_v316_path_but_rejects_secrets(
    tmp_path: Path,
    secret_kind: str,
) -> None:
    fixture = _make_result_gate_repo(tmp_path)
...
def test_publication_recovery_accepts_only_exact_idempotent_partial_bytes(
    ...
    result_pending = root / f"{contract.RESULT_ARTIFACT_PATH}.v316-pending"
    result_pending.write_bytes(result_bytes)
    comparison_path = root / contract.COMPARISON_PATH
    derived = runner.build_comparison_update(fixture["comparison_before"], public)
    pending = root / f"{contract.COMPARISON_PATH}.v316-pending"
...
def test_publication_recovery_accepts_only_exact_pause_pending_bytes(
    ...
    pause_path = root / contract.PAUSE_ARTIFACT_PATH
    pause_pending = root / f"{contract.PAUSE_ARTIFACT_PATH}.v316-pending"
