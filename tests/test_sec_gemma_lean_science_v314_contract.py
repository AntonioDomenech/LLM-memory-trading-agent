...
def test_canonical_json_and_hash_helpers_are_exact_and_redacted() -> None:
    ...
    _assert_code(
        "v314_contract_json_nonfinite", contract.canonical_json_bytes, {"x": float("nan")}
    )
    _assert_code(
        "v314_contract_json_key", contract.canonical_json_bytes, {1: "not allowed"}
    )
    _assert_code(
        "v314_contract_json_type", contract.canonical_json_bytes, {"x": object()}
    )
    _assert_code("v314_contract_hash_input", contract.sha256_bytes, "bytes required")
...
def test_generic_self_hash_is_strict_and_detached() -> None:
    ...
    _assert_code("v314_contract_self_hash_mismatch", contract.validate_self_sha256, changed)
    _assert_code("v314_contract_self_hash_present", contract.add_self_sha256, sealed)
...
def test_frozen_science_projection_is_exact_38320_bytes() -> None:
    ...
    _assert_code(
        "v314_contract_science_projection_hash",
    ...
    missing.pop("ledger")
    _assert_code(
        "v314_contract_science_projection_keys",
        contract.verify_frozen_science_projection,
        missing,
    )
...
def test_three_git_authority_families_are_exact_and_disjoint() -> None:
    assert (contract.BASE_COMMIT, contract.BASE_TREE, contract.BASE_PARENT) == (
        "efe541664c95863631fec470fdb11ea2fef6b599",
        "2fa9a5c1afec001f8046d87a0a26ef60b677d458",
        "2f996bc963807f4a901676589b0b66d93855c184",
    )
    assert (
    ...
    predecessor = contract.build_v313_rejection_pins()
    assert predecessor["preregistration"] == {
        "commit": "679cebcfc8c86124be12c1a7ebbce2171735712d",
        "tree": "f6880ecc0c6fa878b8c3798ae782183afa896113",
        "path": "docs/aapl_sec_gemma_lean_science_v3_13.md",
        "git_blob_sha1": "8c2a32ae424d820ec73d2d053b588c1e3dda4a96",
        "literal_sha256": (
            "600806541de00846e13f2a1b6a7b4b0bdff09e8d8d3e0432635bfc9b3dab00b2"
        ),
        "literal_bytes": 38_652,
    }
    assert predecessor["implementation"] == {
        "commit": contract.BASE_PARENT,
        "tree": "7fe69f3cd715e27a6c3eb09fed402419591ba118",
    ...
    assert predecessor["rejection"]["literal_bytes"] == 4_932
    assert predecessor["phase1_node_count"] == 322
    assert predecessor["phase1_case_sensitive_unique_node_count"] == 322
    assert predecessor["phase1_duplicate_node_count"] == 0
    assert predecessor["phase1_required_node_count"] == 21
    assert predecessor["official_qualification_run_count"] == 0
...
def test_effect_budgets_cover_normal_pause_resume_and_preflight() -> None:
    ...
    _assert_code(
        "v314_contract_effect_counts",
        contract.validate_effect_counts,
        changed,
        route="normal_complete",
    )
    _assert_code(
        "v314_contract_effect_route",
        contract.validate_effect_counts,
        changed,
        route="best_result",
    )
...
def test_pilot_projection_has_strict_greater_than_boundary() -> None:
    ...
    _assert_code("v314_contract_pilot_durations", contract.projected_pilot_ns, exact[:4])
    _assert_code(
        "v314_contract_pilot_durations",
...
def test_only_literal_development_command_is_accepted() -> None:
    ...
    for rejected in ("confirmation", "final", "development ", None, b"development"):
        _assert_code("v314_contract_command", contract.validate_command, rejected)
...
def test_local_production_closure_and_import_upper_bound_are_exact() -> None:
    ...
    _assert_code(
        "v314_contract_local_import_duplicate",
        contract.validate_local_import_paths,
        (observed[0], observed[0]),
    )
    _assert_code(
        "v314_contract_local_import_outside_closure",
        contract.validate_local_import_paths,
        ("agent_benchmark/unpreregistered_strategy.py",),
    )
    _assert_code(
        "v314_contract_local_import_shape",
        contract.validate_local_import_paths,
...
def test_qualification_scope_schemas_runtime_and_counts_are_frozen() -> None:
    ...
    assert qualification["phases"] == ["v314", "dependencies", "requests_identity"]
...
def test_contract_manifest_is_strict_self_hashed_and_detached() -> None:
    ...
    assert manifest["v313_rejection_authority"] == (
        contract.build_v313_rejection_pins()
    )
    ...
    _assert_code(
        "v314_contract_manifest_mismatch", contract.validate_contract_manifest, changed
    )
...
def test_preregistration_implementation_and_preflight_ancestry() -> None:
    ...
    _assert_code(
        "v314_contract_preflight_ancestry",
...
def test_implementation_allowlist_and_public_paths_are_exact() -> None:
    ...
    assert contract.IMPLEMENTATION_PRODUCTION_PATHS[0].endswith("v314_contract.py")
    assert contract.IMPLEMENTATION_TEST_PATHS[0].endswith("v314_contract.py")
