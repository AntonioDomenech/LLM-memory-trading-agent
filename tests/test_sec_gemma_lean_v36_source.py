...
import agent_benchmark.sec_gemma_lean_v36_source as source_module
...
def test_q1_q2_master_authority_is_mandatory_and_q1_must_match() -> None:
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="quarter evidence set"):
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="target set differs"):
...
def test_submissions_snapshot_accepts_unoccupied_inclusive_bounds_and_layout() -> None:
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="repeats"):
        parse_submissions_snapshot(duplicate, {history_name: history})

    duplicate_key = main.replace(b'"cik":320193', b'"cik":320193,"cik":320193')
    with pytest.raises(SecGemmaLeanV36SourceError, match="duplicate JSON object key"):
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="zero/one flag"):
        parse_submissions_snapshot(
...
def test_strict_master_gzip_and_exact_apple_projection() -> None:
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="trailing|concatenated"):
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="repeated raw row"):
        parse_strict_master_gzip(repeated, year=2024, quarter=1)
    malformed = gzip.compress(
        (MASTER_HEADER + "\n320193|APPLE|10-Q|2024-02-01\n").encode(),
        mtime=0,
    )
    with pytest.raises(SecGemmaLeanV36SourceError, match="five fields"):
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="line-control"):
...
def test_blank_company_apple_targets_and_other_empty_fields_reject() -> None:
    ...
    for line, message in empty_cases:
        with pytest.raises(SecGemmaLeanV36SourceError, match=message):
            parse_strict_master_gzip(
                _master_payload([line]), year=2024, quarter=1
            )
...
def test_cross_stage_rejects_each_frozen_prefix_drift_kind(
    ...
    with pytest.raises(SecGemmaLeanV36SourceError):
...
def test_authoritative_calendar_is_exact_and_compact_identity_is_sealed() -> None:
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="authoritative"):
        reconcile_complete_submission(
            target,
    ...
    _reseal(forged)
    with pytest.raises(SecGemmaLeanV36SourceError, match="calendar"):
        validate_stage_source_seal(forged)
...
def test_pre_master_submissions_bounds_are_rejection_only_and_exact() -> None:
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="I upper bound"):
...
def test_prior_raw_chain_is_replayed_before_subtraction_and_deep_snapshotted() -> None:
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="authenticate|replay"):
        derive_complete_submission_plan(
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="prior bundle"):
        derive_complete_submission_plan(
            final.master_reconciliation,
...
def test_cross_stage_prior_hash_and_exact_p_carry_are_not_forgeable() -> None:
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="hash/full chain"):
...
def test_u_d_and_science_use_availability_order_while_p_uses_filing_order() -> None:
    ...
    _reseal(forged)
    with pytest.raises(SecGemmaLeanV36SourceError, match="U/D"):
        validate_stage_source_seal(forged)
...
def test_final_completed_year_gate_is_mandatory_and_reconstructed() -> None:
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="year 2000"):
        validate_completed_year_source_coverage(rows)

    forged = deepcopy(final.seal)
    forged["completed_year_source_coverage"]["years"][0]["10-K"] = 0
    _reseal(forged)
    with pytest.raises(SecGemmaLeanV36SourceError, match="completed-year source gate"):
        validate_stage_source_seal(forged)

    wrong_stage = deepcopy(development.seal)
    wrong_stage["completed_year_source_coverage"] = {"forged": True}
    _reseal(wrong_stage)
    with pytest.raises(SecGemmaLeanV36SourceError, match="only in the final"):
...
def test_plain_header_form_ascii_horizontal_whitespace_is_family_neutral() -> None:
    ...
    for tagged_response in (sec_tagged, ims_tagged):
        for invalid_line in invalid_lines:
            with pytest.raises(SecGemmaLeanV36SourceError, match="form"):
...
def test_raw_header_and_all_document_form_sequence_topology_must_be_exact() -> None:
    ...
    for duplicate_response in duplicate_fields:
        ...
        # behavior; singular raw fields are a v3.6 adapter rule only.
...
def test_gzip_corrupt_truncated_and_both_size_caps(
    ...
    corrupt[-8] ^= 1
    with pytest.raises(SecGemmaLeanV36SourceError, match="CRC|gzip"):
        parse_strict_master_gzip(bytes(corrupt), year=2024, quarter=1)
    with pytest.raises(SecGemmaLeanV36SourceError, match="truncated|CRC"):
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="compressed byte cap"):
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="decompressed byte cap"):
        parse_strict_master_gzip(payload, year=2024, quarter=1)
...
def test_master_exact_set_rejects_missing_unexpected_and_forged_evidence() -> None:
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="set is not exact"):
        reconcile_master_exact_set(
            development.submissions_snapshot,
            evidence[:-1],
            "development",
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="set differs"):
        reconcile_master_exact_set(
            development.submissions_snapshot,
            missing,
            "development",
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="authenticate"):
        reconcile_master_exact_set(
            development.submissions_snapshot,
            forged,
            "development",
...
def test_compact_replay_rejects_blob_receipt_evidence_and_callback_tampering() -> None:
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="hash/length"):
        detached_replay(
            "development",
            replay.checkpoint,
            manifests,
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="evidence differs"):
        detached_replay(
            "development",
            evidence_checkpoint,
            mismatched_evidence,
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="hash/bindings"):
        detached_replay(
            "development",
            replay.checkpoint,
            tampered_receipt,
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="failed safely"):
        detached_replay(
            "development",
            replay.checkpoint,
            manifests,
...
def test_compact_replay_rejects_missing_orphan_duplicate_and_misordered_roles() -> None:
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="missing required role"):
        detached_replay(
            "development",
            missing_checkpoint,
            missing,
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="orphan"):
        detached_replay(
            "development",
            orphan_checkpoint,
            orphan,
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="duplicated"):
        detached_replay(
            "development",
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="misordered|mismatched"):
        detached_replay(
            "development",
...
def test_compact_replay_rejects_resealed_checkpoint_and_recursive_prior_blob_tamper() -> None:
    ...
    with pytest.raises(SecGemmaLeanV36SourceError, match="source seal differs"):
        detached_replay(
            "development",
