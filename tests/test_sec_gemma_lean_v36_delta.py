...
V36_DELTA_MODULE_SHA256 = "34c865edd755932219aa7da634558c44bd8f3d193b8e33e6461b43cc6314ddf8"
LOGIC_PATH = "agent_benchmark/logic.py"
TEST_PATH = "tests/test_logic.py"
DOC_PATH = "docs/prereg.md"
EXPECTED_V36_COUNTERPARTS = {
...
def _make_counterpart_repo(tmp_path: Path) -> dict[str, Any]:
    ...
    _git(repo, "config", "user.email", "offline@example.invalid")
    _git(repo, "config", "core.autocrlf", "false")
    old_path = "agent_benchmark/v35_logic.py"
    new_path = "agent_benchmark/v36_logic.py"
    source = b'def run():\n    return "source-only"\n'
...
def test_default_git_pins_are_real_exact_objects_and_prereg_is_doc_only() -> None:
    ...
    prereg = "18e3c47b4e3b29449d95a241b0989d60487c623f"
    prereg_tree = "b75ded8fcddf81f80a0a0f4d17a150d47d2c260d"
    base = "d1fec255a5ae6a77a047906683fc3e1f50f49298"
    base_tree = "d85a6a79ac2f198ce50daf3aaeb25fbe8ff8b646"
    doc_path = "docs/aapl_sec_gemma_lean_evidence_v3_6.md"
    doc_blob = "81dea7bddb023716e2615302a31e9be88743b3d0"
    doc_sha256 = "8ef18188b1813f0ab759fc705205b528bc39fddf56472f15b44da9d1bfa99d73"
...
def test_default_new_file_allow_inventories_match_frozen_worktree_structure() -> None:
    repo = Path(__file__).resolve().parents[1]
    expected_paths = tuple(sorted(EXPECTED_V36_COUNTERPARTS))
    ...
    assert tuple(sorted(delta._V36_IMPLEMENTATION_PATHS)) == expected_paths
...
def test_committed_counterpart_rejects_same_symbol_import_hostile_body(
    ...
    # committed-v3.5 semantic comparison.
...
def test_checker_rejects_blank_company_scope_expansion_row_drop_and_special_case(
    ...
    new_path = "agent_benchmark/sec_gemma_lean_v36_source.py"
    baseline = (
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ({"counterpart_path": "agent_benchmark/missing_v35.py"}, "Git rejected"),
    ),
)
...
def test_external_anchor_rejects_delta_body_with_stale_test_literal(
    ...
    _git(repo, "config", "user.name", "Offline Test")
    _git(repo, "config", "user.email", "offline@example.invalid")
    old_path = "agent_benchmark/sec_gemma_lean_v35_delta.py"
    new_path = "agent_benchmark/sec_gemma_lean_v36_delta.py"
    anchor_path = "tests/test_sec_gemma_lean_v36_delta.py"
    ...
    _write(
        repo,
        anchor_path,
        f'V36_DELTA_MODULE_SHA256 = "{benign_sha}"\n'.encode("ascii"),
    )
