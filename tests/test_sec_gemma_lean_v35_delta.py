...
V35_DELTA_MODULE_SHA256 = "7193512060f467808ffe28e8c6a045e14bb171a95b5adae66031da2058c67ad1"
LOGIC_PATH = "agent_benchmark/logic.py"
TEST_PATH = "tests/test_logic.py"
DOC_PATH = "docs/prereg.md"
EXPECTED_V35_COUNTERPARTS = {
...
def _make_counterpart_repo(tmp_path: Path) -> dict[str, Any]:
    ...
    _git(repo, "config", "user.email", "offline@example.invalid")
    _git(repo, "config", "core.autocrlf", "false")
    old_path = "agent_benchmark/v34_logic.py"
    new_path = "agent_benchmark/v35_logic.py"
    source = b'def run():\n    return "source-only"\n'
...
def test_default_git_pins_are_real_exact_objects_and_prereg_is_doc_only() -> None:
    ...
    prereg = "50d7e21a0171edf0f356622c2160f996296fd503"
    prereg_tree = "176ce7b59d89513a74078d487c0ea15ea40e64d6"
    base = "359dd70af9fd4d2fa352c4514e8cd9c10d625aa3"
    base_tree = "7d775f342fcf2199bbf6c8ae513d627ead886aa0"
    doc_path = "docs/aapl_sec_gemma_lean_evidence_v3_5.md"
    doc_blob = "92c4d0773dbd2b4338a1d00797861927480b4eab"
    doc_sha256 = "bdf2c07db01717cb2cf4eb4fe002e20a93d4ac0aa2e1a5b3534da8ff22259bf1"
...
def test_default_new_file_allow_inventories_match_frozen_worktree_structure() -> None:
    repo = Path(__file__).resolve().parents[1]
    expected_paths = tuple(sorted(EXPECTED_V35_COUNTERPARTS))
    ...
    assert tuple(sorted(delta._V35_IMPLEMENTATION_PATHS)) == expected_paths
...
def test_committed_counterpart_rejects_same_symbol_import_hostile_body(
    ...
    # committed-v3.4 semantic comparison.
...
def test_checker_rejects_blank_company_scope_expansion_row_drop_and_special_case(
    ...
    new_path = "agent_benchmark/sec_gemma_lean_v35_source.py"
    baseline = (
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ({"counterpart_path": "agent_benchmark/missing_v34.py"}, "Git rejected"),
    ),
)
...
def test_external_anchor_rejects_delta_body_with_stale_test_literal(
    ...
    _git(repo, "config", "user.name", "Offline Test")
    _git(repo, "config", "user.email", "offline@example.invalid")
    old_path = "agent_benchmark/sec_gemma_lean_v34_delta.py"
    new_path = "agent_benchmark/sec_gemma_lean_v35_delta.py"
    anchor_path = "tests/test_sec_gemma_lean_v35_delta.py"
    ...
    _write(
        repo,
        anchor_path,
        f'V35_DELTA_MODULE_SHA256 = "{benign_sha}"\n'.encode("ascii"),
    )
